...
由
\[
0=\operatorname{Var}(X-Y)=\operatorname{Var}(X)+\operatorname{Var}(Y)-2\operatorname{Cov}(X,Y)
=4+4-2\operatorname{Cov}(X,Y),
\]
得 $\operatorname{Cov}(X,Y)=4$。因此
\ansmath{\Sigma=
\begin{pmatrix}
4&4\\ 4&4
\end{pmatrix}}
...
\begin{question}
有 $n$ 个编号为 1 到 $n$ 的小球，$n$ 个编号为 1 到 $n$ 的盒子，将小球放入盒子之中，每个盒子放且仅放一个小球。小球编号与盒子编号相等称之为配对。求配对数 $X$ 的方差。
\begin{solution}
记 $I_i=\mathbf 1\{\pi(i)=i\}$，则 $X=\sum_{i=1}^n I_i$。有
\[
\operatorname{E}[I_i]=\frac1n,
\qquad \operatorname{E}[I_iI_j]=\frac{(n-2)!}{n!}=\frac{1}{n(n-1)}\ (i\ne j).
\]
故
\[
\operatorname{E}[X]=n\cdot\frac1n=1,
\]
\[
\operatorname{E}[X^2]=\sum_i\operatorname{E}[I_i]+2\sum_{i<j}\operatorname{E}[I_iI_j]
=1+2\binom n2\frac{1}{n(n-1)}=2.
\]
于是
\[
\operatorname{Var}(X)=\operatorname{E}[X^2]-\operatorname{E}[X]^2=2-1=1.
\]
\end{solution}
\end{question}
...
\begin{solution}
\[
P(T>t)=P(N_t=0)=\ee^{-\lambda t},\qquad t\ge 0.
\]
故
\[
F_T(t)=1-\ee^{-\lambda t},\qquad
f_T(t)=\lambda \ee^{-\lambda t}\ (t\ge 0).
\]
\end{solution}
\end{question}
...
\begin{question}
$X$ 和 $Y$ 都服从标准正态分布，$Z=X-Y$，求 $\operatorname{E}[Z]$，$\operatorname{E}[|Z|]$，$V[|Z|]$。
\begin{solution}
有 $Z\sim N(0,2)$，所以
\[
\operatorname{E}[Z]=0,
\qquad \operatorname{E}[|Z|]=\sqrt2\sqrt{\frac{2}{\pi}}=\frac{2}{\sqrt\pi}.
\]
又
\[
\operatorname{E}[|Z|^2]=\operatorname{E}[Z^2]=2,
\]
故
\[
\operatorname{Var}(|Z|)=2-\left(\frac{2}{\sqrt\pi}\right)^2=2-\frac{4}{\pi}.
\]
\end{solution}
\end{question}

\begin{question}
雷达显示屏是一个半径为 $R$ 的圆形区域，圆内随机出现一个光点且其位置服从均匀分布。求光点与圆心距离的期望和方差。（10 分）
\begin{solution}
距离 $r$ 的密度为
\[
...
因此
\[
\operatorname{E}[r]=\int_0^R r\frac{2r}{R^2}\dd{r}=\frac{2R}{3},
\qquad
\operatorname{E}[r^2]=\int_0^R r^2\frac{2r}{R^2}\dd{r}=\frac{R^2}{2}.
\]
故
\[
\operatorname{Var}(r)=\frac{R^2}{2}-\left(\frac{2R}{3}\right)^2=\frac{R^2}{18}.
\]
\end{solution}
\end{question}

\begin{question}
一个矩形，边长分别为 $a$ 和 $b$，且 $a>b$。在矩形的任意边上任取两个点，求这两个点的距离的平方的期望值。（10 分）
\begin{solution}
...
故
\[
\operatorname{E}[D^2]=2\operatorname{Var}(X)+2\operatorname{Var}(Y).
\]
对边界均匀分布，可算得
\[
\operatorname{E}[X]=\frac a2,
\qquad
\operatorname{E}[X^2]=\frac{2\int_0^a x^2\dd{x}+ba^2}{2(a+b)},
\]
\[
\operatorname{E}[Y]=\frac b2,
\qquad
\operatorname{E}[Y^2]=\frac{2\int_0^b y^2\dd{y}+ab^2}{2(a+b)}.
\]
化简后
\ansmath{\operatorname{E}[D^2]=\frac{a^2+2ab+b^2}{6}=\frac{(a+b)^2}{6}}
\end{solution}
\end{question}

\begin{question}
作业原题，见图 1。其题面如下：（10 分）
...
\begin{question}
某一个衰变事件（具体是什么我不记得了）的发生是极为稀有的事情。本底数为 3，现在观察到 6 个疑似信号，求显著性水平。（10 分）
\begin{solution}
\ansmath{p=P(N\ge 6\mid \lambda=3)
=1-\sum_{k=0}^{5}\ee^{-3}\frac{3^k}{k!}
\approx 0.08392}
\end{solution}
\end{question}
...
\]
问方差有没有明显差异？参考数据：卡方分布的两个上 $\alpha$ 分位数为 $\chi^2_{0.025}(14)=26.119$，$\chi^2_{0.975}(14)=5.629$。（10 分）
\begin{solution}
...
\begin{question}
蒲丰投针实验计算 $\pi$ 的公式为 $\pi\approx\dfrac{2lN}{an}$，其中 $N$ 为总投掷次数，$n$ 为针与线相交次数。某次实验中，$l=2.5\mathrm{cm}$，$a=3\mathrm{cm}$，$N=3408$，$n=1808$，于是算出 $\pi=3.1415929$。忽略 $l$ 和 $a$ 的误差，计算 $\pi$ 的不确定度。（7 分）
\begin{solution}
...
\begin{question}
仪器在任意时间内观察到噪声的概率相等。在 $[0,T]$ 的时间窗口内观察到两个噪声，如果两个噪声间距小于 $t$，就被误鉴别为信号。在时间窗口内已经观察到两个噪声的前提下，求误鉴别为信号的概率。（6 分）
\begin{solution}
...
即
\[
P(N_a=k)=\ee^{-\lambda}\frac{\lambda^{k-1}}{(k-1)!}
=a\frac{(-\ln a)^{k-1}}{(k-1)!},
\qquad k=1,2,\dots.
\]
