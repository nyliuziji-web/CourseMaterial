\heading{实验物理中的统计方法-yzw-25春-期中}

\begin{question}
已知 $\operatorname{E}[XY]=\operatorname{E}[X]\operatorname{E}[Y]$，下列一定成立的是：
...
\end{enumerate}
\begin{solution}
$\operatorname{E}[XY]=\operatorname{E}[X]\operatorname{E}[Y]$ 等价于 $\operatorname{Cov}(X,Y)=0$。因此一定成立的是
\[
...
\begin{question}
$X\sim N(0,1)$，$Y\sim N(0,1)$，令 $Z=X-Y$，求 $\operatorname{E}[Z]$、$\operatorname{E}[|Z|]$ 和 $V[|Z|]$。
\begin{solution}
$Z\sim N(0,2)$，故
\[
\operatorname{E}[Z]=0,
\qquad \operatorname{E}[|Z|]=\frac{2}{\sqrt\pi},
...
\begin{question}
证明公式 $\operatorname{E}[X]=\operatorname{E}[\operatorname{E}[X\mid Y]]$，并应用该公式计算经过两个独立的串联子电倍增管电子数（服从泊松分布，均值分别为 $\nu_1,\nu_2$）的期望和方差。具体而言，将一个电子打入一个电极中，产生的电子数 $N$ 服从均值为 $\nu_1$ 的泊松分布，这些电子全部入射第二个电极，产生的电子数 $X_i$ 服从均值为 $\nu_2$ 的泊松独立同分布，求统计量 $Y=\sum_{i=1}^{N}X_i$ 的期望和方差。（20）
\begin{solution}
全期望公式为
\[
\operatorname{E}[X]=E\bigl(\operatorname{E}[X\mid Y]\bigr).
\]
在本题中，给定 $N$ 后，
\[
Y\mid N\sim \mathrm{Poisson}(N\nu_2),
\qquad \operatorname{E}[Y\mid N]=N\nu_2,
\qquad \operatorname{Var}(Y\mid N)=N\nu_2.
\]
因此
\[
\operatorname{E}[Y]=\operatorname{E}[E(Y\mid N)]=\nu_2\operatorname{E}[N]=\nu_1\nu_2.
\]
再由全方差公式
\[
...
=\nu_1\nu_2+\nu_1\nu_2^2.
\]
\ansmath{\operatorname{Var}(Y)=\operatorname{E}[\operatorname{Var}(Y\mid N)]+\operatorname{Var}(\operatorname{E}[Y\mid N])
=\nu_2\operatorname{E}[N]+\nu_2^2\operatorname{Var}(N)
=\nu_1\nu_2+\nu_1\nu_2^2}
\end{solution}
\end{question}
