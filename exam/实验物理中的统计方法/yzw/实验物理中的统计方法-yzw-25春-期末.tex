...
由样本方差的基本性质，
\[
\operatorname{E}\left[\sum_{i=1}^n(X_i-\bar X)^2\right]=(n-1)\sigma^2.
\]
因此
\[
\operatorname{E}\left[\frac{1}{n-1}\sum_{i=1}^n(X_i-\bar X)^2\right]=\sigma^2.
\]
故命题正确。
\anstext{正确}
\end{solution}
...
\[
\operatorname{E}\left[\sum_{i=1}^N(X_i-\bar X)^2\right]=n(N-1)p(1-p).
\]
\begin{solution}
对单个观测量，
\[
...
又因为
\[
\operatorname{E}\left[\sum_{i=1}^N(X_i-\bar X)^2\right]=(N-1)\V(X_i),
\]
所以
\[
...
=1-\sum_{n=0}^4\frac{3^n\ee^{-3}}{n!}.
\]
展开为
\[
\alpha=1-\ee^{-3}\left(1+3+\frac{3^2}{2!}+\frac{3^3}{3!}+\frac{3^4}{4!}\right).
\]
数值上
\[
\alpha\approx 0.1847.
...
由泊松分布
\[
P(N=0\mid \mu_{\mathrm{up}})=\ee^{-\mu_{\mathrm{up}}},
\]
故
\[
\ee^{-\mu_{\mathrm{up}}}=0.05.
\]
...
指数分布密度为
\[
g(t;\tau)=\frac1\tau \ee^{-t/\tau},\qquad t\ge0.
\]
令
\[
U=\frac{T}{\tau}.
\]
则 $U$ 服从均值为 $1$ 的指数分布，分布函数为 $F_U(u)=1-\ee^{-u}$。中心 $90\%$ 区间两端各留 $5\%$ 尾概率，因此
\[
u_{0.05}=-\ln0.95,
\qquad
u_{0.95}=-\ln0.05.
