\heading{实验物理中的统计方法-yzw-模拟题1-期中}

\begin{question}
从 $\{1,2,3,4,5,6\}$ 中随机取一个数 $X$，再从 $\{1,2,\cdots,X\}$ 中随机取一个数 $Y$。求 $\operatorname{E}[Y]$。
\begin{solution}
由条件期望公式，
\[
\operatorname{E}[Y\mid X=x]=\frac{1+x}{2}.
\]
于是
\[
\operatorname{E}[Y]=\operatorname{E}\bigl(\operatorname{E}[Y\mid X]\bigr)=\frac{1+\operatorname{E}[X]}{2}.
\]
而
\[
\operatorname{E}[X]=\frac{1+2+3+4+5+6}{6}=\frac72.
\]
故
\ansmath{\operatorname{E}[Y]=\frac{1+\frac72}{2}=\frac94}
\end{solution}
\end{question}
...
对 $u>0$，
\[
P(U>u)=P(X>u,Y>u)=\ee^{-\lambda u}\ee^{-\lambda u}=\ee^{-2\lambda u}.
\]
故
\[
F_U(u)=1-\ee^{-2\lambda u},\qquad
f_U(u)=2\lambda \ee^{-2\lambda u},\quad u>0.
\]
再看 $I$，有
\[
...
由无记忆性或直接积分可得
\[
P(U>u,I=1)=\frac12 \ee^{-2\lambda u}=P(U>u)P(I=1).
\]
同理对 $I=0$ 也成立，所以
...
\begin{question}
有 $n$ 个编号为 $1$ 到 $n$ 的球，$n$ 个编号为 $1$ 到 $n$ 的盒子，将球随机放入盒子，每个盒子放且仅放一个球。称“球号与盒号相同”为一次配对。设配对数为 $K$，求 $\operatorname{E}[K(K-1)]$ 及 $\operatorname{Var}(K)$。
\begin{solution}
记
\[
I_i=\mathbf 1\{\pi(i)=i\},
...
则
\[
\operatorname{E}[I_i]=\frac1n,\qquad \operatorname{E}[I_iI_j]=\frac{(n-2)!}{n!}=\frac{1}{n(n-1)}\quad(i\ne j).
\]
于是
\[
\operatorname{E}[K(K-1)]=\sum_{i\ne j}\operatorname{E}[I_iI_j]
=n(n-1)\cdot \frac{1}{n(n-1)}=1.
\]
又
\[
\operatorname{E}[K]=\sum_i \operatorname{E}[I_i]=1,
\qquad
\operatorname{E}[K^2]=\operatorname{E}[K(K-1)]+\operatorname{E}[K]=2.
\]
故
\ansmath{\operatorname{Var}(K)=\operatorname{E}[K^2]-\operatorname{E}[K]^2=2-1=1}
\end{solution}
\end{question}
...
\[
f_{T_1,T_2}(u,t)=\lambda^2 \ee^{-\lambda t},\qquad 0<u<t.
\]
而
\[
f_{T_2}(t)=\lambda^2 t \ee^{-\lambda t},\qquad t>0.
\]
故
\[
...
由归一化条件，
\[
1=c\int_0^1\int_0^{1-x}(x+y)\dd{y}\dd{x}
=c\cdot \frac13,
\]
所以 $c=3$。
再算边缘密度：
\[
f_X(x)=\int_0^{1-x}3(x+y)\dd{y}
=3x(1-x)+\frac32(1-x)^2
...
后验均值为
\[
\operatorname{E}[\theta\mid D]=\frac{6}{10}=\frac35,
\]
所以下一次为正面的预测概率为
\ansmath{P(H_{\rm next}\mid D)=\frac35}
再由
\[
P(HH\mid D)=\operatorname{E}[\theta^2\mid D]
...
因此
\[
\operatorname{E}[Y\mid N]=N\nu_2,\qquad \operatorname{Var}(Y\mid N)=N\nu_2.
\]
由全期望公式，
\[
\operatorname{E}[Y]=\operatorname{E}(\operatorname{E}[Y\mid N])=\nu_2\operatorname{E}[N]=\nu_1\nu_2.
\]
由全方差公式，
\[
\operatorname{Var}(Y)=\operatorname{E}(\operatorname{Var}(Y\mid N))+\operatorname{Var}(\operatorname{E}[Y\mid N])
=\nu_2\operatorname{E}[N]+\nu_2^2\operatorname{Var}(N).
\]
由于 $N\sim \mathrm{Poisson}(\nu_1)$，故 $\operatorname{E}[N]=\operatorname{Var}(N)=\nu_1$，从而
\ansmath{\operatorname{E}[Y]=\nu_1\nu_2,\qquad \operatorname{Var}(Y)=\nu_1\nu_2+\nu_1\nu_2^2}
\end{solution}
\end{question}
...
期望为
\[
\operatorname{E}[R]=\int_a^b r\frac{2r}{b^2-a^2}\dd{r}
=\frac{2}{b^2-a^2}\cdot \frac{b^3-a^3}{3}.
\]
即
\ansmath{\operatorname{E}[R]=\frac{2(b^3-a^3)}{3(b^2-a^2)}}
\end{solution}
\end{question}
...
=1-\sum_{k=0}^{6}\ee^{-2.5}\frac{2.5^k}{k!}.
\]
数值上
