\heading{实验物理中的统计方法-yzw-模拟题1-期末}
...
Z=\frac{X}{Y},
\]
其中 $X,Y$ 的标准差分别为 $\sigma_X,\sigma_Y$，协方差为 $c=\operatorname{Cov}(X,Y)$。写出小误差近似下 $Z$ 的不确定度公式。
\begin{solution}
由误差传播公式，
\[
\sigma_Z^2\approx
...
\]
故
\ansmath{\sigma_Z^2\approx \frac{\sigma_X^2}{Y^2}+\frac{X^2\sigma_Y^2}{Y^4}-\frac{2Xc}{Y^3}}
\end{solution}
\end{question}
...
对卡方分布，
\[
\operatorname{E}[X]=2n,\qquad \operatorname{Var}(X)=4n.
\]
令
\[
g(x)=\sqrt{x},
...
由 delta 方法，
\[
\sqrt{X}\approx N\!\left(g(2n),\ [g'(2n)]^2\operatorname{Var}(X)\right).
\]
因此
\[
[g'(2n)]^2\operatorname{Var}(X)
=\frac{1}{4\cdot 2n}\cdot 4n=\frac12.
\]
故
\ansmath{\sqrt{X}\approx N\!\left(\sqrt{2n},\ \frac12\right)}
\end{solution}
\end{question}
...
先算总体均值：
\[
\operatorname{E}[X]=\int_0^1 x(\theta+1)x^\theta\dd{x}
=\frac{\theta+1}{\theta+2}.
\]
令 $\bar X=\operatorname{E}[X]$，解得矩估计
\[
...
Fisher 信息为
\[
I_1(\theta)=-\operatorname{E}\!\left[\frac{\partial^2 \ell_1}{\partial \theta^2}\right]
=\frac{1}{(\theta+1)^2}.
\]
所以
\[
...
\]
于是 MLE 的渐近方差为
\ansmath{\operatorname{Var}(\hat\theta_{\rm MLE})\approx \frac{(\theta+1)^2}{n}}
\end{solution}
\end{question}
...
似然函数为
\[
L(\lambda)=\prod_{i=1}^n \ee^{-\lambda}\frac{\lambda^{x_i}}{x_i!},
\qquad
...
又有
\[
\operatorname{E}[\bar X]=\lambda,\qquad \operatorname{Var}(\bar X)=\frac{\lambda}{n}.
\]
单个样本的 Fisher 信息为
\[
...
\qquad
\operatorname{Bias}(Y)=-\frac{\theta}{n+1}.
\]
因此
\[
\operatorname{MSE}(Y)=\operatorname{Var}(Y)+\operatorname{Bias}(Y)^2
=\frac{2\theta^2}{(n+1)(n+2)}.
\]
而
\[
\operatorname{MSE}(\tilde\theta)=\operatorname{Var}(\tilde\theta)
=\left(\frac{n+1}{n}\right)^2\operatorname{Var}(Y)
=\frac{\theta^2}{n(n+2)}.
\]
比较得
\[
...
\[
\operatorname{Var}(\hat a)=\frac{1}{\sum_{i=1}^n \dfrac{x_i^2}{\sigma_i^2}}.
\]
代回可得
\[
\chi^2_{\min}
