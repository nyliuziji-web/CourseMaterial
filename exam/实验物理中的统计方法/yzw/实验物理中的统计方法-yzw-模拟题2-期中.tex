\heading{实验物理中的统计方法-yzw-模拟题2-期中}

\begin{question}
$X\sim N(0,1)$，$Y\sim N(0,1)$，并且
\[
\operatorname{Var}(2X-Y)=1.
\]
求 $(X,Y)$ 的协方差矩阵。
\begin{solution}
由
\[
\operatorname{Var}(2X-Y)=4\operatorname{Var}(X)+\operatorname{Var}(Y)-4\operatorname{Cov}(X,Y),
\]
代入 $\operatorname{Var}(X)=\operatorname{Var}(Y)=1$ 得
\[
1=4+1-4\operatorname{Cov}(X,Y).
\]
故
\[
...
\begin{question}
一个显示屏每次等概率显示 $1$ 到 $N$ 中的一个整数。某人连续看了 $n$ 次，记录下看到的最大值 $K$。求 $K$ 的概率分布，并写出 $\operatorname{E}[K]$ 的求法。
\begin{solution}
先有
\[
...
期望可由尾和公式写成
\[
\operatorname{E}[K]=\sum_{k=1}^{N}P(K\ge k)
=\sum_{k=1}^{N}\left[1-\left(\frac{k-1}{N}\right)^n\right].
\]
也可写作
\ansmath{\operatorname{E}[K]=N-\frac{1}{N^n}\sum_{j=0}^{N-1}j^n}
\end{solution}
\end{question}
...
\]
求 $Y$ 的概率密度函数，并说明 $\operatorname{E}[Y]$ 是否存在。
\begin{solution}
令
\[
...
对边界均匀分布，直接计算可得
\[
\operatorname{E}[X]=\frac a2,\qquad \operatorname{E}[Y]=\frac b2,
\]
\[
\operatorname{E}[X^2]=\frac{a^2(2a+3b)}{6(a+b)},\qquad
\operatorname{E}[Y^2]=\frac{b^2(3a+b)}{6(a+b)}.
\]
代回化简得
\[
\operatorname{E}[D^2]=\frac{(a+b)^2}{6}.
\]
因此
\ansmath{\operatorname{E}[D^2]=\frac{(a+b)^2}{6}}
\end{solution}
\end{question}
...
由归一化条件，
\[
1=c\int_0^1\int_0^x x\dd{y}\dd{x}
=c\int_0^1 x^2\,dx
=\frac{c}{3}.
\]
故
...
再算
\[
P\!\left(Y<\frac X2\right)=\int_0^1\int_0^{x/2}3x\dd{y}\dd{x}
...
进一步有
\[
\operatorname{E}[Y]=\int_0^\theta y\frac{n y^{n-1}}{\theta^n}\dd{y}
=\frac{n}{n+1}\theta,
\]
\[
\operatorname{E}[Y^2]=\int_0^\theta y^2\frac{n y^{n-1}}{\theta^n}\dd{y}
=\frac{n}{n+2}\theta^2.
\]
所以
\[
