\heading{实验物理中的统计方法-yzw-模拟题2-期末}
...
单个样本的 Fisher 信息为
\[
I_1(\lambda)=\operatorname{E}\left[\left(\frac{\partial \ell_1}{\partial\lambda}\right)^2\right]=\frac{1}{\lambda^2},
\]
又因为
\[
...
后验均值为
\[
\operatorname{E}[\varepsilon\mid D]=\frac{10}{14}=\frac57.
\]
...
似然函数为
\[
L(s)\propto \ee^{-(s+b)}\frac{(s+b)^0}{0!}=\ee^{-(s+b)}.
\]
乘以平坦先验后，
\[
p(s\mid n=0)\propto \ee^{-s},\qquad s\ge 0.
\]
归一化后得
\[
p(s\mid n=0)=\ee^{-s},\qquad s\ge 0,
\]
即
\ansmath{s\mid n=0\sim \mathrm{Exp}(1)}
求 $90\%$ 上限 $s_{90}$：
\[
P(s\le s_{90}\mid n=0)=1-\ee^{-s_{90}}=0.9.
\]
故
\[
\ee^{-s_{90}}=0.1,\qquad s_{90}=-\ln 0.1=\ln 10.
\]
因此
