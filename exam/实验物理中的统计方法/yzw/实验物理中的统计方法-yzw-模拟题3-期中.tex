...
又由条件期望公式，
\[
\operatorname{E}[Y\mid X=x]=\frac{x+1}{2},
\qquad
\operatorname{E}[Y]=\operatorname{E}\bigl(\operatorname{E}[Y\mid X]\bigr)=\frac{\operatorname{E}[X]+1}{2}.
\]
而
\[
...
\]
故
\ansmath{\operatorname{E}[Y]=\frac{\frac{n+1}{2}+1}{2}=\frac{n+3}{4}}
\end{solution}
\end{question}
...
\item 求 $\operatorname{E}[Y\mid X=x]$。
\end{enumerate}
\begin{solution}
由归一化条件，
\[
1=c\int_0^1\int_0^x (x+2y)\dd{y}\dd{x}
=c\int_0^1 2x^2\,dx
=\frac{2c}{3}.
\]
故
...
边缘密度为
\[
f_X(x)=\int_0^x \frac32(x+2y)\dd{y}
...
再算概率：
\[
P\!\left(Y>\frac X2\right)=\int_0^1\int_{x/2}^{x}\frac32(x+2y)\dd{y}\dd{x}.
\]
计算得
\[
\int_{x/2}^{x}(x+2y)\dd{y}=\frac{5x^2}{4},
\]
故
\[
P\!\left(Y>\frac X2\right)=\int_0^1 \frac32\cdot \frac{5x^2}{4}\dd{x}=\frac58.
\]
条件密度为
\[
...
于是
\[
\operatorname{E}[Y\mid X=x]=\int_0^x y\,\frac{x+2y}{2x^2}\dd{y}=\frac{7x}{12}.
\]
\end{solution}
\end{question}
...
后验均值为
\[
\operatorname{E}[\varepsilon\mid D]=\frac{10}{15}=\frac23.
\]
后面两次都成功的后验预测概率为
\[
\operatorname{E}[\varepsilon^2\mid D]
...
即
\[
p=1-\ee^{-3}\sum_{k=0}^{6}\frac{3^k}{k!}.
\]
数值上
\[
p\approx 0.0335.
