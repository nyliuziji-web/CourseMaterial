\heading{实验物理中的统计方法-yzw-模拟题3-期末}
...
W=\ln\frac{X}{Y},
\]
其中 $X,Y$ 的标准差分别为 $\sigma_X,\sigma_Y$，协方差为 $c=\cov(X,Y)$。写出小误差近似下 $W$ 的不确定度公式。
\begin{solution}
由误差传播公式，
\[
\sigma_W^2\approx
...
先求总体均值：
\[
\operatorname{E}[X]=\int_0^{\theta} x\cdot \frac{2x}{\theta^2}\dd{x}
=\frac{2\theta}{3}.
\]
故矩估计量满足
\[
...
于是
\[
\operatorname{E}[X_{(n)}]=\frac{2n}{2n+1}\theta.
\]
因此无偏估计量可取
...
似然函数为
\[
L(s)\propto \ee^{-(s+b)}\frac{(s+b)^2}{2!}.
\]
乘以平坦先验后，去掉与 $s$ 无关的常数，得
\[
p(s\mid n=2)\propto \ee^{-s}(s+0.5)^2,
\qquad s\ge 0.
\]
后验众数由
\[
...
后验均值为
\[
\operatorname{E}[s\mid n=2]=\frac{\int_0^{\infty} s \ee^{-s}(s+0.5)^2\dd{s}}{\int_0^{\infty} \ee^{-s}(s+0.5)^2\dd{s}}.
\]
直接积分得
\[
\int_0^{\infty} \ee^{-s}(s+0.5)^2\dd{s}=\frac{13}{4},
\qquad
\int_0^{\infty} s \ee^{-s}(s+0.5)^2\dd{s}=\frac{33}{4}.
\]
因此
\[
\operatorname{E}[s\mid n=2]=\frac{33/4}{13/4}=\frac{33}{13}\approx 2.538.
\]
\end{solution}
\end{question}
