\heading{实验物理中的统计方法-yzw-模拟题4-期中}
...
又
\[
\operatorname{E}[S]=4\pi \operatorname{E}[R^2]
=\frac{4\pi}{b-a}\int_a^b r^2\,dr
=\frac{4\pi}{3}(a^2+ab+b^2).
\]
因此
\ansmath{\operatorname{E}[S]=\frac{4\pi}{3}(a^2+ab+b^2)}
\end{solution}
\end{question}
...
\[
P(T_1\leq t\mid N(T)=4)=1-\left(1-\frac{t}{T}\right)^4,
\qquad 0<t<T.
\]
对 $t$ 求导得密度
\[
...
\item 求边缘密度 $f_Y(y)$；
\item 求 $P(X+Y<1)$；
\item 求 $\operatorname{E}[X\mid Y=y]$。
\end{enumerate}
\begin{solution}
归一化条件给出
\[
1=c\int_0^1\int_0^y y\dd{x}\dd{y}=c\int_0^1 y^2\dd{y}=\frac{c}{3}.
\]
故
\ansmath{c=3}
边缘密度为
\[
f_Y(y)=\int_0^y 3y\dd{x}=3y^2,
\qquad 0<y<1.
\]
再求概率。由条件 $0<x<y<1$ 与 $x+y<1$，需分两段积分：
\[
P(X+Y<1)=\int_0^{1/2}\int_0^y 3y\dd{x}\dd{y}+\int_{1/2}^{1}\int_0^{1-y}3y\dd{x}\dd{y}.
\]
计算得
\[
P(X+Y<1)=\frac38.
...
\ansmath{\operatorname{E}[X\mid Y=y]=\frac{y}{2}}
\end{solution}
\end{question}
...
\]
数值上（分子 $0.97\times10^{-4}=9.7\times10^{-5}$，分母 $\approx9.8\times10^{-5}$）
\[
P(e\mid 3)\approx 0.9898.
\]
同理
\[
