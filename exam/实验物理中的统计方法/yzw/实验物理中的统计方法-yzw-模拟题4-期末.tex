\heading{实验物理中的统计方法-yzw-模拟题4-期末}
...
\]
代入得到（残差依次为 $-0.08,\ 0.19,\ -0.14,\ 0.03$，平方和 $0.063$）
\[
\chi^2_{\min}=\frac{0.063}{0.04}=1.575.
\]
\end{solution}
\end{question}
...
似然函数为
\[
L(s)\propto \ee^{-(s+b)}(s+b).
\]
代入 $b=1$ 并乘以平坦先验后，得
\[
p(s\mid n=1)\propto \ee^{-s}(s+1),
\qquad s\ge 0.
\]
归一化常数为
\[
\int_0^{\infty} \ee^{-s}(s+1)\dd{s}=2.
\]
故后验密度为
\ansmath{p(s\mid n=1)=\frac12 \ee^{-s}(s+1),
\qquad s\ge 0}
设 $s_{90}$ 满足
\[
\int_0^{s_{90}}\frac12 \ee^{-s}(s+1)\dd{s}=0.9.
\]
即
\[
1-\frac12 \ee^{-s_{90}}(s_{90}+2)=0.9.
\]
因此需解
\[
\ee^{-s_{90}}(s_{90}+2)=0.2.
\]
数值上
\[
s_{90}\approx 3.27.
