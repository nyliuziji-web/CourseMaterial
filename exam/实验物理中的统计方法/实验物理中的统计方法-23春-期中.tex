...
=\frac{77}{300}.
\]
\ansmath{P(Y=2)=\sum_{x=2}^{5}P(X=x)P(Y=2\mid X=x)
=\frac15\sum_{x=2}^{5}\frac1x
=\frac{77}{300}}
\end{solution}
\end{question}
...
\]
其余处为 $0$。
\ansmath{f_S(s)=f_R\!\left(\sqrt{\frac{s}{\pi}}\right)\left|\frac{d}{ds}\sqrt{\frac{s}{\pi}}\right|
 =\frac{1}{2(b-a)\sqrt{\pi s}},
\quad \pi a^2<s<\pi b^2}
\end{solution}
\end{question}
...
\]
\ansmath{\operatorname{Var}(X)=E[X^2]-E[X]^2=\frac16}
\end{solution}
\end{question}
...
\ansmath{\sigma_{\hat\varepsilon}=\sqrt{\frac{\hat\varepsilon(1-\hat\varepsilon)}{N}}
=\sqrt{\frac{0.9\times 0.1}{2000}}\approx 6.7\times 10^{-3}}
\end{solution}
\end{question}
...
\]
\end{enumerate}
\ansmath{\theta\mid H\sim \mathrm{Beta}(2,1),\qquad f(\theta\mid H)=2\theta,\ 0<\theta<1,\quad  \theta\mid H,T\sim \mathrm{Beta}(2,2),\qquad f(\theta\mid H,T)=6\theta(1-\theta),\ 0<\theta<1}
\end{solution}
\end{question}
