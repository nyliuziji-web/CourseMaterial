...
\]
故选 \textbf{B}。
\ansmath{\operatorname{Var}(X+Y)=\operatorname{Var}(X)+\operatorname{Var}(Y)}
\end{solution}
\end{question}
...
\]
\ansmath{P(M\le k)=\left(\frac{k}{N}\right)^n,
\qquad
P(M=k)=\left(\frac{k}{N}\right)^n-\left(\frac{k-1}{N}\right)^n}
\end{solution}
\end{question}
...
\]
\ansmath{f_S(s)=\frac{1}{(b-a)\sqrt{\pi s}},
\qquad \frac{\pi a^2}{4}<s<\frac{\pi b^2}{4}}
\end{solution}
\end{question}
...
\]
\ansmath{F_{\text{sys}}(t)=1-[1-F(t)]^2}
\end{solution}
\end{question}
...
=\frac{2l}{\pi a}.
\]
\ansmath{P=\frac{2}{a\pi}\int_0^{\pi/2} l\sin\theta\,d\theta
=\frac{2l}{\pi a}}
\end{solution}
\end{question}
