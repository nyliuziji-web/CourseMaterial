...
\noteinfo{内容精修版；已补足必要独立性、连续性和参数范围条件。}

\begin{question}
设 $X,Y$ 的方差均存在。已知 $E[XY]=E[X]E[Y]$，下列一定成立的是：
...
\end{choiceoptions}
\begin{solution}
$E[XY]=E[X]E[Y]$ 等价于 $\cov(X,Y)=0$。因此一定成立的是
\[
\V(X+Y)=\V(X)+\V(Y).
\]
故选 \textbf{B}。
...
\begin{question}
从 $\{1,\cdots,N\}$ 中独立随机生成 $n$ 个数（可重复），记最大值为 $M$。求 $P(M=k)$，其中 $k=1,2,\ldots,N$。
\begin{solution}
\[
...
\begin{question}
若圆的直径 $d$ 在区间 $[a,b]$ 上服从均匀分布，其中 $0<a<b$，求面积 $S=\dfrac{\pi d^2}{4}$ 的概率密度函数。
\begin{solution}
由 $d=2\sqrt{S/\pi}$，
\[
...
\begin{question}
$X\sim N(0,1)$，$Y\sim N(0,1)$，且 $X,Y$ 相互独立。令 $Z=X-Y$，求 $E[Z]$、$E[|Z|]$ 和 $V[|Z|]$。
\begin{solution}
$Z\sim N(0,2)$，故
\[
E[Z]=0,
\qquad E[|Z|]=\frac{2}{\sqrt\pi},
\qquad \V(|Z|)=2-\frac{4}{\pi}.
\]
\end{solution}
\end{question}

\begin{question}
将 $n\ge2$ 个编号为 $1\sim n$ 的礼物随机放入编号为 $1\sim n$ 的 $n$ 个盒子中，求配对数（即礼物编号等于盒子编号的个数）$k$ 的方差。
\begin{solution}
配对数即随机排列的不动点个数，故
\ansmath{\V(k)=1}
\end{solution}
...
\begin{question}[10 分]
蒲丰投针问题：设针长为 $l$，平行线间距为 $a$，且 $l<a$，求针与线相交的概率。
\begin{solution}
...
\begin{question}[20 分]
证明公式 $E[X]=E[E[X\mid Y]]$，并应用该公式计算经过两个独立的串联次级电子倍增管电子数（服从泊松分布，均值分别为 $\nu_1,\nu_2$）的期望和方差。具体而言，将一个电子打入一个电极中，产生的电子数 $N$ 服从均值为 $\nu_1$ 的泊松分布，这些电子全部入射第二个电极，产生的电子数 $X_i$ 服从均值为 $\nu_2$ 的泊松独立同分布，求统计量 $Y=\sum_{i=1}^{N}X_i$ 的期望和方差。
\begin{solution}
全期望公式为
\[
...
=\nu_1\nu_2+\nu_1\nu_2^2.
\]
\ansmath{\V(Y)=E[\V(Y\mid N)]+\V(E[Y\mid N])
=\nu_2E[N]+\nu_2^2\V(N)
=\nu_1\nu_2+\nu_1\nu_2^2}
\end{solution}
\end{question}
...
\begin{question}[10 分]
设 $X\sim U(0,1)$，定义 $Y=\tan\bigl(\pi(X-\frac12)\bigr)$，求 $Y$ 的分布。
\begin{solution}
...
\begin{question}[10 分]
设 $X_i\sim U(0,\theta)$，$i=1,\cdots,n$ 独立同分布，令 $Y=\max(X_i)$，求 $Y$ 的概率密度函数、期望和方差。
\begin{solution}
\[
...
\]
\begin{enumerate}
\item 若只有一层探测器发出信号，粒子为光子的概率是多少？
\item 若两层探测器都发出信号，粒子为电子的概率是多少？
\end{enumerate}
\begin{solution}
