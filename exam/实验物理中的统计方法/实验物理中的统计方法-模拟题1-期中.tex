...
\]
故
\ansmath{E[Y]=\frac{1+\frac72}{2}=\frac94}
\end{solution}
\end{question}
...
\begin{question}
有 $n$ 个编号为 $1$ 到 $n$ 的球，$n$ 个编号为 $1$ 到 $n$ 的盒子，将球随机放入盒子，每个盒子放且仅放一个球。称“球号与盒号相同”为一次配对。设配对数为 $K$，求 $E[K(K-1)]$ 及 $\operatorname{Var}(K)$。
\begin{solution}
记
\[
I_i=\mathbf 1\{\pi(i)=i\},
...
\]
故
\ansmath{\operatorname{Var}(K)=E[K^2]-E[K]^2=2-1=1}
\end{solution}
\end{question}
...
\]
故
\ansmath{f_X(x)=\frac32(1-x^2),\qquad 0<x<1}
