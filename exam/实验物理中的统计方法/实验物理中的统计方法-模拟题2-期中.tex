...
\]
即
\ansmath{P(K=k)=\frac{k^n-(k-1)^n}{N^n},\qquad k=1,2,\cdots,N}
期望可由尾和公式写成
\[
...
\]
故
\ansmath{f_Y(y)=\frac{1}{\pi(1+y^2)},\qquad y\in \mathbb R}
这就是标准 Cauchy 分布。由于
\[
...
\]
故
\ansmath{P=\frac{2t}{T}-\left(\frac{t}{T}\right)^2}
\end{solution}
\end{question}
...
\]
因此
\ansmath{P\!\left(Y<\frac X2\right)=\frac12}
\end{solution}
\end{question}
...
\]
故
\ansmath{\hat\pi\approx 3.1417,\qquad \sigma_\pi\approx 0.0467}
\end{solution}
\end{question}
...
\]
因此
\ansmath{X\mid (X+Y=n)\sim \mathrm{Bin}\!\left(n,\frac{\nu_1}{\nu_1+\nu_2}\right)}
\end{solution}
\end{question}
...
\]
因此
\ansmath{f_Y(y)=\frac{n y^{n-1}}{\theta^n},\qquad 0<y<\theta}
进一步有
\[
...
\]
即
\ansmath{E[Y]=\frac{n}{n+1}\theta,\qquad \operatorname{Var}(Y)=\frac{n\theta^2}{(n+1)^2(n+2)}}
\end{solution}
\end{question}
