...
\]
因此
\ansmath{E[Y]=\frac{\frac{n+1}{2}+1}{2}=\frac{n+3}{4}}
\end{solution}
\end{question}
...
\]
因此
\ansmath{P\!\left(Y>\frac X2\right)=\frac58}
条件密度为
\[
...
\]
故
\ansmath{E[Y\mid X=x]=\frac{7x}{12}}
\end{solution}
\end{question}
...
\]
所以
\ansmath{f_{T_2\mid T_3=t}(u)=\frac{2u}{t^2},
\qquad 0<u<t}
\end{solution}
\end{question}
...
\]
所以
\ansmath{\hat\varepsilon\approx 0.917}
按二项分布近似，
\[
...
\]
故
\ansmath{\hat\varepsilon_{\rm sys}\approx 0.993}
\end{solution}
\end{question}
...
\]
即
\ansmath{\varepsilon\mid D\sim \mathrm{Beta}(10,5)}
后验均值为
\[
E[\varepsilon\mid D]=\frac{10}{15}=\frac23.
\]
故
\ansmath{E[\varepsilon\mid D]=\frac23}
后面两次都成功的后验预测概率为
\[
...
\]
因此
\ansmath{P(\text{后两次都成功}\mid D)=\frac{11}{24}}
\end{solution}
\end{question}
...
\]
故
\ansmath{p=P(N\ge 7\mid \lambda=3)=1-e^{-3}\sum_{k=0}^{6}\frac{3^k}{k!}\approx 0.0335}
\end{solution}
\end{question}
