...
R=\frac{X}{X+Y},
\]
其中 $X,Y$ 的标准差分别为 $\sigma_X,\sigma_Y$，协方差为 $c=\operatorname{Cov}(X,Y)$。写出小误差近似下 $R$ 的不确定度公式。
\begin{solution}
由误差传播公式，
\[
\sigma_R^2\approx
...
\]
故
\ansmath{\sigma_R^2\approx \frac{Y^2\sigma_X^2+X^2\sigma_Y^2-2XYc}{(X+Y)^4}}
\end{solution}
\end{question}
...
\]
因此
\ansmath{I_n(p)=\frac{n}{p(1-p)}}
由极大似然估计的渐近正态性，
\[
\operatorname{Var}(\hat p)\approx \frac{p(1-p)}{n}.
\]
所以
\ansmath{\operatorname{Var}(\hat p)\approx \frac{p(1-p)}{n}}
当 $n=100,k=37$ 时，
\[
\hat p=0.37,
\qquad
...
\]
因此
\ansmath{p\in (0.275,\ 0.465)\qquad (\text{approx. }95\%\ \text{CI})}
\end{solution}
\end{question}
...
\]
因此
\ansmath{\sigma^2\in (1.204,\ 6.918)\qquad (95\%\ \text{CI})}
\end{solution}
\end{question}
...
\]
故
\ansmath{\text{power}\approx 0.834}
\end{solution}
\end{question}
