...
\item 分离变量写成 $\psi=\Phi \ee^{-\ii Et/\hbar}$。
...
\ansmath{\|X_0\|^2=\int_{-l}^{l}1\dd{x}=2l,
\qquad
...
\[
  u(x,t)=\frac{f_e(x+at)+f_e(x-at)}{2}+\frac{1}{2a}\int_{x-at}^{x+at}g_e(\xi)\dd{\xi}
 +\int_0^t (t-\tau)A\cos\omega\tau\dd{\tau}.
\]
化简得
\[
  u(x,t)=x^2+a^2t^2+\frac{1}{2a}\int_{x-at}^{x+at}|\xi|\dd{\xi}+\frac{A}{\omega^2}(1-\cos\omega t).
\]
其中
\[
\int_{x-at}^{x+at}|\xi|\dd{\xi}
...
故解可写成
\[
 u(x,t)=\frac{q}{K}(l-x)+\sum_{n=0}^{\infty}c_n \ee^{-\kappa\mu_n^2 t}\cos(\mu_n x),
\]
其中系数由初值
\[
 u(x,0)=x-\cos x
...
决定，即
\[
  c_n=\frac{2}{l}\int_0^l\Bigl[x-\cos x-\frac{q}{K}(l-x)\Bigr]\cos(\mu_n x)\dd{x}.
\]
这就是温度分布的标准展开式。

\ansmath{c_n=\frac{2}{l}\int_0^l\Bigl[x-\cos x-\frac{q}{K}(l-x)\Bigr]\cos(\mu_n x)\dd{x}}
