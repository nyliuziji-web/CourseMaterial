...
\left.\dfrac{\partial u}{\partial x}\right|_{x=0}=0,\quad \left.\dfrac{\partial u}{\partial x}\right|_{x=l}=\ee^{-\alpha^{2} t}, & t>0,\\
...
u(x,t)=v(x,t)+\ee^{-\alpha^2 t}\phi(x),
\]
其中 $\phi$ 取为满足
\[
...
\]
于是 $\ee^{-\alpha^2 t}\phi(x)$ 本身满足热方程，并且正好给出非齐次边界条件，因此 $v$ 满足齐次 Neumann 问题
\[
\begin{cases}
...
故可展开为余弦级数
\[
v(x,t)=c_0+\sum_{n=1}^{\infty}c_n \ee^{-\kappa n^2\pi^2 t/l^2}\cos\frac{n\pi x}{l}.
\]
其中
\[
c_0=\frac1l\int_0^l\bigl(A-\phi(x)\bigr)\dd{x}
...
\begin{aligned}
c_n
&=\frac{2}{l}\int_0^l\bigl(A-\phi(x)\bigr)\cos\frac{n\pi x}{l}\dd{x}
=-\frac{2}{l}\int_0^l\phi(x)\cos\frac{n\pi x}{l}\dd{x}\\
&=-\frac{2}{l}\cdot\frac{1}{\beta\sinh(\beta l)}
\int_0^l \cosh(\beta x)\cos\frac{n\pi x}{l}\dd{x}
=\frac{2\kappa l(-1)^{n+1}}{\alpha^2 l^2+\kappa n^2\pi^2}.
\end{aligned}
\]
因此
\ansmath{u(x,t)=\frac{\ee^{-\alpha^2 t}\cosh\!\left(\frac{\alpha x}{\sqrt{\kappa}}\right)}{\left(\frac{\alpha}{\sqrt{\kappa}}\right)\sinh\!\left(\frac{\alpha l}{\sqrt{\kappa}}\right)}
+ A-\frac{\kappa}{\alpha^2 l}
+\sum_{n=1}^{\infty}
\frac{2\kappa l(-1)^{n+1}}{\alpha^2 l^2+\kappa n^2\pi^2}
 \ee^{-\kappa n^2\pi^2 t/l^2}\cos\frac{n\pi x}{l}.}
...
由正交性，
\[
A_0=\frac12\int_{-1}^1 f(\mu)\dd{\mu}
=\ln a+\frac12\int_{-1}^1\ln(1+\mu)\dd{\mu}
=\ln(2a)-1.
\]
对 $n\ge 1$，常数项不贡献系数，且
\[
\int_{-1}^1\ln(1+\mu)P_n(\mu)\dd{\mu}
=\frac{2(-1)^{n+1}}{n(n+1)},
\]
故
\[
A_n a^n
=\frac{2n+1}{2}\int_{-1}^1\ln(1+\mu)P_n(\mu)\dd{\mu}
