...
这是卷积。记
\[
F(x)=\int_0^x \cos(x-t)J_0(t)\dd{t}.
\]
作 Laplace 变换：
\[
...
\]
又由 Laplace 变换导数性质 $\mathcal L\{xf(x)\}=-\frac{d}{ds}\mathcal L\{f(x)\}$，
\[
\mathcal L\{xJ_0(x)\}=-\frac{d}{ds}\frac{1}{\sqrt{s^2+1}}=\frac{s}{(s^2+1)^{3/2}}.
\]
所以
\ansmath{\int_0^x \cos(x-t)J_0(t)\dd{t}=xJ_0(x).}
...
\[
 v(r,\theta,t)=
 \sum_{n\ge1}A_n \ee^{-\kappa j_{1,n}^2 t/a^2}J_1\!\Bigl(\frac{j_{1,n}r}{a}\Bigr)\sin\theta
 +
 \sum_{n\ge1}B_n \ee^{-\kappa j_{2,n}^2 t/a^2}J_2\!\Bigl(\frac{j_{2,n}r}{a}\Bigr)\cos2\theta,
\]
...
= \bigl[\bar y(l)z'(l)-\bar y'(l)z(l)\bigr]
- \bigl[\bar y(0)z'(0)-\bar y'(0)z(0)\bigr].
\]
代入边界条件 $y(l)=\alpha y(0)$, $y'(l)=\beta y'(0)$（取复共轭得 $\bar y(l)=\bar\alpha\bar y(0)$, $\bar y'(l)=\bar\beta\bar y'(0)$；$z$ 满足同类条件）：
\begin{align*}
&= \bigl[\bar\alpha\bar y(0)\cdot\beta z'(0)-\bar\beta\bar y'(0)\cdot\alpha z(0)\bigr]
- \bigl[\bar y(0)z'(0)-\bar y'(0)z(0)\bigr] \\
&= (\bar\alpha\beta-1)\bigl[\bar y(0)z'(0)-\bar y'(0)z(0)\bigr].
\end{align*}
要使得对所有满足边界条件的 $y,z$ 均有 $[\bar y z'-\bar y' z]_{0}^{l}=0$，需且只需 $\bar\alpha\beta = 1$（等价地 $\alpha\bar\beta = 1$）。
此时该 Sturm--Liouville 本征值问题是自伴的，本征函数构成完备正交系。

\ansmath{\bar\alpha\beta = 1}
...
\[
 u(\mathbf r,t)=
 \iiint G(\mathbf r,t;\mathbf r',0)h(\mathbf r')\,\dd{^3\mathbf r'}
 +\frac{\partial}{\partial t}\iiint G(\mathbf r,t;\mathbf r',0)g(\mathbf r')\,\dd{^3\mathbf r'}
 +\iiint\!\int_0^t G(\mathbf r,t;\mathbf r',\tau)f(\mathbf r',\tau)\dd{\tau}\,\dd{^3\mathbf r'}.
\]
