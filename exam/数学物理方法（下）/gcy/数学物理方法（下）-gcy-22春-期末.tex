...
=\ii\bigl[-g_1^*(x)f_1(x)+g_2^*(x)f_2(x)\bigr]_{x=a}^{x=b}.
\]
...
u \text{ 在无穷远只有会聚波}, & \text{取时间因子为 } \ee^{-i\omega t},
\end{cases}
\]
...
设自由空间中满足辐射条件的基本解为
\[
G_1(\mathbf r;\mathbf r')=-\frac{\ee^{-\ii k|\mathbf r-\mathbf r'|}}{4\pi|\mathbf r-\mathbf r'|}.
\]
...
\frac{\ee^{-\ii k\sqrt{(x-x')^2+(y-y')^2+(z+z')^2}}}{4\pi\sqrt{(x-x')^2+(y-y')^2+(z+z')^2}}
-
\frac{\ee^{-\ii k\sqrt{(x-x')^2+(y-y')^2+(z-z')^2}}}{4\pi\sqrt{(x-x')^2+(y-y')^2+(z-z')^2}}}
