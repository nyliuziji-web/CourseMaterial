...
=-\int_0^{\pi}u'(x)\overline{v(x)}\dd{x}
=\int_0^{\pi}u(x)\overline{v'(x)}\dd{x}-u(\pi)\overline{v(\pi)}+u(0)\overline{v(0)}.
\]
...
令 $\gamma=\ee^{\ii\theta}$（$\theta\in\mathbb R$），求 $\hat L^\dagger$ 的本征值：
\[
  v'=\lambda v
 \Longrightarrow v(x)=C\ee^{\lambda x}.
\]
代入边界条件 $v(0)=\overline\gamma v(\pi)=\ee^{-\ii\theta}v(\pi)$ 得
\[
 1=\ee^{-\ii\theta}\ee^{\lambda\pi}
 \Longrightarrow
  \ee^{\lambda\pi}=\ee^{\ii\theta}.
\]
故
\ansmath{\lambda_n=\frac{\ii(\theta+2\pi n)}{\pi},\qquad n\in\mathbb Z,}
对应本征函数可取
\ansmath{v_n(x)=\ee^{\lambda_n x}.}
...
\[
\begin{cases}
\dfrac{\partial u}{\partial t}-\kappa\dfrac{\partial^2u}{\partial x^2}=\ee^{-\alpha t},
&-\infty<x<+\infty,\ t>0,\\
u|_{t=0}=\delta(x).
\end{cases}
\]
...
\[
 u_2'(t)=\ee^{-\alpha t},\qquad u_2(0)=0.
\]
故
\[
 u_2(t)=\frac{1-\ee^{-\alpha t}}{\alpha}.
\]
所以
\ansmath{u(x,t)=\frac{1}{\sqrt{4\pi\kappa t}}\exp\!\left(-\frac{x^2}{4\kappa t}\right)
 +\frac{1-\ee^{-\alpha t}}{\alpha}.}
...
\[
N_{n,m}=
\int_0^{\pi}\!\sin^2(n\varphi)\dd{\varphi}
\int_0^R \rho J_n^2\!\Bigl(\frac{j_{n,m}}{R}\rho\Bigr)\dd{\rho}
