...
对 $[x,\,x+\dd x]$ 微段作能量平衡：热传导净流入 $\kappa\,u_{xx}\,S\,\dd x$，
内部发热 $\alpha\,u(\beta-u)\,S\,\dd x$，表面散热 $-q(x,t)\,l\,\dd x$，
内能增加率 $\rho c\,u_t\,S\,\dd x$。由能量守恒消去 $S\,\dd x$，得
\[
...
\[
u_{tt}+2\beta u_t = c^2 u_{xx},\qquad 0<x<l,\;t>0,
\]
\[
u(0,t)=0,\quad u(l,t)=h(t),\qquad
u(x,0)=0,\quad u_t(x,0)=0.
\]

令 $u=v+\dfrac{x}{l}h(t)$ 吸收非齐次边界，得
\[
v_{tt}+2\beta v_t = c^2 v_{xx}-\dfrac{x}{l}(h''+2\beta h'),\quad
v(0,t)=v(l,t)=0,
\]
\[
v(x,0)=-\dfrac{x}{l}h(0),\quad v_t(x,0)=-\dfrac{x}{l}h'(0).
\]

将 $v$ 按本征函数 $\sin\dfrac{n\pi x}{l}$ 展开，令 $\omega_n=n\pi c/l$，
模态方程为
\[
v_n''+2\beta v_n'+\omega_n^2 v_n = g_n(t),\qquad
g_n(t)=-\frac{2l(-1)^{n+1}}{n\pi}\bigl(h''+2\beta h'\bigr).
\]
齐次解（欠阻尼 $\beta<\omega_n$）：
\[
v_n^{(h)}(t)=\ee^{-\beta t}\bigl(A_n\cos\tilde\omega_n t+B_n\sin\tilde\omega_n t\bigr),\quad
\tilde\omega_n=\sqrt{\omega_n^2-\beta^2},
\]
系数由初值 $v_n(0),v_n'(0)$ 确定。

\noindent\textbf{周期性驱动：} 若 $h(t)=A\sin\Omega t$，则
$g_n(t)=D_n(\Omega^2\sin\Omega t-2\beta\Omega\cos\Omega t)$，
其中 $D_n=2lA(-1)^{n+1}/(n\pi)$。
设 $v_n^{(p)}=\operatorname{Re}[V_n\ee^{\ii\Omega t}]$，代入得
\[
V_n(\omega_n^2-\Omega^2+2\ii\beta\Omega)
= -D_n\Omega(2\beta+\ii\Omega)
\;\Longrightarrow\;
V_n = -\frac{D_n\Omega(2\beta+\ii\Omega)}
{\omega_n^2-\Omega^2+2\ii\beta\Omega}.
\]
稳态解为
\ansmath{
v_n^{(p)}(t)= -D_n\Omega\,
\operatorname{Re}\!\left[
\frac{(2\beta+\ii\Omega)\,\ee^{\ii\Omega t}}
{\omega_n^2-\Omega^2+2\ii\beta\Omega}
\right],
}
其显式形式为
\[
v_n^{(p)}(t)=\frac{D_n}{\Delta_n}\Bigl[
\Omega^2(\omega_n^2-\Omega^2-4\beta^2)\sin\Omega t
-2\beta\Omega\omega_n^2\cos\Omega t
\Bigr],
\]
其中 $\Delta_n=(\omega_n^2-\Omega^2)^2+(2\beta\Omega)^2$。

\ansmath{
u(x,t)=\frac{x}{l}h(t)+\sum_{n=1}^\infty
\bigl[v_n^{(h)}(t)+v_n^{(p)}(t)\bigr]\sin\frac{n\pi x}{l}.
}
