...
\]
积分可得 $\int r^{2}J_{1}\dd{r} = R^{3}J_{2}(\mu_{1n})/\mu_{1n}$，得系数
\[
...
\]
代回 $\xi=x\ee^{-\alpha t}$ 与
$\tau=(1-\ee^{-2\alpha t})/(2\alpha)$，即得所求解。

\textbf{方法二（Laplace--Fourier 变换法）：}
先对 $t$ 作 Laplace 变换，记
\[
U(x,p)=\mathcal{L}_{t}\{u(x,t)\}
=\int_{0}^{\infty}\ee^{-pt}u(x,t)\,\dd t.
\]
由初值条件得
\[
pU-\psi(x)-\kappa U_{xx}+\alpha xU_{x}=0.
\]
再对 $x$ 作 Fourier 变换，记
\[
\hat U(k,p)=\mathcal{F}_{x}\{U(x,p)\},\qquad
\hat\psi(k)=\mathcal{F}_{x}\{\psi(x)\},
\]
并取
\[
\hat f(k)=\int_{-\infty}^{\infty}f(x)\ee^{-\ii kx}\,\dd x.
\]
利用
\[
\mathcal{F}_{x}\{U_{xx}\}=-k^{2}\hat U,
\qquad
\mathcal{F}_{x}\{xU_{x}\}=-\hat U-k\frac{\partial\hat U}{\partial k},
\]
可得关于 $k$ 的一阶常微分方程
\[
\alpha k\frac{\partial\hat U}{\partial k}
+(\alpha-p-\kappa k^{2})\hat U=-\hat\psi(k).
\]

令 $a=\kappa/(2\alpha)$。对 $k\neq0$，取在 $|k|\to\infty$ 时不增长的解，可统一写成
\[
\hat U(k,p)
=\frac{\ee^{ak^{2}}}{\alpha|k|}
\int_{|k|}^{\infty}
\ee^{-ar^{2}}\,
\hat\psi\!\bigl(\operatorname{sgn}(k)r\bigr)
\exp\!\left[-\frac{p}{\alpha}\ln\frac{r}{|k|}\right]\dd r.
\]
其中 $k=0$ 处由连续极限确定。对 $p$ 作逆 Laplace 变换，并记
\[
\hat U(k,t)=\mathcal{L}^{-1}_{p\to t}\{\hat U(k,p)\}.
\]
利用
\[
\mathcal{L}^{-1}_{p\to t}\{\ee^{-p\tau}\}=\delta(t-\tau),
\]
得
\[
\hat U(k,t)
=\frac{\ee^{ak^{2}}}{\alpha|k|}
\int_{|k|}^{\infty}\ee^{-ar^{2}}
\hat\psi\!\bigl(\operatorname{sgn}(k)r\bigr)
\delta\!\left(t-\frac{1}{\alpha}\ln\frac{r}{|k|}\right)\dd r.
\]
$\delta$ 函数的支点为 $r_{*}=|k|\ee^{\alpha t}$，且
\[
\delta\!\left(t-\frac{1}{\alpha}\ln\frac{r}{|k|}\right)
=\alpha r_{*}\,\delta(r-r_{*}),
\]
故
\[
\hat U(k,t)
=\ee^{\alpha t}
\exp\!\left[-\frac{\kappa k^{2}}{2\alpha}
\bigl(\ee^{2\alpha t}-1\bigr)\right]
\hat\psi\!\left(k\ee^{\alpha t}\right).
\]
最后作 Fourier 逆变换，并令 $q=k\ee^{\alpha t}$，得到
\[
u(x,t)=\frac{1}{2\pi}\int_{-\infty}^{\infty}
\ee^{\ii qx\ee^{-\alpha t}}
\exp\!\left[-\frac{\kappa q^{2}}{2\alpha}
\bigl(1-\ee^{-2\alpha t}\bigr)\right]
\hat\psi(q)\,\dd q.
\]
