...
\[
 v(x)=Ce^{\lambda x}.
\]
代入 $v(0)=\ee^{-i\theta}v(\pi)$，得
\[
1=\ee^{-i\theta}\ee^{\lambda\pi}\Longrightarrow \ee^{\lambda\pi}=\ee^{i\theta}.
\]
故
\ansmath{\lambda_n=\frac{i(\theta+2\pi n)}{\pi},\qquad v_n(x)=\ee^{\lambda_n x},\qquad n\in\mathbb Z.}
\end{solution}
\end{question}
...
Duhamel 原理给出
\[
 u(x,t)=\int_0^t \ee^{-\alpha\tau}G(x,t-\tau)\,d\tau.
\]
因此
