...
不够直接；更直接地注意到
\[
\mathcal L\left\{\int_0^x tJ_0(t)\dd{t}\right\}=\frac1s\mathcal L\{xJ_0(x)\}=\frac1s\frac{s}{(s^2+1)^{3/2}}=\frac{1}{(s^2+1)^{3/2}}.
\]
因此
\[
I(x)=\int_0^x tJ_0(t)\dd{t}=xJ_1(x).
\]
故
\ansmath{\int_0^x \sin(x-t)J_0(t)\dd{t}=xJ_1(x).}
\end{solution}
\end{question}
...
\[
\langle \hat Lu,v\rangle=-\int_0^{\pi}u'\bar v\dd{x}=
\int_0^{\pi}u\overline{v'}\dd{x}-u(\pi)\overline{v(\pi)}+u(0)\overline{v(0)}.
\]
...
\[
  v(x)=C\ee^{\lambda x}.
\]
代入 $v(0)=\ee^{-\ii\theta}v(\pi)$，得
\[
 1=\ee^{-\ii\theta}\ee^{\lambda\pi}\Longrightarrow \ee^{\lambda\pi}=\ee^{\ii\theta}.
\]
故
\ansmath{\lambda_n=\frac{\ii(\theta+2\pi n)}{\pi},\qquad v_n(x)=\ee^{\lambda_n x},\qquad n\in\mathbb Z.}
\end{solution}
\end{question}
...
\[
\begin{cases}
 u_t-\kappa u_{xx}=\ee^{-\alpha t}\delta(x), & -\infty<x<\infty,\ t>0,\\
 u(x,0)=0.
\end{cases}
\]
...
Duhamel 原理给出
\[
  u(x,t)=\int_0^t \ee^{-\alpha\tau}G(x,t-\tau)\dd{\tau}.
\]
因此
\ansmath{u(x,t)=\int_0^t \frac{\ee^{-\alpha\tau}}{\sqrt{4\pi\kappa (t-\tau)}}
\exp\!\left(-\frac{x^2}{4\kappa (t-\tau)}\right)\dd{\tau}.}
若换元 $s=t-\tau$，也可写成
\[
  u(x,t)=\ee^{-\alpha t}\int_0^t \frac{\ee^{\alpha s}}{\sqrt{4\pi\kappa s}}
\exp\!\left(-\frac{x^2}{4\kappa s}\right)\dd{s}.
\]
这就是所求显式积分表示。
\end{solution}
\end{question}
...
注意体元为 $dV=\rho\dd{\rho}\dd{\varphi}\dd{z}$，积分后恰给出 $Q$。
\item 在直角坐标下可写成
\ansmath{\rho_e(x,y,z)=\frac{Q}{2\pi a}\,\delta\!\left(\sqrt{x^2+y^2}-a\right)\delta(z).}
也可用 $\delta(x^2+y^2-a^2)$ 改写，但需同时补上雅可比因子。
\item 在球坐标中圆环对应 $r=a,\ \theta=\pi/2$。因体元为 $r^2\sin\theta\dd{r}\dd{\theta}\dd{\varphi}$，可取
...
其中
\[
N_{n,m}=\int_0^{\pi}\!\sin^2 n\theta\dd{\theta}\int_0^R rJ_n^2(j_{n,m}r/R)\dd{r}.
\]
这就是所求本征展开结构。
\end{solution}
\end{question}
