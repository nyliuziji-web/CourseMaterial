...
\hat L=-i\frac{d}{dx},
\qquad
D(\hat L)=\{u\in H^1(0,2\pi):u(2\pi)=\ee^{i\theta}u(0)\},
\]
...
=-\ii\int_0^{2\pi}u'\bar v\dd{x}+\ii\int_0^{2\pi}u\overline{v'}\dd{x}
=-\ii[u\bar v]_0^{2\pi}.
\]
而准周期边界给出
\[
u(2\pi)=\ee^{\ii\theta}u(0),\qquad v(2\pi)=\ee^{\ii\theta}v(0),
\]
故
\[
[u\bar v]_0^{2\pi}=\ee^{\ii\theta}u(0)\ee^{-\ii\theta}\overline{v(0)}-u(0)\overline{v(0)}=0.
\]
...
解为
\[
u(x)=C \ee^{\ii\lambda x}.
\]
代入边界条件：
\[
\ee^{\ii2\pi\lambda}=\ee^{\ii\theta}.
\]
故
\[
...
相应本征函数可取
\ansmath{u_n(x)=\frac{1}{\sqrt{2\pi}}\ee^{\ii(n+\theta/(2\pi))x}.}
\end{enumerate}
\end{solution}
\end{question}
...
\]
的解为
\ansmath{u(x)=\int_0^l G(x,\xi)f(\xi)\dd{\xi}.}
\end{solution}
\end{question}
