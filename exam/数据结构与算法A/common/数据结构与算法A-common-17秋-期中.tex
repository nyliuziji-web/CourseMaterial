\heading{数据结构与算法A-common-17秋-期中}
...
\begin{solution}
随机插入得到的 BST 与快速排序递归树同分布。设 $\operatorname{E}[I_N]$ 为内部路径长度期望，根的左子树规模 $i$ 在 $0\sim N-1$ 上等可能，因此
  \[
  \operatorname{E}[I_N]=N-1+\frac1N\sum_{i=0}^{N-1}(\operatorname{E}[I_i]+\operatorname{E}[I_{N-1-i}]),
  \]
  解得 $\operatorname{E}[I_N]=O(N\log N)$。
\end{solution}
\end{question}
