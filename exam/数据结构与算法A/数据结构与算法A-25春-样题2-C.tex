...
\begin{enumerate}
  \item \texttt{head->next == NULL}
  \item \texttt{head != NULL}
  \item \texttt{head->next == head}
  \item \texttt{head == NULL}
\end{enumerate}
\begin{solution}
选 D。带头结点的单链表空表判断为 \texttt{head->next == NULL}，但不带头结点的情形空表即 \texttt{head == NULL}。本题指后者。
\end{solution}
\end{question}
...
\begin{enumerate}
  \item 冒泡排序（Bubble S||t）
  \item 直接选择排序（Selection S||t）
  \item 快速排序（Quick S||t）
  \item 希尔排序（Shell S||t）
\end{enumerate}
\begin{solution}
...
\begin{solution}
填空答案：(1) \texttt{inDegree[e->v]++;} (2) \texttt{enqueue(\&q, i);} (3) \texttt{seq.push\_back(k);} (4) \texttt{--inDegree[e->v];} (5) \texttt{enqueue(\&q, e->v);} (6) \texttt{seq.size() != n}。
\end{solution}
\end{question}
...
\begin{solution}
填空答案：(1) \texttt{p->next = newNode(a[i]);} (2) \texttt{p->next = p->next->next;} (3) \texttt{p = p->next;}。
\end{solution}
\end{question}
...
\begin{solution}
填空答案：(1) \texttt{return total == G->n;} (2) \texttt{if (u != parent)} (3) \texttt{if (dfsCycle(G, u, v, visited))} (4) \texttt{if (!visited[i])}。
\end{solution}
\end{question}
...
\begin{solution}
填空答案：(1) \texttt{s = (R >= heapSize || a[L] > a[R]) ? L : R;} (2) \texttt{swap(a[i], a[s]);} (3) \texttt{for (int k = n / 2 - 1; k >= 0; --k) goDown(k);} (4) \texttt{swap(a[i], a[0]);} (5) \texttt{goDown(0);}。
\end{solution}
\end{question}
