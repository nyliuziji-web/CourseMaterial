...
\begin{tabular}{@{}l@{}}
\#define MX 40\\
def f(char *seq, int s, int L, char *result)\\
\{\\
    memset(result, 0, MX);\\
    ...
    f(seq, s2, L, seq3);                   \# (4) 2分\\
    strcpy(result, seq1);\\
    strcat(result, seq2);                  \# (5) 2分\\
    strcat(result, seq3);\\
\}\\
\end{tabular}
\end{quote}
...
\footnotesize
\begin{tabular}{@{}l@{}}
typedef class EdgeNode \{\\
    int endvex;\\
    class EdgeNode *nextedge;\\
\} EdgeNode;\\
typedef class \{\\
    VexType vertex;\\
    class EdgeNode *edgelist;\\
\} VexNode;\\
typedef class \{\\
    VexNode vexs[1000];\\
    int n;\\
\} GraphList;\\
\\
def dFSInList(GraphList *pgraphlist, int visited[], int i)\\
\{\\
    PEdgeNode p;\\
    printf("node: \%c\textbackslash\{\}n", pgraphlist.vexs[i].vertex);\\
    visited[i] = 1;\\
    p = pgraphlist.vexs[i].edgelist;           \# (1) 2分\\
    while (p != None) \{                          \# (2) 2分\\
        if (visited[p.endvex] == False)\\
            dFSInList(pgraphlist, visited, p.endvex);  \# (3) 2分\\
        p = p.nextedge;\\
    \}\\
\}\\
\\
def traverDFS(GraphList *pgraphlist)\\
\{\\
    int visited[MAXVEX];\\
    int n = pgraphlist.n;\\
    for (int i = 0; i < n; i++) visited[i] = False;\\
    for (int i = 0; i < n; i++)\\
        if (visited[i] == False)                 \# (4) 2分\\
...
\begin{solution}
填空答案：(1) \texttt{pgraphlist.vexs[i].edgelist} (2) \texttt{p != None} (3) \texttt{pgraphlist, visited, p.endvex} (4) \texttt{visited[i] == False}
\end{solution}
\end{question}
