...
\small
\begin{tabular}{@{}r l@{}}
(1)&\texttt{inorderTraversal(root->left)},\\
(2)&\texttt{inorderTraversal(root->right)},\\
(3)&\texttt{return NULL},\\
(4)&\texttt{newNode(s[ptr])},\\
(5)&\texttt{tree->left = buildTree()},\\
(6)&\texttt{tree->right = buildTree()},\\
(7)&\texttt{buildTree()}.
\end{tabular}
\end{center}
构造时按先序读取：遇到 \texttt{@} 返回空指针；否则生成当前根结点，再递归构造左、右子树。
\end{solution}
\end{question}
