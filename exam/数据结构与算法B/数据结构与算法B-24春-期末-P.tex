...
\small
\begin{tabular}{@{}r l@{}}
(1)&\texttt{nd.next = self.tail.next},\\
(2)&\texttt{self.tail.next == self.tail},\\
(3)&\texttt{ptr.next != self.tail},\\
(4)&\texttt{self.tail = ptr},\\
(5)&\texttt{ptr != self.tail.next}.
\end{tabular}
\end{center}
\end{solution}
\end{question}
...
(3)&\texttt{nodes[L].data = nd},\\
(4)&\texttt{nodes[R].data = nd},\\
(5)&\texttt{nodes[i].data is None}.
\end{tabular}
\end{center}
这里用 \texttt{data} 临时记录父结点编号，未被记录为任何结点儿子的结点即为根。
\end{solution}
\end{question}
...
(3)&\texttt{used[x] = True},\\
(4)&\texttt{(x, prev[x])},\\
(5)&\texttt{G[x][v] is not None and G[x][v] < dist[v]},\\
(6)&\texttt{prev[v] = x}.
\end{tabular}
\end{center}
\end{solution}
...
\small
\begin{tabular}{@{}r l@{}}
(1)&\texttt{not visited[i]},\\
(2)&\texttt{dfs(i, visited, graph, n)},\\
(3)&\texttt{graph[u].append(v)},\\
(4)&\texttt{graph[v].append(u)},\\
(5)&\texttt{not visited[i]},\\
