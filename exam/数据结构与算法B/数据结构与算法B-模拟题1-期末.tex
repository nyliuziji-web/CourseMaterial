...
\]
\begin{center}
\begin{tikzpicture}[x=10mm,y=8mm,every node/.style={draw,circle,font=\small,minimum size=6mm,inner sep=0pt},>=stealth]
\node (v0) at (0,0) {$V_0$};
\node (v1) at (2,1) {$V_1$};
\node (v3) at (2,-1) {$V_3$};
\node (v2) at (4,0) {$V_2$};
\node (v4) at (6,0) {$V_4$};
\draw[->] (v0)--node[above,draw=none,fill=white]{10}(v1);
\draw[->] (v0)--node[below,draw=none,fill=white]{30}(v3);
\draw[->] (v0) to[bend left=25] node[above,draw=none,fill=white]{100}(v4);
\draw[->] (v1)--node[above,draw=none,fill=white]{50}(v2);
\draw[->] (v3)--node[below,draw=none,fill=white]{20}(v2);
\draw[->] (v3) to[bend right=18] node[below,draw=none,fill=white]{60}(v4);
\draw[->] (v2)--node[above,draw=none,fill=white]{10}(v4);
\end{tikzpicture}
\end{center}

\begin{solution}
由矩阵可得有向边：$V_0\to V_1(10)$、$V_0\to V_3(30)$、$V_0\to V_4(100)$、$V_1\to V_2(50)$、$V_3\to V_2(20)$、$V_3\to V_4(60)$、$V_2\to V_4(10)$。从 $V_0$ 执行 Dijkstra 算法：
\begin{center}
\small
\begin{tabular}{c c c c}
\toprule
步骤 & 确定顶点 & 当前距离 $(d_1,d_2,d_3,d_4)$ & 前驱更新 \\ 
\midrule
初始 & $V_0$ & $(10,\infty,30,100)$ & $pre_1=0,pre_3=0,pre_4=0$ \\ 
1 & $V_1$ & $(10,60,30,100)$ & $pre_2=1$ \\ 
2 & $V_3$ & $(10,50,30,90)$ & $pre_2=3,pre_4=3$ \\ 
3 & $V_2$ & $(10,50,30,60)$ & $pre_4=2$ \\ 
4 & $V_4$ & $(10,50,30,60)$ & 完成 \\ 
\bottomrule
\end{tabular}
\end{center}
故最短路径为 $V_0\to V_3\to V_2\to V_4$，长度为 $30+20+10=60$。
\end{solution}
\end{question}
