...
\begin{solution}
填空答案：(1) \texttt{node} (2) \texttt{p.next} (3) \texttt{None} (4) \texttt{p.next} (5) \texttt{self.head} (6) \texttt{self.head = q}
\end{solution}
\end{question}
...
\begin{solution}
填空答案：(1) \texttt{self.left.inorderTraversal()} (2) \texttt{self.right.inorderTraversal()} (3) \texttt{return None} (4) \texttt{BinaryTree(s[ptr])} (5) \texttt{buildTree()} (6) \texttt{buildTree()} (7) \texttt{buildTree()}
\end{solution}
\end{question}
