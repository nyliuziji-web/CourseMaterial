...
\[
\bar n_i^{\rm MB}=g_i \ee^{(\mu-\varepsilon_i)/(k_BT)},\qquad
\bar n_i^{\rm BE}=\frac{g_i}{\ee^{(\varepsilon_i-\mu)/(k_BT)}-1},\qquad
\bar n_i^{\rm FD}=\frac{g_i}{\ee^{(\varepsilon_i-\mu)/(k_BT)}+1}.
\]
经典稀薄气体满足 MB 分布，速度分布为
\[
f(\mathbf v)=n\left(\frac{m}{2\pi k_BT}\right)^{3/2}\ee^{-mv^2/(2k_BT)},
\]
速率分布为
\[
f(v)=4\pi n\left(\frac{m}{2\pi k_BT}\right)^{3/2}v^2\ee^{-mv^2/(2k_BT)}.
\]
...
=\frac{\int_{-\infty}^{\infty}\frac{p_x^2}{2m}\ee^{-p_x^2/(2mk_BT)}\,dp_x}{\int_{-\infty}^{\infty}\ee^{-p_x^2/(2mk_BT)}\,dp_x}
=\frac12k_BT.
\]
一般地，若哈密顿量含有二次项 $a q^2/2$，则
\[
\left\langle \frac12a q^2\right\rangle
=\frac{\int_{-\infty}^{\infty}\frac12a q^2 \ee^{-aq^2/(2k_BT)}\,dq}{\int_{-\infty}^{\infty}\ee^{-aq^2/(2k_BT)}\,dq}
=\frac12k_BT.
\]
