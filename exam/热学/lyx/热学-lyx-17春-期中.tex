...
\end{choiceoptions}
\begin{solution}
泻流分布满足 $F_B(v)\propto v^3\ee^{-mv^2/(2k_BT)}$，与通常体相中的麦克斯韦速率分布不同。
\ansmath{\text{D}}
\end{solution}
\end{question}
...
\begin{solution}
概率密度与 $\ee^{-mgz/(k_BT)}$ 成正比。归一化后
\[
\langle U\rangle=\int_0^{\infty}mgz\cdot \frac{mg}{k_BT}\ee^{-mgz/(k_BT)}\,dz=k_BT.
\]
\end{solution}
\end{question}
...
\]
其中 $F_M(v)\propto v^2 \ee^{-mv^2/(2k_BT)}$ 为三维麦克斯韦速率分布，$\bar v=\sqrt{8k_BT/(\pi m)}$。求该分子束的最概然速率、平均速率和方均根速率。
\begin{solution}
有
\[
F_B(v)\propto v^3\ee^{-\alpha v^2},\qquad \alpha=\frac{m}{2k_BT}.
\]
最概然速率由 $\frac{d}{dv}\ln F_B(v)=0$ 得
\[
v_{\mathrm{mp}}=\sqrt{\frac{3k_BT}{m}}.
\]
平均速率为
\[
\langle v\rangle=\frac{\int_0^{\infty}v^4\ee^{-\alpha v^2}dv}{\int_0^{\infty}v^3\ee^{-\alpha v^2}dv}
=\frac{3\sqrt\pi}{2\sqrt2}\sqrt{\frac{k_BT}{m}}
=\frac{3\pi}{8}\bar v.
\]
方均根速率为
\[
\sqrt{\langle v^2\rangle}=\left(\frac{\int_0^{\infty}v^5\ee^{-\alpha v^2}dv}{\int_0^{\infty}v^3\ee^{-\alpha v^2}dv}\right)^{1/2}
=2\sqrt{\frac{k_BT}{m}}.
\]
...
解得
\[
p_L(t)-\frac{p_0}{2}=\left(p_0-\frac{p_0}{2}\right)\ee^{-2ADt/(VL)}.
\]
即
\[
p_L(t)=\frac{p_0}{2}\left(1+\ee^{-2ADt/(VL)}\right).
\]
\end{solution}
\end{question}
