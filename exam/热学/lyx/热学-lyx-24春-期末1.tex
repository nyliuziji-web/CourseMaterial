...
\[
p_i=\frac{\ee^{-E_i/(k_BT)}}{\sum_j \ee^{-E_j/(k_BT)}}.
\]
证明熵与自由能关系，并求熵函数。
\begin{solution}
配分函数 $Z=\sum_i \ee^{-E_i/(k_BT)}$，自由能
\[
F=-k_BT\ln Z.
\]
于是
...
\]
由 $p_i=\ee^{-E_i/(k_BT)}/Z$ 取对数得
\[
\ln p_i=-\frac{E_i}{k_BT}-\ln Z,
\qquad\text{即}\qquad
E_i=-k_BT\left(\ln p_i+\ln Z\right).
\]
代入内能，并利用归一化 $\sum_i p_i=1$，得
\[
U=\sum_i p_iE_i
=-k_BT\sum_i p_i\ln p_i-k_BT\ln Z\sum_i p_i
=-k_BT\sum_i p_i\ln p_i-k_BT\ln Z.
\]
再由热力学关系 $F=U-TS$，得
\[
S=\frac{U-F}{T}
=\frac{-k_BT\sum_i p_i\ln p_i-k_BT\ln Z+k_BT\ln Z}{T}
