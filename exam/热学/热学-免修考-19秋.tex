...
\ansmath{f(v)=4\pi n\left(\frac{m}{2\pi k_BT}\right)^{3/2}v^2e^{-mv^2/(2k_BT)}}
\end{solution}
\end{question}
...
\]
\ansmath{t=\frac{4V}{A\bar v}\ln\frac{p_0}{p_f}\simeq 3.1\times10^1\,\mathrm s}
\end{solution}
\end{question}
...
\frac{D}{D_0}=2\sqrt2.
\]
\ansmath{\eta:\kappa:D:\bar\lambda=\sqrt2:\sqrt2:2\sqrt2:2\text{（相对于原值的倍数）}}
\end{solution}
\end{question}
...
p\,(V-aT^3-C)=b.
\]
\ansmath{V=aT^3+\frac{b}{p}+C\quad\text{或}\quad p(V-aT^3-C)=b}
\end{solution}
\end{question}
...
\ansmath{C_{V,m}=3R\quad(\text{若为定压热容，则 }C_{p,m}=4R)}
\end{solution}
\end{question}
...
\anstext{一级相变：潜热与一阶导数跳变；二级相变：无潜热、二阶导数异常；超导：对称性破缺；沸腾：通常不按严格对称性破缺分类。}
\end{solution}
\end{question}
...
\]
\ansmath{u_S=\sigma-T\left(\frac{\partial\sigma}{\partial T}\right)_{A_S}}
\end{solution}
\end{question}
...
\ansmath{\bar\lambda=\frac1{n\pi d^2},\quad \bar\lambda'\simeq0.101\,\mathrm m,
\quad T\simeq229\,\mathrm K,
\quad p_{\max}\sim 10^{-9}\,\mathrm{Pa}}
\end{solution}
\end{question}
...
\ansmath{K_{DE}<-\frac13;\quad \text{若取通常能量随膨胀降低且允许不稳定，则 }-1<K_{DE}<-\frac13}
\end{solution}
\end{question}
...
\ansmath{\overline{s^2}=2Dt,\qquad D=\frac{k_BT}{6\pi\eta r_0}}
\end{solution}
\end{question}
...
\ansmath{c^2=\frac{v^2}{M_m}\left[\gamma\frac{RT}{(v-b)^2}-\frac{2a}{v^3}\right]}
\end{solution}
\end{question}
