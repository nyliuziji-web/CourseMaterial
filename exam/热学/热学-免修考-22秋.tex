...
\]
\ansmath{\eta,\kappa,D,\bar\lambda:\ \sqrt2,\sqrt2,2\sqrt2,2}
\end{solution}
\end{question}
...
\anstext{一级：潜热和一阶导数跳变；二级：无潜热、二阶导数异常；超导：对称性破缺；沸腾：普通液气一级相变。}
\end{solution}
\end{question}
...
\ansmath{p=p_0+\frac{4\sigma}{R},\quad
\left(p_0+\frac{4\sigma}{R}\right)\frac{R^3}{R'^3}-p_0+\frac{q^2}{32\pi^2\varepsilon_0R'^4}=\frac{4\sigma}{R'}}
\end{solution}
\end{question}

\begin{question}
雪线高度问题。设近地面温度为 $T_0$，雪线处温度为熔点 $T_m$，大气温度随高度按 lapse rate $\Gamma=-dT/dz$ 下降。求雪线高度。
\begin{solution}
若温度随高度近似线性
\[
T(z)=T_0-\Gamma z,
...
\Gamma_d=\frac{g}{c_p},
\qquad
h=\frac{c_p(T_0-T_m)}{g}.
\]
若空气接近饱和，应将 $\Gamma$ 替换为湿绝热递减率，此时因凝结潜热释放，$\Gamma$ 较小，雪线高度相应更高。
\ansmath{h=\frac{T_0-T_m}{\Gamma}\quad(\text{干绝热时 }h=\frac{c_p(T_0-T_m)}{g})}
\end{solution}
\end{question}
...
\]
\ansmath{\left(\frac{dp}{dT}\right)_{\rm tr}=\frac{\Delta\alpha}{\Delta\kappa_T}}
\end{solution}
\end{question}
