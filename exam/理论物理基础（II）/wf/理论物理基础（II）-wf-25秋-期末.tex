...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  动量表象为
  \[
  \tilde\psi(p)=\frac{1}{\sqrt{2\pi\hbar}}\int_{-\infty}^{\infty}\ee^{-ipx/\hbar}\psi(x)\,dx
  =\frac{A}{\sqrt{2\pi\hbar}}\left(\int_0^L \ee^{-ipx/\hbar}dx-\int_{-L}^0 \ee^{-ipx/\hbar}dx\right),
  \]
  化简得
  ...
  \item 展开为能量本征态后，
  \[
  \varphi(x,t)=\sum_{n=1}^{\infty}\ee^{-iE_nt/\hbar}\psi_n(x)\langle \psi_n|\psi\rangle.
  \]
...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  \[
  1=|C|^2\int_0^{\infty}r^4\ee^{-r/a_0}\,dr
  ...
  因此随时间演化为
  \[
  \varphi(\mathbf r,t)=\frac{\ee^{-iE_2 t/\hbar}}{2}
  \left[(1+\ee^{-iD\hbar t})\psi_{2,1,1}+(1-\ee^{-iD\hbar t})\psi_{2,1,-1}\right].
  \]
...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  \ansmath{M=
  \begin{pmatrix}
  1-i\beta & -i\beta\,\ee^{-2ikL}\\
  i\beta\,\ee^{2ikL} & 1+i\beta
  \end{pmatrix}.}

  \item 将(2)中的转移矩阵与 Bloch 条件结合，要求矩阵
  \[
  \begin{pmatrix}
  (1-i\beta)\ee^{ikL} & -i\beta \ee^{-ikL}\\
  i\beta \ee^{ikL} & (1+i\beta)\ee^{-ikL}
  \end{pmatrix}
  \]
  具有本征值 $\ee^{iKL}$。于是其迹满足
  \[
  \ee^{iKL}+\ee^{-iKL}=2\cos(KL)=2\cos(kL)+2\beta\sin(kL).
  \]
  因而
