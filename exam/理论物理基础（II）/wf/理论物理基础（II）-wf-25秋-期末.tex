...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  \item 归一化条件给出
  \[
  \int_{-L}^L |\psi(x)|^2\,\dd{x}=2L A^2=1,
  \]
  故
  \[
  A=\frac{1}{\sqrt{2L}}.
  ...
  动量表象为
  \[
  \tilde\psi(p)=\frac{1}{\sqrt{2\pi\hbar}}\int_{-\infty}^{\infty}\ee^{-\ii px/\hbar}\psi(x)\,\dd{x}
  =\frac{A}{\sqrt{2\pi\hbar}}\left(\int_0^L \ee^{-\ii px/\hbar}\dd{x}-\int_{-L}^0 \ee^{-\ii px/\hbar}\dd{x}\right),
  \]
  化简得
  \ansmath{A=\frac{1}{\sqrt{2L}},\qquad
  \tilde\psi(p)=\frac{\ii\sqrt\hbar}{p\sqrt{\pi L}}\left[\cos\left(\frac{pL}{\hbar}\right)-1\right].}
  ...
  \item 展开为能量本征态后，
  \[
  \varphi(x,t)=\sum_{n=1}^{\infty}\ee^{-\ii E_nt/\hbar}\psi_n(x)\langle \psi_n|\psi\rangle.
  \]
...
）的一些本征态波函数
\[
\psi_{2,1,m}(r,\theta,\phi)=C\,Y_1^m(\theta,\phi)\,r \ee^{-r/(2a_0)},
\]
其中 $m_e$ 为电子质量，
\[
...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  \[
  1=|C|^2\int_0^{\infty}r^4\ee^{-r/a_0}\,\dd{r}
  ...
  因此随时间演化为
  \[
  \varphi(\mathbf r,t)=\frac{\ee^{-\ii E_2 t/\hbar}}{2}
  \left[(1+\ee^{-\ii D\hbar t})\psi_{2,1,1}+(\ee^{-\ii D\hbar t}-1)\psi_{2,1,-1}\right].
  \]
...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  \ansmath{M=
  \begin{pmatrix}
  1-\ii\beta & -\ii\beta\,\ee^{-2\ii kL}\\
  \ii\beta\,\ee^{2\ii kL} & 1+\ii\beta
  \end{pmatrix}.}

  \item 将(2)中的转移矩阵与 Bloch 条件结合，要求矩阵
  \[
  \begin{pmatrix}
  (1-\ii\beta)\ee^{\ii kL} & -\ii\beta \ee^{-\ii kL}\\
  \ii\beta \ee^{\ii kL} & (1+\ii\beta)\ee^{-\ii kL}
  \end{pmatrix}
  \]
  具有本征值 $\ee^{\ii KL}$。于是其迹满足
  \[
  \ee^{\ii KL}+\ee^{-\ii KL}=2\cos(KL)=2\cos(kL)+2\beta\sin(kL).
  \]
  因而
