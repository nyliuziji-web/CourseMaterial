\heading{理论物理基础（II）-wf-模拟题1-期中}

\instructions{默认记 $\beta=1/(k_B T)$，$z=\ee^{\beta\mu}$ 为逸度，$\lambda=h/\sqrt{2\pi m k_B T}$ 为热波长。若无特别说明，粒子为不可分辨粒子。}
...
\[
p_i \propto \Omega_r(E_{\text{tot}}-E_i)\propto \ee^{-\beta E_i},
\]
...
三种平均占据数分别为
\[
\bar n_r^{\mathrm{MB}} = \ee^{-\alpha-\beta\varepsilon_r},
\qquad
\bar n_r^{\mathrm{BE}} = \frac{1}{\ee^{\alpha+\beta\varepsilon_r}-1},
\qquad
\bar n_r^{\mathrm{FD}} = \frac{1}{\ee^{\alpha+\beta\varepsilon_r}+1}.
\]
当 $\ee^{\alpha+\beta\varepsilon_r}\gg 1$，也即稀薄高温、满足 $n\lambda^3\ll 1$ 的经典极限下，
\[
\bar n_r^{\mathrm{BE}}\approx \bar n_r^{\mathrm{FD}}
\approx \ee^{-\alpha-\beta\varepsilon_r},
\]
...
因此
\[
Z_1=\frac{1}{h^3}\int \dd[3]{p}\,\ee^{-\beta p^2/2m}\int_A \dd{x}\,\dd{y}\int_0^H \dd{z}\,\ee^{-\beta mgz}.
\]
动量积分给出
\[
...
位置积分给出
\[
\int_0^H \ee^{-\beta mgz}\,\dd{z}=\frac{1-\ee^{-\beta mgH}}{\beta mg}.
\]
故
\ansmath{Z_1=
\frac{A(2\pi m k_B T)^{3/2}}{h^3}\,
\frac{1-\ee^{-\beta mgH}}{\beta mg}
=
\frac{A}{\lambda^3}\frac{1-\ee^{-\beta mgH}}{\beta mg}.}
...
由归一化条件
\[
A\int_0^H n(z)\,\dd{z}=N
\]
得
\[
C=\frac{N\beta mg}{A(1-\ee^{-\beta mgH})}.
\]
因此
\ansmath{n(z)=\frac{N\beta mg}{A(1-\ee^{-\beta mgH})}\ee^{-\beta mgz}.}
...
单粒子配分函数为
\[
z_1=\ee^{\beta \mu B}+\ee^{-\beta \mu B}=2\cosh(\beta\mu B).
\]
由于粒子可分辨且相互独立，
...
零温内能为
\[
U=2\cdot \frac{V}{(2\pi)^3}\int_{k\le k_F}\frac{\hbar^2 k^2}{2m}\,\dd[3]{k}
...
故单格点巨配分函数为
\[
\Xi_1=\sum_{N=0,1} \ee^{\beta\mu N}\ee^{-\beta E}
=1+\ee^{\beta(\mu-\varepsilon)}.
\]
$M$ 个格点独立，因此
\ansmath{\Xi=(\Xi_1)^M=\left(1+\ee^{\beta(\mu-\varepsilon)}\right)^M.}
...
=M\frac{x}{1+x}
=
\frac{M}{\ee^{-\beta(\mu-\varepsilon)}+1}.
\]
即
...
M f,
\qquad
f=\frac{1}{\ee^{-\beta(\mu-\varepsilon)}+1}.}
