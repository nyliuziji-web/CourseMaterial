...
\begin{solution}
在圆环上，定态方程解为平面波 $\ee^{in\phi}$。周期性条件要求 $n\in\mathbb Z$。因此
\ansmath{\psi_n(\phi)=\frac{1}{\sqrt{2\pi}}\ee^{in\phi},
\qquad
...
又因
\[
1+\cos\phi=1+\frac{\ee^{\ii\phi}+\ee^{-\ii\phi}}{2},
\]
所以
\[
...
随时间演化后
\[
\psi(\phi,t)=\sqrt{\frac23}\,\psi_0(\phi)+\ee^{-\ii E_1t/\hbar}\left[\frac{1}{\sqrt6}\,\psi_1(\phi)+\frac{1}{\sqrt6}\,\psi_{-1}(\phi)\right],
...
\[
|+\rangle_y=\frac{1}{2}
\begin{pmatrix}-1\\-\ii\sqrt2\\1\end{pmatrix},
\qquad
...
\[
|\psi(t)\rangle=\ee^{-\ii Dt\hat S_y/\hbar}|1\rangle
...
单粒子归一化本征态为
\[
\varphi_n(\phi)=\frac{1}{\sqrt{2\pi}}\ee^{in\phi},
\qquad
