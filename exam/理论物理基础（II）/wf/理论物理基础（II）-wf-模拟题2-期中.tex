\heading{理论物理基础（II）-wf-模拟题2-期中}

\instructions{默认记 $\beta=1/(k_B T)$，$z=\ee^{\beta\mu}$，$\lambda=h/\sqrt{2\pi m k_B T}$。必要时可直接使用积分 $\int_0^\infty \frac{x^{1/2}}{\ee^x-1}\,dx=\Gamma(3/2)\zeta(3/2)$。}
...
巨正则分布写为
\[
P_{r,N}=\frac{1}{\Xi}\ee^{-\beta(E_{r,N}-\mu N)},
\]
其中
\[
\Xi=\sum_{N}\sum_r \ee^{-\beta(E_{r,N}-\mu N)}.
\]
...
单粒子配分函数为
\[
Z_1=\frac{1}{h^3}\int \dd[3]{p}\,\ee^{-\beta p^2/2m}\int \dd[3]{r}\,\ee^{-\beta \alpha r^2/2}.
\]
动量积分给出
\[
\int \dd[3]{p}\,\ee^{-\beta p^2/2m}=(2\pi m k_B T)^{3/2},
\]
位置积分为高斯积分，
\[
\int \dd[3]{r}\,\ee^{-\beta \alpha r^2/2}
...
单振子配分函数为等比级数
\[
z_1=\sum_{n=0}^{\infty}\ee^{-\beta(n+1/2)\hbar\omega}
=\ee^{-\beta\hbar\omega/2}\sum_{n=0}^{\infty}(\ee^{-\beta\hbar\omega})^n.
\]
故
\ansmath{z_1=\frac{\ee^{-\beta\hbar\omega/2}}{1-\ee^{-\beta\hbar\omega}}.}
振子彼此独立，因此
\ansmath{Z=(z_1)^N.}
...
由
\[
\ln z_1=-\frac12\beta\hbar\omega-\ln(1-\ee^{-\beta\hbar\omega}),
\]
得
\[
U=N\left[\frac12\hbar\omega+\frac{\hbar\omega}{\ee^{\beta\hbar\omega}-1}\right].
\]
故
\ansmath{U=N\hbar\omega\left(\frac12+\frac{1}{\ee^{\beta\hbar\omega}-1}\right).}

令 $x=\beta\hbar\omega=\hbar\omega/(k_B T)$，则
\[
U=N\hbar\omega\left(\frac12+\frac{1}{\ee^x-1}\right).
\]
对 $T$ 求导得
\ansmath{C_V
=
N k_B\frac{x^2 \ee^x}{(\ee^x-1)^2}.}
...
\ansmath{P_N=\frac{\langle N\rangle^N}{N!}\ee^{-\langle N\rangle},}
这是泊松分布。
...
\begin{enumerate}[label=(\arabic*),leftmargin=2.5em]
  ...
  N_{\mathrm{ex}}
  =
  \int_0^\infty \frac{g(\varepsilon)\,d\varepsilon}{z^{-1}\ee^{\beta\varepsilon}-1};
  \]
...
对理想 Bose 气体，平均占据数为
\[
\bar n(\varepsilon)=\frac{1}{z^{-1}\ee^{\beta\varepsilon}-1}.
\]
因此激发态总粒子数为
\ansmath{N_{\mathrm{ex}}
=
\int_0^\infty \frac{g(\varepsilon)\,\dd{\varepsilon}}{z^{-1}\ee^{\beta\varepsilon}-1}.}

由于 Bose 分布要求分母为正，故平衡时化学势不能超过基态能量。此处基态能量取为 $0$，所以
\[
\mu\le 0,\qquad z=\ee^{\beta\mu}\le 1.
\]
...
N_{\mathrm{ex}}^{\max}(T)
=
CV\int_0^\infty \frac{\varepsilon^{1/2}\,\dd{\varepsilon}}{\ee^{\beta\varepsilon}-1}.
\]
令 $x=\beta\varepsilon$，则
\[
\varepsilon=\frac{x}{\beta}=k_BTx,\qquad \dd{\varepsilon}=k_B T\,\dd{x},
\]
故
\[
N_{\mathrm{ex}}^{\max}(T)
=
CV(k_B T)^{3/2}\int_0^\infty \frac{x^{1/2}\,\dd{x}}{\ee^x-1}.
\]
利用题给积分，
