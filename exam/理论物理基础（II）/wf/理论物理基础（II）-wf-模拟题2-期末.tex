...
时间演化为
\ansmath{\psi(x,t)=\frac{1}{\sqrt2}\left(\ee^{-i\omega t/2}\psi_0(x)+\ee^{-i3\omega t/2}\psi_1(x)\right).}

位置算符只连接相邻能级，因此
\[
\langle x\rangle_t=\frac12\left(\ee^{-i\omega t}\langle 0|x|1\rangle+\ee^{i\omega t}\langle 1|x|0\rangle\right)
=\sqrt{\frac{\hbar}{2m\omega}}\cos(\omega t).
\]
故
\ansmath{\langle x\rangle_t=\sqrt{\frac{\hbar}{2m\omega}}\cos(\omega t).}

概率密度中只出现相对相位 $\ee^{-i\omega t}$，因此当
\[
\ee^{-i\omega t}=1
\]
时恢复到初始形式，即
...
因此时间演化为
\[
\psi(t)=\frac{1}{\sqrt2}\left(\ee^{-i(E_1+D\hbar)t/\hbar}\psi_++\ee^{-i(E_1-D\hbar)t/\hbar}\psi_-\right).
\]
化回 $\{\psi_x,\psi_y\}$ 基，得
\ansmath{\psi(t)=\ee^{-iE_1 t/\hbar}\left[\cos(Dt)\,\psi_x+\sin(Dt)\,\psi_y\right].}
