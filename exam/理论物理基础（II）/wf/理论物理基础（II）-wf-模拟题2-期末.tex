...
给定波函数
\[
\psi(x)=A\left(1+\sqrt{\frac{2m\omega}{\hbar}}\,x\right)\ee^{-m\omega x^2/(2\hbar)}.
\]
...
谐振子前两个归一化本征态为
\[
\psi_0(x)=\left(\frac{\alpha}{\pi}\right)^{1/4}\ee^{-\alpha x^2/2},
\qquad
\psi_1(x)=\left(\frac{\alpha}{\pi}\right)^{1/4}\sqrt{2\alpha}\,x\,\ee^{-\alpha x^2/2}.
\]
...
时间演化为
\ansmath{\psi(x,t)=\frac{1}{\sqrt2}\left(\ee^{-\ii\omega t/2}\psi_0(x)+\ee^{-\ii 3\omega t/2}\psi_1(x)\right).}

位置算符只连接相邻能级，因此
\[
\langle x\rangle_t=\frac12\left(\ee^{-\ii\omega t}\langle 0|x|1\rangle+\ee^{\ii\omega t}\langle 1|x|0\rangle\right)
=\sqrt{\frac{\hbar}{2m\omega}}\cos(\omega t).
\]
故
\ansmath{\langle x\rangle_t=\sqrt{\frac{\hbar}{2m\omega}}\cos(\omega t).}

概率密度中只出现相对相位 $\ee^{-\ii\omega t}$，因此当
\[
\ee^{-\ii\omega t}=1
\]
时恢复到初始形式，即
...
其第一激发态的两组归一化波函数可取为
\[
\psi_x(x,y)=C\,x\,\ee^{-m\omega(x^2+y^2)/(2\hbar)},
\qquad
\psi_y(x,y)=C\,y\,\ee^{-m\omega(x^2+y^2)/(2\hbar)}.
\]
...
\[
1=C^2\int x^2 \ee^{-\alpha(x^2+y^2)}dxdy=C^2\cdot \frac{\pi}{2\alpha^2},
\]
得
...
直接作用可得
\[
\hat L_z\psi_x=\ii\hbar\psi_y,
\qquad
\hat L_z\psi_y=-\ii\hbar\psi_x.
\]
故在基 $\{\psi_x,\psi_y\}$ 下
...
其本征态为
\[
\psi_+=\frac{\psi_x+\ii\psi_y}{\sqrt2},
\qquad
\psi_-=\frac{\psi_x-\ii\psi_y}{\sqrt2},
\]
...
因此时间演化为
\[
\psi(t)=\frac{1}{\sqrt2}\left(\ee^{-\ii(E_1+D\hbar)t/\hbar}\psi_++\ee^{-\ii(E_1-D\hbar)t/\hbar}\psi_-\right).
\]
化回 $\{\psi_x,\psi_y\}$ 基，得
\ansmath{\psi(t)=\ee^{-\ii E_1 t/\hbar}\left[\cos(Dt)\,\psi_x+\sin(Dt)\,\psi_y\right].}
...
\]
这与无自旋两费米子的一维谐振子基态相同。由反对称态的结构，
\[
\langle x_1^2\rangle=\langle x_2^2\rangle=\frac12\bigl(\langle x^2\rangle_0+\langle x^2\rangle_1\bigr)=\frac{\hbar}{m\omega},
\]
\[
\langle x_1 x_2\rangle=-|\langle 0|x|1\rangle|^2=-\frac{\hbar}{2m\omega},
\]
其中 $\langle 0|x|1\rangle=\sqrt{\hbar/(2m\omega)}$。因此
\[
\langle (x_1-x_2)^2\rangle
=\langle x_1^2\rangle+\langle x_2^2\rangle-2\langle x_1 x_2\rangle
=\frac{2\hbar}{m\omega}+\frac{\hbar}{m\omega}.
\]
故
\ansmath{\langle (x_1-x_2)^2\rangle_{\mathrm{triplet}}=\frac{3\hbar}{m\omega}.}
\end{solution}
\end{question}
