...
\]
故全系统配分函数
\ansmath{Z_N=(1+3\ee^{-x})^N.}

平均能量
\[
U=-\frac{\partial}{\partial \beta}\ln Z_N
=-N\frac{\partial}{\partial \beta}\ln(1+3\ee^{-x})
=N\varepsilon\frac{3\ee^{-x}}{1+3\ee^{-x}}.
\]
因此
\ansmath{
U=N\varepsilon\frac{3\ee^{-x}}{1+3\ee^{-x}}.
}

热容
\[
C=\left(\frac{\partial U}{\partial T}\right)_N
=Nkx^2\frac{3\ee^{-x}}{(1+3\ee^{-x})^2}.
\]
故
\ansmath{
C=Nk\left(\frac{\varepsilon}{kT}\right)^2
\frac{3\ee^{-\varepsilon/(kT)}}{\left(1+3\ee^{-\varepsilon/(kT)}\right)^2}.
}

令
\[
f(x)=x^2\frac{3\ee^{-x}}{(1+3\ee^{-x})^2}.
\]
...
因此热容峰值温度 $T_\ast$ 由
\ansmath{
\left(\frac{\varepsilon}{kT_\ast}-2\right)\ee^{\varepsilon/(kT_\ast)}
=
...
\[
\ee^{-\beta(E-\mu N)}=\ee^{-\beta(-\varepsilon-\mu)}=\ee^{\beta(\mu+\varepsilon)}.
\]
故单个位的巨配分函数为
\[
\Xi_1=1+\ee^{\beta(\mu+\varepsilon)}.
\]
由于各位独立，
\ansmath{
\Xi=\left(1+\ee^{\beta(\mu+\varepsilon)}\right)^M.
}

恰有 $n$ 个位被占据时，组合因子为 $\binom{M}{n}$，相应权重为 $\ee^{\beta(\mu+\varepsilon)n}$，故
\[
P(n)=\frac{\binom{M}{n}\ee^{\beta(\mu+\varepsilon)n}}{\Xi}
=\binom{M}{n}f^n(1-f)^{M-n},
\]
其中
\[
f=\frac{\ee^{\beta(\mu+\varepsilon)}}{1+\ee^{\beta(\mu+\varepsilon)}}
=\frac{1}{\ee^{-\beta(\mu+\varepsilon)}+1}.
\]
所以
\ansmath{
...
故
\ansmath{
\langle n\rangle=\frac{M}{\ee^{-\beta(\mu+\varepsilon)}+1},
\qquad
\mathrm{Var}(n)=Mf(1-f).
}
...
总能量
\[
U=2\cdot \frac{V}{(2\pi)^3}\int_{|\bm k|\le k_F}\frac{\hbar^2k^2}{2m}\,\dd[3]{k}
=\frac{V\hbar^2}{2m\pi^2}\int_0^{k_F}k^4\,\dd{k}
