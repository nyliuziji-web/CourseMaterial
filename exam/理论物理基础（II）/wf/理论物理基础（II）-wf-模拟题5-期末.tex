...
用 Fourier 变换
\[
\tilde\psi(p,0)=\frac{1}{\sqrt{2\pi\hbar}}\int_{-\infty}^{\infty}\ee^{-\ii px/\hbar}\psi(x,0)\,\dd{x}
\]
可得高斯型动量表象
...
在厚势垒极限 $\kappa a\gg 1$ 下，$\sinh(\kappa a)\sim \tfrac12 \ee^{\kappa a}$，故
\[
T\sim \frac{16E(V_0-E)}{V_0^2}\ee^{-2\kappa a}.
\]
因此
\ansmath{T\propto \ee^{-2\kappa a}\qquad (\kappa a\gg 1),}
表现出典型的量子隧穿指数压低。
\end{solution}
\end{question}
