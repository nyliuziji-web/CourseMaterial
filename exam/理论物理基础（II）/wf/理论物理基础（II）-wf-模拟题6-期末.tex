...
初始基态波函数为
\[
\psi_i(x)=\left(\frac{m\omega}{\pi\hbar}\right)^{1/4}\ee^{-m\omega x^2/(2\hbar)},
\]
而新哈密顿量的基态为
\[
\psi_0^{(f)}(x)=\left(\frac{m\Omega}{\pi\hbar}\right)^{1/4}\ee^{-m\Omega x^2/(2\hbar)}.
\]
两者重叠给出新基态振幅
\[
c_0=\int_{-\infty}^{\infty} \bigl(\psi_0^{(f)}\bigr)^*\psi_i\,\dd{x}
...
由于 $\hat L^2$ 在本子空间中只是整体常数，只需考虑 $\ee^{-\ii Dt\hat L_x/\hbar}$ 对初态的作用。它等价于绕 $x$ 轴旋转。由直接指数化或利用转动矩阵可得
\ansmath{|\psi(t)\rangle=\ee^{-\ii\hbar t/I}\left[\cos(Dt)|1,0\rangle-
\frac{\ii\sin(Dt)}{\sqrt2}\bigl(|1,1\rangle+|1,-1\rangle\bigr)\right].}
