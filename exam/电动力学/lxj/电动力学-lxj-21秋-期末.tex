...
\[
  ...
  \qquad
  B_z(\mathbf r,t)=B_z(x,y)\ee^{\ii(kz-\omega t)} .
\]
题中给出
\[
  \mathbf B_\perp=\frac{\ii k}{\Omega^2}\nabla_\perp B_z,
  \qquad
...
\begin{enumerate}
  ...
  该模式可取
  \[
    B_z=B_0\cos\frac{\pi x}{a}\,\ee^{-\ii\omega t} .
  \]
  因为 $k=0$ 时 $\mathbf B_\perp=0$，所以
  \[
    \mathbf B=B_0\cos\frac{\pi x}{a}\,\ee^{-\ii\omega t}\hat{\mathbf z} .
  \]
  用 Maxwell 方程 $\nabla\times\mathbf B=\partial_t\mathbf E$ 求电场。设时间因子为 $\ee^{-\ii\omega t}$，则
  \[
    \nabla\times\mathbf B=\frac{\pi B_0}{a}\sin\frac{\pi x}{a}\,\ee^{-\ii\omega t}\hat{\mathbf y}
    =-\ii\omega\mathbf E .
  \]
  由于 $\omega=\pi/a$，
  \[
    \boxed{\mathbf E=\ii B_0\sin\frac{\pi x}{a}\,\ee^{-\ii\omega t}\hat{\mathbf y}} .
  \]
...
\begin{enumerate}
  ...
  \item 最短的证明是使用四加速度 $a^\mu=\dd{U^\mu}/\dd{\tau}$。它的模
  \[
    a^\mu a_\mu
  \]
...
\begin{enumerate}
  ...
  \item 证明静态解
  \[
    A^\mu(\mathbf r)=\int\frac{J^\mu(\mathbf r')\ee^{-m|\mathbf r-\mathbf r'|}}{|\mathbf r-\mathbf r'|}\,\dd[3]{r'}
  \]
  是方程的解。
...
\begin{enumerate}
  ...
  Yukawa Green 函数满足
  \[
    (\nabla^2-m^2)\frac{\ee^{-m|\mathbf r-\mathbf r'|}}{|\mathbf r-\mathbf r'|}
  ...
  \begin{aligned}
    ...
    \frac{\lambda_0|z'|\ee^{-m\sqrt{\rho^2+z'^2}}}{\sqrt{\rho^2+z'^2}}\,\dd{z'} \\
    &=2\lambda_0\int_0^\infty
    \frac{z'\ee^{-m\sqrt{\rho^2+z'^2}}}{\sqrt{\rho^2+z'^2}}\,\dd{z'} .
  \end{aligned}
  \]
  令 $u=\sqrt{\rho^2+z'^2}$，则 $z'\dd{z'}/u=\dd{u}$，下限 $u=\rho$，上限 $u=\infty$。因此
  \[
    \boxed{\phi(\rho)=\frac{2\lambda_0}{m}\ee^{-m\rho}} .
  \]

  \item 电场为
  \[
    \mathbf E=-\nabla\phi=-\frac{\dd{\phi}}{\dd{\rho}}\hat{\boldsymbol\rho}
    =\boxed{2\lambda_0\ee^{-m\rho}\hat{\boldsymbol\rho}} .
  \]
  当 $m\to0$ 时，电势
  \[
