...
\begin{solution}
\[
\operatorname{E}(r_A)=0.5\times30\%+0.5\times(-10\%)=10\%,
\]
\[
\operatorname{E}(r_B)=0.5\times0\%+0.5\times20\%=10\%.
\]
标准差：
\[
...
该例中，单个资产方差并不小，但由于两种状态下的波动方向相反，恰当配置后可以抵消；因此分散化效果取决于资产之间如何共同波动，而不只取决于每个资产“自己有多波动”。\ansmath{\operatorname{E}(r_A)=\operatorname{E}(r_B)=10\%,\ \sigma_A=20\%,\ \sigma_B=10\%,\ w=\frac13,\ r_f=10\%}
\end{solution}
\end{question}
