...
\]
对偶态，内部解在 $x>0$ 为 $A\ee^{\kappa x}+B\ee^{-\kappa x}$，故 $x=0$ 处
$\psi'(0^+)-\psi'(0^-)=2\kappa(A-B)$、$\psi(0)=A+B$，得
\[
  A={2\kappa-\beta_2\over\beta_2+2\kappa}B.
\]
在 $x=L$ 处，连续性给出 $C=A\ee^{\kappa L}+B\ee^{-\kappa L}$，跳变条件给出
\[
  A\ee^{\kappa L}(2\kappa+\beta_1)+\beta_1 B\ee^{-\kappa L}=0.
\]
两式消去 $A,B$ 得
\[
