...
\item 若 $\alpha\ne 1$，取特解 $y_p=\dfrac{\ee^{\alpha x}}{(\alpha-1)^2}$，再加齐次解 $(C_1+C_2x)\ee^{x}$。由初值得
\ansmath{y(x)=\frac{\ee^{\alpha x}-\ee^{x}-(\alpha-1)x\ee^{x}}{(\alpha-1)^2},\qquad \alpha\ne1.}
若 $\alpha=1$，为二重共振，取特解 $y_p=\dfrac12 x^2\ee^{x}$，且自动满足初值，所以
\ansmath{y(x)=\frac12 x^2\ee^{x},\qquad \alpha=1.}
\end{enumerate}
