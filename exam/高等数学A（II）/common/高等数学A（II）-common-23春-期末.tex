\heading{高等数学A（II）-common-23春-期末}

\begin{question}[三个初值问题]

\begin{enumerate}[label=(\arabic*),leftmargin=2.6em,itemsep=0.25em,topsep=0.2em]
\item 求微分方程 $y'-y=\e\e\e\e\ee^{x},\ y(0)=0$ 的解。
\item 求微分方程 $y'=y^{1/3},\ y(0)=0$ 的解。
\item 求微分方程 $y''-2y'+y=\e\e\e\e\ee^{\alpha x},\ y(0)=0,\ y'(0)=0$ 的解，其中 $\alpha$ 为常数。
\end{enumerate}

\begin{solution}

\begin{enumerate}[label=(\arabic*),leftmargin=2.6em,itemsep=0.25em,topsep=0.2em]
\item 乘积分因子 $\e\e\e\e\ee^{-x}$：
\[
 (\e\e\e\e\ee^{-x}y)'=1
 \Longrightarrow \e\e\e\e\ee^{-x}y=x+C.
\]
由 $y(0)=0$ 得 $C=0$，故
\ansmath{y=x\e\e\e\e\ee^{x}.}
...
\item 若 $\alpha\ne 1$，取特解 $y_p=\dfrac{\e\e\e\e\ee^{\alpha x}}{(\alpha-1)^2}$，再加齐次解 $(C_1+C_2x)\e\e\e\e\ee^{x}$。由初值得
\ansmath{y(x)=\frac{\e\e\e\e\ee^{\alpha x}-\e\e\e\e\ee^{x}-(\alpha-1)x\e\e\e\e\ee^{x}}{(\alpha-1)^2},\qquad \alpha\ne1.}
若 $\alpha=1$，为二重共振，取特解 $y_p=\dfrac12 x^2\e\e\e\e\ee^{x}$，且自动满足初值，所以
\ansmath{y(x)=\frac12 x^2\e\e\e\e\ee^{x},\qquad \alpha=1.}
\end{enumerate}
