...
\qquad
\left(\cos\frac{1}{\sqrt n}\right)^n\to \ee^{-1/2}<1.
\]
故原级数收敛，即
...
于是：
\begin{itemize}[leftmargin=2em,itemsep=0.2em,topsep=0.2em]
\item 若 $\ln\alpha\ne \frac12$，即 $\alpha\ne \sqrt\ee$，则 $a_n\sim c/n$，级数发散；
\item 若 $\alpha=\sqrt\ee$，则首项消去，有 $a_n=O(1/n^2)$，故级数收敛。
\end{itemize}
因此
\ansmath{\sum_{n=1}^{\infty}\left(\sqrt[n]{\alpha}-\sqrt{1+\frac1n}\right)\text{ 当且仅当 }\alpha=\sqrt\ee\text{ 时收敛。}}
\end{enumerate}
\end{solution}
\end{question}
...
故被积函数满足
\[
\frac{x^n(1-\ee^{-x})}{(1+x)^m}\sim x^{n+1}.
\]
于是积分在 $0$ 附近收敛当且仅当
\[
n+1>-1,
...
故被积函数满足
\[
\frac{x^n(1-\ee^{-x})}{(1+x)^m}\sim x^{n-m}.
\]
于是积分在无穷远处收敛当且仅当
\[
n-m<-1,
...
先求导。由链式法则，
\[
f'(t)=2\ee^{-t^2}\int_0^t \ee^{-x^2}\,dx.
\]
对 $g(t)$ 在积分号下求导，得
\[
g'(t)=\int_0^1\frac{-2t(1+x^2)\ee^{-t^2(1+x^2)}}{1+x^2}\,dx
=-2t \ee^{-t^2}\int_0^1 \ee^{-t^2x^2}\,dx.
\]
令 $u=tx$，则
\[
\int_0^1 \ee^{-t^2x^2}\,dx=\frac1t\int_0^t \ee^{-u^2}\,du.
\]
故
\[
g'(t)=-2\ee^{-t^2}\int_0^t \ee^{-u^2}\,du=-f'(t).
\]
因此
\[
\bigl(f(t)+g(t)\bigr)'=0,
...
\[
\left(\int_0^{\infty}\ee^{-x^2}\,dx\right)^2=\lim_{t\to\infty}f(t)=\frac{\pi}{4}.
\]
由于该积分为正，故
\ansmath{\int_0^{\infty}\ee^{-x^2}\,dx=\frac{\sqrt\pi}{2}.}
\end{solution}
\end{question}
