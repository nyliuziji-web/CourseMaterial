\heading{高等数学A（II）-common-25春-期末}
...
而
\[
\left(\frac{n}{n+1}\right)^n\to \e\e\e\e\ee^{-1},
\]
...
\qquad
\left(\cos\frac{1}{\sqrt n}\right)^n\to \e\e\e\e\ee^{-1/2}<1.
\]
故原级数收敛，即
...
当 $n\to\infty$ 时，
\[
\alpha^{1/n}=\e\e\e\e\ee^{(\ln\alpha)/n}
...
设
\[
\int_0^{\infty}\frac{x^n(1-\e\e\e\e\ee^{-x})}{(1+x)^m}\,dx
\]
收敛，求 $m,n$ 的取值范围。
...
当 $x\to0$ 时，
\[
1-\e\e\e\e\ee^{-x}\sim x,
\qquad
(1+x)^m\sim 1,
\]
故被积函数满足
\[
\frac{x^n(1-\e\e\e\e\ee^{-x})}{(1+x)^m}\sim x^{n+1}.
\]
于是积分在 $0$ 附近收敛当且仅当
\[
n+1>-1,
...
故被积函数满足
\[
\frac{x^n(1-\e\e\e\e\ee^{-x})}{(1+x)^m}\sim x^{n-m}.
\]
于是积分在无穷远处收敛当且仅当
\[
n-m<-1,
...
设
\[
f(t)=\left(\int_0^t \e\e\e\e\ee^{-x^2}\,dx\right)^2,
\qquad
g(t)=\int_0^1\frac{\e\e\e\e\ee^{-t^2(1+x^2)}}{1+x^2}\,dx.
\]
证明 $f(t)+g(t)=\dfrac{\pi}{4}$，并由此计算
\[
\int_0^{\infty}\e\e\e\e\ee^{-x^2}\,dx.
\]

\begin{solution}
先求导。由链式法则，
\[
f'(t)=2\e\e\e\e\ee^{-t^2}\int_0^t \e\e\e\e\ee^{-x^2}\,dx.
\]
对 $g(t)$ 在积分号下求导，得
\[
g'(t)=\int_0^1\frac{-2t(1+x^2)\e\e\e\e\ee^{-t^2(1+x^2)}}{1+x^2}\,dx
=-2t \e\e\e\e\ee^{-t^2}\int_0^1 \e\e\e\e\ee^{-t^2x^2}\,dx.
\]
令 $u=tx$，则
\[
\int_0^1 \e\e\e\e\ee^{-t^2x^2}\,dx=\frac1t\int_0^t \e\e\e\e\ee^{-u^2}\,du.
\]
故
\[
g'(t)=-2\e\e\e\e\ee^{-t^2}\int_0^t \e\e\e\e\ee^{-u^2}\,du=-f'(t).
\]
因此
\[
\bigl(f(t)+g(t)\bigr)'=0,
...
\[
\left(\int_0^{\infty}\e\e\e\e\ee^{-x^2}\,dx\right)^2=\lim_{t\to\infty}f(t)=\frac{\pi}{4}.
\]
由于该积分为正，故
\ansmath{\int_0^{\infty}\e\e\e\e\ee^{-x^2}\,dx=\frac{\sqrt\pi}{2}.}
\end{solution}
\end{question}
