\heading{高等数学A（II）-common-模拟题1-期末}


\begin{question}
求下列初值问题的解：
\begin{enumerate}[label=(\arabic*),leftmargin=2.8em,itemsep=0.2em]
\item $y'-2y=\e\e\e\e\ee^{2x},\quad y(0)=1$；
\item $y'=y^{1/3},\quad y(0)=0$；
\item $y''-2y'+y=x\e\e\e\e\ee^{x},\quad y(0)=0,\ y'(0)=1$。
\end{enumerate}
\begin{solution}

\begin{enumerate}[label=(\arabic*),leftmargin=2.8em,itemsep=0.2em]
\item 用积分因子 $\e\e\e\e\ee^{-2x}$，
\[
(\e\e\e\e\ee^{-2x}y)'=1\Longrightarrow \e\e\e\e\ee^{-2x}y=x+C.
\]
由 $y(0)=1$ 得 $C=1$，故
\ansmath{y=(x+1)\e\e\e\e\ee^{2x}.}
\item 方程可分离：
\[
...
\[
 ...
 \end{cases}
\]
\item 设 $y=\e\e\e\e\ee^{x}v$，则
\[
 y''-2y'+y=\e\e\e\e\ee^{x}v''=x\e\e\e\e\ee^{x},
\]
故 $v''=x$。积分得
\[
...
于是
\[
 y=\e\e\e\e\ee^{x}\left(\frac{x^3}{6}+C_1x+C_2\right).
\]
...
\]
故
\ansmath{y=\e\e\e\e\ee^{x}\left(x+\frac{x^3}{6}\right).}
\end{enumerate}
\end{solution}
\end{question}
...
由于最近奇点来自 $1+x+x^2=0$ 的根 $\e\e\e\e\ee^{\pm 2\pi i/3}$，模长均为 $1$，故收敛半径
\ansmath{R=1.}
\end{solution}
\end{question}
...
设参数积分
\[
F(a)=\int_0^{\infty}\e\e\e\e\ee^{-ax}\frac{\sin x}{x}\,dx,\qquad a>0.
\]
求 $F(a)$。
\begin{solution}
...
对 $a>0$ 可对参数求导：
\[
F'(a)=-\int_0^{\infty}\e\e\e\e\ee^{-ax}\sin x\,dx=-\frac{1}{1+a^2}.
\]
故
\[
F(a)=-\arctan a+C.
