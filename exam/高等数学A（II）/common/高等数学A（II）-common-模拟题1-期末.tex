...
\item 用积分因子 $\ee^{-2x}$，
\[
(\ee^{-2x}y)'=1\Longrightarrow \ee^{-2x}y=x+C.
\]
由 $y(0)=1$ 得 $C=1$，故
\ansmath{y=(x+1)\ee^{2x}.}
\item 方程可分离：
...
于是
\[
 y=\ee^{x}\left(\frac{x^3}{6}+C_1x+C_2\right).
\]
...
\]
故
\ansmath{y=\ee^{x}\left(x+\frac{x^3}{6}\right).}
\end{enumerate}
\end{solution}
\end{question}
...
由于最近奇点来自 $1+x+x^2=0$ 的根 $\ee^{\pm 2\pi\ii/3}$，模长均为 $1$，故收敛半径
\ansmath{R=1.}
\end{solution}
\end{question}
...
设参数积分
\[
F(a)=\int_0^{\infty}\ee^{-ax}\frac{\sin x}{x}\,dx,\qquad a>0.
\]
求 $F(a)$。
\begin{solution}
...
对 $a>0$ 可对参数求导：
\[
F'(a)=-\int_0^{\infty}\ee^{-ax}\sin x\,dx=-\frac{1}{1+a^2}.
\]
故
\[
F(a)=-\arctan a+C.
