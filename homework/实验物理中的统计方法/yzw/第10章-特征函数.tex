...
\[
\phi(k)=\int_{-\infty}^{\infty} \ee^{ikx} f(x;\mu,\sigma^2)\,\dd{x}.
\]
...
\[
\phi(k)=\int_0^\infty \ee^{ikx}\frac1{\xi}\ee^{-x/\xi}\,\dd{x}
=\frac1{\xi}\int_0^\infty \ee^{-x(1/\xi-ik)}\,\dd{x}.
\]
...
\int_0^\infty z^{n/2-1}\ee^{-z(1/2-ik)}\,dz.
\]

利用伽马积分 \[
\int_0^\infty z^{a-1}\ee^{-bz}\,dz=\frac{\Gamma(a)}{b^a},
\] 得
...
在 \(k=0\) 展开，或者直接使用 \[
\operatorname{E}[X^m]=\left.\frac{1}{i^m}\frac{d^m\phi}{dk^m}\right|_{k=0},
\] 可得
...
\[
\phi_Y(k)=\ee^{-ik\sqrt{n/2}}\phi_Z\!\left(\frac{k}{\sqrt{2n}}\right)
=\ee^{-ik\sqrt{n/2}}\left(1-\frac{2ik}{\sqrt{2n}}\right)^{-n/2}.
\]
...
\[
\phi(k)=\int_{-\infty}^{\infty}\frac{\ee^{ikx}}{\pi(1+x^2)}\,\dd{x}.
\]
...
\[
\phi(k)=\int_{-\infty}^{\infty} \ee^{ikx}\delta(x-\mu)\,\dd{x}=\ee^{ik\mu}.
\]

反过来，由傅里叶反演公式，

\[
\delta(x-\mu)=\frac{1}{2\pi}\int_{-\infty}^{\infty} \ee^{-ik\mu}\ee^{ikx}\,dk
=\frac1{2\pi}\int_{-\infty}^{\infty} \ee^{ik(x-\mu)}\,dk .
\]
