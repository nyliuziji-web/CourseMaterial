...
\(1/\operatorname{E}[x]\) 进行比较，取 \(\alpha=1,\beta=2\)。
\begin{solution}
若 \(x\sim U[\alpha,\beta]\)，则

\[
\operatorname{E}\left[\frac1x\right]=\frac{1}{\beta-\alpha}\int_\alpha^\beta\frac1x\,\dd{x}
...
\[
\frac1{\operatorname{E}[x]}=\frac{2}{\alpha+\beta}.
\]

取 \(\alpha=1,\beta=2\)，

\[
\operatorname{E}[1/x]=\log2\approx0.6931,
\qquad
1/\operatorname{E}[x]=2/3\approx0.6667.
\]
...
\[
f(x;\xi)=\frac{1}{\xi}\ee^{-x/\xi},\qquad x\ge0.
\]
...
\[
F(x)=\int_0^x\frac1\xi \ee^{-t/\xi}\,\dd{t}=1-\ee^{-x/\xi},\qquad x\ge0.
\]
...
&=\frac{\ee^{-x_0/\xi}-\ee^{-(x_0+x')/\xi}}{\ee^{-x_0/\xi}}\\
&=1-\ee^{-x'/\xi}=P(x\le x').
\end{split}
\]
...
\item
  令 \(x=(y-\mu)/\sigma\)，即
  \(y=\mu+\sigma x\)，\(\left|\frac{\dd y}{\dd x}\right|=\sigma\)。于是
\end{enumerate}

\[
f_X(x)=f_Y(\mu+\sigma x)\sigma
=\frac1{\sqrt{2\pi}}\ee^{-x^2/2},
\]
...
\[
\begin{split}
\operatorname{E}[x]&=\int x f(x;\mu,\sigma^2)\,\dd{x},\\
V[x]&=\int (x-\operatorname{E}[x])^2 f(x;\mu,\sigma^2)\,\dd{x}.
\end{split}
\]
...
\begin{enumerate}
\item
  \(x=\ee^y\)，反变换 \(y=\log x\)，Jacobian 为 \(\left|\frac{\dd y}{\dd x}\right|=1/x\)。所以
\end{enumerate}
...
\begin{enumerate}[start=2]
\item
  由于 \(x=\ee^y\)，且 \(y\sim N(\mu,\sigma^2)\)，高斯矩母函数给出
\end{enumerate}

\[
\operatorname{E}[x]=\operatorname{E}[\ee^y]=\ee^{\mu+\sigma^2/2}.
\]

又

\[
\operatorname{E}[x^2]=\operatorname{E}[\ee^{2y}]=\ee^{2\mu+2\sigma^2},
\]

因此

\[
V[x]=\ee^{2\mu+2\sigma^2}-\ee^{2\mu+\sigma^2}
=\ee^{2\mu+\sigma^2}(\ee^{\sigma^2}-1).
\]
...
\[
V[x]\approx \left(\frac{dx}{dy}\bigg|_{\mu}\right)^2V[y]=\ee^{2\mu}\sigma^2.
\]
...
\[
P(x,n)=\frac{1}{\Gamma(n)}\int_0^x \ee^{-t}t^{n-1}\,dt.
\]
\begin{solution}
自由度为 \(n\) 的 \(\chi^2\) 密度为

\[
f(t;n)=\frac{1}{2^{n/2}\Gamma(n/2)}t^{n/2-1}\ee^{-t/2},\qquad t>0.
\]

累积分布

\[
F_{\chi^2}(x;n)=\int_0^x f(t;n)\,\dd{t}.
\]

令 \(u=t/2\)，\(\dd{t}=2\dd{u}\)，则

\[
F_{\chi^2}(x;n)=\frac{1}{\Gamma(n/2)}\int_0^{x/2}\ee^{-u}u^{n/2-1}\,\dd{u}
