\heading{实验物理中的统计方法-第5章 参数估计的一般概念}
...
\[
\sigma^2=\int_{-\infty}^{\infty}(x-\mu)^2f(x)\,\dd{x},
\]
...
\[
\sigma^2=\operatorname{E}[(x-\mu)^2]=\int_{-\infty}^{\infty}(x-\mu)^2 f(x)\,\dd{x}.
\]

在区域 \(|x-\mu|\ge a\) 上，恒有 \((x-\mu)^2\ge a^2\)，于是

\[
\sigma^2 \ge \int_{|x-\mu|\ge a}(x-\mu)^2f(x)\,\dd{x}
\ge a^2\int_{|x-\mu|\ge a}f(x)\,\dd{x}.
\]
...
\[
\operatorname{E}[\bar x]=\mu,\qquad V[\bar x]=\frac{\sigma^2}{n}.
\]
...
\(\operatorname{E}[x_ix_j]=\mu^2\ (i\ne j)\)，\(\operatorname{E}[x_i^2]=\mu^2+\sigma^2\ (i=1,2,\ldots,n)\)。）
...
\[
\operatorname{E}\left[\sum_{i=1}^n (x_i-\bar x)^2\right]
= n\sigma^2 - nV(\bar x)
...
\[
\operatorname{E}[S^2]=\frac1n\sum_{i=1}^n \operatorname{E}[(x_i-\mu)^2]=\frac1n\sum_{i=1}^n \sigma^2=\sigma^2.
\]
...
\[
V[s^2]=\operatorname{E}[s^4]-(\operatorname{E}[s^2])^2=\frac1n\left(\mu_4-\frac{n-3}{n-1}\mu_2^2\right),
\]

其中 \(\mu_k=\operatorname{E}[(x-\mu)^k]\) 为 \(x\) 的 \(k\) 阶中心矩。为此，需要先证明
\(s^2\) 可以写为
...
\[
\begin{split}
\operatorname{E}[s^4]=&\frac{1}{(n-1)^2}\sum_{i,j=1}^{n}\operatorname{E}[x_i^2x_j^2]
-\frac{2}{n(n-1)^2}\sum_{i,j,k=1}^{n}\operatorname{E}[x_ix_jx_k^2]\\
&+\frac{1}{n^2(n-1)^2}\sum_{i,j,k,l=1}^{n}\operatorname{E}[x_ix_jx_kx_l].
\end{split}
\]
...
的函数。从中减掉本章第 2 题得到的 \((\operatorname{E}[s^2])^2\) 即可得到结果。
...
\begin{solution}
\[
V[s^2]=\operatorname{E}[s^4]-(\operatorname{E}[s^2])^2=
\frac1n\left(\mu_4-\frac{n-3}{n-1}\mu_2^2\right),
\]

其中 \(\mu_k=\operatorname{E}[(x-\mu)^k]\) 是中心矩。
...
由于独立同分布且 \(\operatorname{E}[x]=0\)，许多项会消失，只剩下由 \(\mu_2=\operatorname{E}[x^2]\) 与
\(\mu_4=\operatorname{E}[x^4]\) 组成的项。整理后可得

\[
\operatorname{E}[s^4]=\frac1n\mu_4+
