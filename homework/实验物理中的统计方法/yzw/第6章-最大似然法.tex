...
\[
\operatorname{E}[\hat\mu]=\mu,\qquad V[\hat\mu]=\frac{\sigma^2}{n}.
\]
...
而第五章已知 \(\operatorname{E}[s^2]=\sigma^2\)，高斯情形下
...
\[
\operatorname{E}[\hat\sigma^2]=\frac{n-1}{n}\sigma^2,
\qquad
...
\[
\operatorname{E}[\hat p]=\frac{\operatorname{E}[n]}{N}=p.
\]
...
其中 \(b=\operatorname{E}[\hat\theta]-\theta\) 为估计量的偏置。可分以下几步证明：
...
=\frac{\partial}{\partial\theta}\int\cdots\int f_{\rm joint}(\mathbf{x};\theta)\,\dd{x}_1\cdots \dd{x}_n=0.
\]
...
\item 因为 \(L\) 作为联合密度满足 \(\int L(\mathbf x;\theta)\,\dd{\mathbf x}=1\)，所以
\[
\operatorname{E}[U]=\int L\frac{\partial\log L}{\partial\theta}\,\dd{\mathbf x}
=\int\frac{\partial L}{\partial\theta}\,\dd{\mathbf x}
=\frac{\partial}{\partial\theta}1=0.
\]
因此 \(\operatorname{cov}(\hat\theta,U)=\operatorname{E}[\hat\theta U]\)，且
\(V[U]=\operatorname{E}[U^2]\)。

\item 设偏置 \(b(\theta)=\operatorname{E}[\hat\theta]-\theta\)。由于估计量函数本身不显含
\(\theta\)，
\[
\frac{\partial}{\partial\theta}\operatorname{E}[\hat\theta]
=\frac{\partial}{\partial\theta}\int\hat\theta L\,\dd{\mathbf x}
=\int\hat\theta\frac{\partial L}{\partial\theta}\,\dd{\mathbf x}
=\operatorname{E}[\hat\theta U].
\]
另一方面 \(\operatorname{E}[\hat\theta]=\theta+b(\theta)\)，所以
\[
\operatorname{E}[\hat\theta U]=1+\frac{\partial b}{\partial\theta}.
\]
又由 \(\operatorname{E}[U]=0\) 再对 \(\theta\) 求导：
\[
0=\frac{\partial}{\partial\theta}\operatorname{E}[U]
=\operatorname{E}\left[\frac{\partial U}{\partial\theta}\right]+\operatorname{E}[U^2]
=\operatorname{E}\left[\frac{\partial^2\log L}{\partial\theta^2}\right]+\operatorname{E}[U^2].
\]
故
\[
\operatorname{E}[U^2]=-\operatorname{E}\left[\frac{\partial^2\log L}{\partial\theta^2}\right].
\]
...
{-\operatorname{E}\left[\partial^2\log L/\partial\theta^2\right]}.
\]
...
\[
f(t;\tau)=\frac{1}{\tau}\ee^{-t/\tau},\qquad t\ge0,
\]
...
\[
f(t;\lambda)=\lambda \ee^{-\lambda t},\qquad t\ge0,
\]
...
\(b=\operatorname{E}[\hat\lambda]-\lambda\)。
\begin{solution}
指数分布均值参数形式为
\[
...
\(n>1\) 时
\[
\operatorname{E}\left[\frac1S\right]=\frac{\lambda}{n-1},
\]
所以
\[
\operatorname{E}[\hat\lambda]=n\operatorname{E}\left[\frac1S\right]
=\lambda\frac{n}{n-1},
\qquad
b=\operatorname{E}[\hat\lambda]-\lambda=\frac{\lambda}{n-1}.
\]
...
\[
\operatorname{E}[\hat\theta]=\frac{\sum_i \operatorname{E}[n_i]}{\sum_i a_i}
