...
\[
\lambda_i(\theta)=n\int_{x_i^{\min}}^{x_i^{\max}}f(x;\theta)\,\dd{x},
\]
...
\[
\lambda_i(\theta,\nu)=\nu\int_{x_i^{\min}}^{x_i^{\max}} f(x;\theta)\,\dd{x},
\]
...
\sum_{i=1}^N y_i=n.
\]
于是 \(\sum_i (y_i-\operatorname{E}[y_i])=0\)，存在一个零方差方向，矩阵秩至多为
\(N-1\)。
...
\toprule
\(\theta_i\) (°) & 10 & 20 & 30 & 40 & 50 & 60 & 70 & 80 \\
\midrule
\(\theta_r\) (°) & 8 & \(15\frac12\) & \(22\frac12\) & 29 & 35 & \(40\frac12\) & \(45\frac12\) & 50 \\
\bottomrule
\end{tabular}
...
\[
b=\frac{N-1}{2\sum_{i=1}^{N}a(x_i)}+O(\operatorname{E}[(n_i-\nu_i)^3]).
\]
...
\[
b=-\frac{N-1}{\sum_{i=1}^{N}a(x_i)}+O(\operatorname{E}[(n_i-\nu_i)^3]).
\]
...
\(\operatorname{cov}(n_i,n_j)=\delta_{ij}\nu_i\)，可得
\[
b_{LS}=\operatorname{E}[\hat\theta_{LS}]-\theta
=\frac{N-1}{2\sum_i a_i}+O(\operatorname{E}[(n_i-\nu_i)^3]).
\]
所以普通 LS 有正偏。
...
\[
b_{MLS}
= -\frac{N-1}{\sum_i a_i}+O(\operatorname{E}[(n_i-\nu_i)^3]).
\]
