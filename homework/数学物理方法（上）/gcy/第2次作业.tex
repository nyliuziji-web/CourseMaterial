...
\begin{solution}
\begin{enumerate}
    \item 根据柯西黎曼条件，有 \(\pdv{u}{x} = \pdv{v}{y} = \ee^y \cos x, \pdv{u}{y}= -\pdv{v}{x}=\ee^y \sin x \)。因此有 \( \dd{u} = \ee^y \cos x \dd{x} + \ee^y \sin x \dd{y} \) 容易观察到 \(\dd{u}\) 可以写成全微分 \(\dd{u} = \dd{(\ee^y \sin x)}\)，所以可以得到 \(u(x,y)= \ee^y \sin x +C\)，其中 \(C\) 为任意常实数。综上，我们可以写出 \( f(z)=\ee^y (\sin x + \ii  \cos x) + C = \ii \ee^{-\ii x +y} + C=\ii \ee^{-\ii z} + C  \)
    \item 根据柯西黎曼条件，有 \(\pdv{u}{x}=\pdv{v}{y}=x/(x^2+y^2)^2 \cdot (-2y), \pdv{u}{y}=-\pdv{v}{x}=(x^2-y^2)/(x^2+y^2)^2\)。因此有\( \dd{u} = \frac{-2x y \dd{x} +(x^2 - y^2) \dd{y}}{(x^2+y^2)^2} \) 容易观察到 \(\dd{u}\) 可以写成全微分 \(\dd{u}=\dd{\frac{y}{x^2 + y^2}}\)，所以可以得到 \(u(x,y)=\frac{y}{x^2+y^2}+C\)，其中 \(C\) 为任意常实数。综上，我们可以写出 \( f(z)=\frac{y+\ii x}{x^2+y^2} + C=\frac{\ii}{z} + C \)
\end{enumerate}
\end{solution}
\end{question}
...
\]
因此可以得到 \[
\dd{v}=\pdv{v}{x} \dd{x}+\pdv{v}{y} \dd{y} = -\dv{b}{y} \dd{x} + \dv{a}{x} \dd{y}
\]
这个时候注意到 \[
