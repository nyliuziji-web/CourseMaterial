...
(3) \(\sin \sqrt{z}\)； \\
(4) \(\cos \sqrt{z}\)； &
(5) \(\displaystyle \frac{\sin \sqrt{z}}{\sqrt{z}}\)； &
(6) \(\displaystyle \frac{\cos \sqrt{z}}{\sqrt{z}}\)。
\end{tabular}
\end{center}
\begin{solution}
上述表达式中 (1)(2)(3)(6) 为多值函数，(4)(5) 为函数。
\end{solution}
\end{question}

\begin{question}
\(n\) 为任意给定的整数，由 \( \ee^{1+2n \pi \ii} = \ee \) 等量替换，有 \( (\ee^{1+2n \pi \ii})^{1+2n \pi \ii} = \ee \)
又 \( \ee^{(1+2n \pi \ii)(1+2n \pi \ii)} = \ee^{(1+2n \pi \ii)^2}=\ee^{1+4n \pi \ii - 4n^2 \pi^2}=\ee \cdot \ee^{4n \pi \ii} \cdot \ee^{-4n^2 \pi^2}=\ee \cdot \ee^{-4n^2 \pi^2} \)
比较(2)式和(3)式，得 \( \ee^{-4n^2 \pi^2}=1, \quad n=0, \pm 1, \pm 2, \dots \)
以上推导，哪里出问题了？请分析如何修正才能得到一致的结果。
\begin{solution}
在上述的推导过程中，一般而言有 \( (\ee^{1+2n \pi \ii})^{1+2n \pi \ii} \neq \ee^{(1+2n \pi \ii)(1+2n \pi \ii)} \)
...
\]
所以有 \[
(z^{\alpha})^{\beta}= \ee^{(1+2\pi n \ii)(1)} =1
\]
\end{solution}
\end{question}
