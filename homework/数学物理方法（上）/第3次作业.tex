...
\end{center}
\begin{solution}
(1) 根式 \(1-z^3=0\) 时，有 \(z=1,\omega , \omega^2\)，其中 \(\omega=\ee^{\frac{2\pi}{3} \ii}\)。接下来验证其是否为分支点，我们可以将原式改写为 \[
...
(2) 根式 \(1-z^3=0\) 时，有 \(z=1,\omega,\omega^2\)，其中 \(\omega = \ee^{\frac{2\pi}{3} \ii}\)。接下来验证其是否为分支点，我们可以将原式改写为 \[
...
\end{center}
\begin{solution}
(1) \(z^{\ii}=\ee^{\ii \ln z}= \ee^{\ii (\ln|z|+\ii \arg z)}=\ee^{-\arg z+\ii \ln|z|}\)，
\begin{itemize}
    ...
    \item \(z=1+\ii\)，有 \(\arg (1+\ii)=2k \pi+\frac{\pi}{4}, |1+\ii|=\sqrt{2}\)，故 \((1+\ii)^{\ii} = \ee^{-\pi-2k \pi+\ii \ln \sqrt{2}},(k \in \mathbb{Z})\)。
\end{itemize}

(2) \(\ln(z^{\ii})=\ln(\ee^{\ii \ln z})=-\arg z+\ii \ln|z| + \ii \cdot 2 \pi q\)，
\begin{itemize}
    ...
    \item \(z=1+\ii\)，有 \(\arg (1+\ii)=2k \pi+\frac{\pi}{4}, |1+\ii|=\sqrt{2}\)，故 \(\ln((1+\ii)^{\ii}) = -\pi-2k \pi+\ii (2 q \pi + \ln \sqrt{2}),(k, q \in \mathbb{Z})\)。
\end{itemize}
\end{solution}
\end{question}
...
(1) 对于路径 \(C_1\)，有 \(\Delta \arg (\frac12-z)=\pi, \Delta \arg(\frac12+z)=2\pi\)，因此有 \[
\arg \sqrt{(1+2z)(1-2z)}=\frac{2\pi +2\pi + \pi}{2}= \frac52 \pi,
\quad \arg\frac{1+2z}{1-2z}=4\pi-\pi+2\pi=5\pi,
\] 故有 \(f(3)=\ln(7/5)+(\sqrt{35} + 5\pi) \ii\)。

(2) 对于路径 \(C_2\)，有 \(\Delta \arg(\frac12-z)=-3\pi, \Delta \arg(\frac12+z)=-2\pi\)，因此有 \[
...
\draw[red,thick] (-1.1000,0.1000) arc[start angle=174.67, delta angle=174.55, radius=1.1024];
\draw[red,thick] (-0.9000,-0.1000) arc[start angle=-173.81, delta angle=186.03, radius=0.9026];
\draw[red,thick] (-2,0.1) -- (-1.1,0.1);
\draw[red,thick] (-2,-0.1) -- (-0.9,-0.1);
\node at (0,-2.8) {(a)};
\end{scope}
% (b)
...
\node at (0,1) [left] {$i$}; \node at (0,-1) [left] {$-i$};
\foreach \q in {(0,1),(0,-1),(-1,0),(1,0)} \filldraw \q circle (2pt);
\draw[red,thick] (0.9,0) arc (0:180:0.1);
\draw[red,thick] (-1.1000,-0.1000) arc[start angle=-174.67, delta angle=174.55, radius=1.1024];
\draw[red,thick] (-0.9000,0.1000) arc[start angle=173.81, delta angle=186.03, radius=0.9026];
\draw[red,thick] (-2,-0.1) -- (-1.1,-0.1);
\draw[red,thick] (-2,0.1) -- (-0.9,0.1);
...
\textbf{(a)} 由题，当 \(z=0\) 时，\(w=0\)，故 \(\bigl.\arg(1-z^2)\bigr|_{z=0}=0\)。考察路径 \(C_1\)，有
\[
...
\node[blue] at (1.5,-0.5) [below] {$C_1$};
\end{tikzpicture}
\end{center}

\textbf{(b)} 由题，当 \(z=0\) 时，\(w=0\)，故 \(\bigl.\arg(1-z^2)\bigr|_{z=0}=0\)。考察路径 \(C_2\)，有
\[
\Delta\arg(1-z)=-\pi,\quad \Delta\arg(1+z)=0,
\]
所以 \(\arg(1-z^2)=-\pi\)，因此有
...
\draw[red,thick] (-2,0.1) -- (-0.9,0.1);
\draw[blue,thick,->] (0,0) .. controls (1.5,0.5) .. (3,0);
\node[blue] at (1.5,0.5) [above] {$C_2$};
\end{tikzpicture}
\end{center}

\textbf{(c)} 沿着 \(C_1\) 可以走到割线下岸，即得到 \(w=\ln 8+\pi\ii\)；反之，沿着 \(C_2\) 可以走到割线上岸，即得到 \(w=\ln 8-\pi\ii\)。
...
\end{enumerate}
\begin{solution}
设 \(z=\ee^{\ii\theta}\)，可以得到 \(\dfrac{\dd{z}}{\sqrt{z}}=\ii\ee^{\ii\theta}\dd{\theta}\cdot\ee^{-\ii\theta/2}=\ii\ee^{\ii\theta/2}\dd{\theta}\)。

\begin{enumerate}[label=(\roman*)]
  \item \(\displaystyle I_1=\int_{0}^{\pi}\ii\ee^{\ii\theta/2}\dd{\theta}=\bigl.2\ee^{\ii\theta/2}\bigr|_{0}^{\pi}=2\ii-2\)。
  \item \(\displaystyle I_2=\int_{0}^{-\pi}\ii\ee^{\ii\theta/2}\dd{\theta}=\bigl.2\ee^{\ii\theta/2}\bigr|_{0}^{-\pi}=-2\ii-2\)。
\end{enumerate}
\end{solution}
\end{question}
...
\[
\begin{array}{ll}
\text{(1)}\ \displaystyle\int_C\operatorname{Im}z\,\dd{z}, &
\text{(2)}\ \displaystyle\int_C|\operatorname{Im}z|\,\dd{z},\\[8pt]
\text{(3)}\ \displaystyle\int_C\operatorname{Im}z\,|\dd{z}|, &
\text{(4)}\ \displaystyle\int_C|\operatorname{Im}z|\,|\dd{z}|,
\end{array}
\]
...
\begin{enumerate}[label=(\arabic*)]
  \item \(\displaystyle I_1=\int_{-1}^{1}(-1)\,\dd{x}+\int_{-1}^{1} y\cdot\ii\,\dd{y}=-2+\Bigl.\frac{\ii}{2}y^2\Bigr|_{-1}^{1}=-2\)。
  \item \(\displaystyle I_2=\int_{-1}^{1}1\,\dd{x}+\int_{-1}^{1}|y|\,\ii\,\dd{y}=2+\ii\)。
  \item \(\displaystyle I_3=\int_{-1}^{1}(-1)\,\dd{x}+\int_{-1}^{1} y\,\dd{y}=-2+\Bigl.\frac12y^2\Bigr|_{-1}^{1}=-2\)。
  \item \(\displaystyle