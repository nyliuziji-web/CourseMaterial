...
\begin{enumerate}
  \item \(\displaystyle f(z)=\oint_{|\zeta|=2} \frac{(\zeta^*)^2 \ee^{\zeta}}{\zeta-z} \dd{}\zeta\)，其中 \(\zeta^*\) 为复数 \(\zeta\) 的复共轭，\(z\) 为复数且 \(|z|\neq2\).

  \item \(\displaystyle \oint_{|z|=1} \ln\frac{1+2z}{1-2z} \cdot \frac{dz}{z+2}\)，沿实轴从 \(\displaystyle -\frac12\) 到 \(\displaystyle \frac12\) 作割线，规定割线上岸 \(\displaystyle \arg\frac{1+2z}{1-2z}=2\pi\).
...
\node at (0.5,0) [below=3pt] {$\frac12$};
\node at (-0.5,0) [below=3pt] {$-\frac12$};
\node at (-1,0) [above=3pt] {$-1$};
\node at (0,-1) [below=3pt] {$-i$};
\node at (0,1) [above=3pt] {$i$};
\node at (1,0) [above=3pt] {$1$};
\node at (-2,0) [above=3pt] {$-2$};
\end{tikzpicture}
\end{center}
...
\begin{solution}
\begin{enumerate}
  \item 在 \(|\zeta|=2\) 的圆上，\(\zeta^*=4/\zeta\)，代入可以得到
\[ f(z)=\oint_{|\zeta|=2} \frac{16\ee^{\zeta}}{\zeta^2(\zeta-z)}\dd{\zeta} \]
...
\end{cases} \]
  ...
  \item 在上述割线的定义下 \(f(z)=\ln\dfrac{1+2z}{1-2z}\) 在 \(\mathbb{C}\setminus\text{割线}\) 上解析。先考察一半径为 \(R\) 的大圆 \(C_R\) 与积分围道 \(C\) 组成的区域
...
\begin{enumerate}
  \item \(\displaystyle \oint_{|z|=2} \frac{\sin z}{z^4}\dd{z}\);\qquad
  \item \(\displaystyle \oint_{|z|=2} \frac{dz}{z^2(z^2+16)}\);\qquad
  \item \(\displaystyle \oint_{|z|=2} \frac{|z|\ee^{z}}{z^2}\dd{z}\).

\end{enumerate}
\begin{solution}

\begin{enumerate}
  \item 考察函数 \(f(z)=\displaystyle\oint_{|\xi|=2} \frac{\phi(\xi)}{\xi-z}\dd{}\xi\)，其中 \(\phi(\xi)=\sin\xi\) 在围道上连续。当 \(|z|<2\) 时，由柯西积分公式，可以得到
...
\[ \oint_{|\xi|=2} \frac{\sin\xi}{\xi^4}\dd{}\xi = -\frac{\pi}{3}\ii. \]
  ...
  \item 考察函数 \(f(z)=\displaystyle\oint_{|\xi|=2} \frac{\phi(\xi)\dd{}\xi}{\xi-z}\)，其中 \(\phi(\xi)=\dfrac1{\xi^2+16}\) 在围道上连续。当 \(|z|<2\) 时，由柯西积分公式，可以得到
...
\[ \oint_{|\xi|=2} \frac{\phi(\xi)}{\xi^2}\dd{}\xi = 0. \]
  ...
  \item 考察函数 \(f(z)=\displaystyle\oint_{|\xi|=2} \frac{\phi(\xi)\dd{}\xi}{\xi-z}\)，其中 \(\phi(\xi)=2\ee^{\xi}\) 在围道上连续。当 \(|z|<2\) 时，由柯西积分公式，可以得到
...
\begin{question}
\begin{enumerate}
  \item 计算积分 \(\displaystyle \oint_{|z|=1} \frac{\ee^{z}}{z^3}\dd{z}\)；

  \item \(a\) 取何值时，变上限积分 \(\displaystyle F(z)=\int_{z_0}^{z} \ee^{\zeta}\Bigl(\frac1\zeta+\frac{a}{\zeta^3}\Bigr)\dd{}\zeta\) 在 \(z\) 复平面内是单值的？

\end{enumerate}
\begin{solution}
\begin{enumerate}
  \item 考察函数 \(f(z)=\displaystyle\oint_{|\xi|=1} \frac{\ee^{\xi}}{\xi-z}\dd{\xi}\)。根据柯西积分公式，当 \(|z|<1\) 时
\[ f(z)=2\pi\ii\,\ee^{z} \]
因此由 \(\ee^{\xi}\) 在 \(|\xi|=1\) 曲线上的连续性，可以知道
\[ f^{(2)}(z)=2! \oint_{|\xi|=1} \frac{\ee^{\xi}}{(\xi-z)^3}\dd{}\xi = 2\pi\ii\,\ee^{z} \]
代入 \(z=0\)，可以得到
\[ \oint_{|\xi|=1} \frac{\ee^{\xi}}{\xi^3}\dd{\xi} = \pi\ii.  \]

  \item 注意到 \(f(\zeta)=\ee^{\zeta}\Bigl(\dfrac1\zeta+\dfrac{a}{\zeta^3}\Bigr)\) 在 \(\mathbb{C}\setminus\{0\}\) 上解析。故若我们考察从 \(z_0\) 到 \(z\) 的两条不同的积分路径 \(\gamma_1,\gamma_2\)，若 \(\gamma_1,\gamma_2\) 围出的区域不包含 \(0\)，则
...
考察 \(f(\zeta)=\ee^{\zeta}\Bigl(\dfrac1\zeta+\dfrac{a}{\zeta^3}\Bigr)\) 在 \(\zeta=0\) 处的留数。展开 \(\ee^{\zeta}=1+\zeta+\dfrac{\zeta^2}{2!}+\dfrac{\zeta^3}{3!}+\cdots\)，有
\[
f(\zeta)=\Bigl(1+\zeta+\frac{\zeta^2}{2}+\frac{\zeta^3}{6}+\cdots\Bigr)\Bigl(\frac1\zeta+\frac{a}{\zeta^3}\Bigr)
=\frac{a}{\zeta^3}+\frac{a}{\zeta^2}+\Bigl(1+\frac{a}{2}\Bigr)\frac1\zeta+\cdots
\]
为使变上限积分 \(F(z)\) 单值，需 \(\operatorname{res}\{f(\zeta),0\}=0\)，即 \(1+\dfrac{a}{2}=0\)，解得 \(a=-2\).
\end{enumerate}
\end{solution}
\end{question}
