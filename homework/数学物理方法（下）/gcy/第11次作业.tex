...
第三式中
\[
\frac{\mu'}{\mu}=\frac{1-x-1}{x}=-1,\qquad \mu=\ee^{-x},
\]
故
\[
(x\ee^{-x}y')'+\lambda \ee^{-x}y=0.
\]

第四式中
\[
\frac{\mu'}{\mu}=-2x,\qquad \mu=\ee^{-x^2},
\]
故
\[
(\ee^{-x^2}y')'+2\lambda \ee^{-x^2}y=0.
\]
综上，四式分别对应
\[
p=x^2,\quad w=x;
...
\]
\[
p=x\ee^{-x},\quad w=\ee^{-x};
\]
\[
p=\ee^{-x^2},\quad w=2\ee^{-x^2}.
\]
\end{solution}
\end{question}
...
形式伴随仍为
\[
\hat L^\dagger z=-\frac{\dd}{\dd{x}}\left(p(x)z'\right)+q(x)z
\]
因为 \(p,q\) 为实函数。这个结论不是直接猜出来的，而是由分部积分得到。对任意允许函数 \(y,z\)，
\[
\int_a^b z\left[-(py')'+qy\right]\dd{x}
=\int_a^b pz'y'\,\dd{x}+\int_a^b qzy\,\dd{x}-\left[zpy'\right]_a^b,
\]
再对 \(pz'\) 项反向分部积分：
\[
\int_a^b y\left[-(pz')'+qz\right]\dd{x}
+\left[p(yz'-zy')\right]_a^b.
\]
于是 Green 公式为
\[
\int_a^b(z\hat Ly-y\hat Lz)\,\dd{x}
...
代入 \(x=l\) 的边界条件：
\[
\lambda A\cos\lambda l-\ii\lambda\beta A\sin\lambda l=0.
\]
非零解要求
\[
\cot\lambda l=\ii\beta.
\]
因此
\[
\lambda_n l=\frac{(2n+1)\pi}{2}-\ii\,\operatorname{arctanh}\beta,
\qquad n=0,1,2,\ldots .
\]
当 \(\beta\ll1\) 时
\[
\lambda_n\approx \frac{(2n+1)\pi}{2l}-\ii\frac{\beta}{l}.
\]
\end{solution}
\end{question}
...
边界条件给出
\[
\ee^{-\ii\lambda b}=\ee^{\ii\theta}\ee^{-\ii\lambda a},
\]
故
\[
...
本征函数可取
\[
y_n(x)=\exp\left(-\ii\lambda_n x\right).
\]
\end{solution}
\end{question}
...
\exp\left[-\frac{(x-\xi)^2}{4\kappa t}\right]\dd{\xi}.
\]
即
\[
...
按提示先取边界
\[
u(a,z)=V_0\ee^{-k|z|}\operatorname{sgn}z
\]
...
\hat f(s)\frac{I_0(|s|\rho)}{I_0(|s|a)}\ee^{\ii sz}\,\dd{s},
\qquad
\hat f(s)=\frac{2V_0}{\ii s}.
\]
等价地，
\[
u(\rho,z)=\frac{2V_0}{\pi}\int_0^\infty
\frac{I_0(k\rho)}{I_0(ka)}\frac{\sin kz}{k}\,\dd{k}.
\]
\end{solution}
\end{question}
