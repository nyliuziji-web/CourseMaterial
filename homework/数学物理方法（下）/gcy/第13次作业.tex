...
\[
y(x)=\int_{-1}^1G(x,\xi)\phi(\xi)\,\dd{\xi} .
\]
\end{solution}
\end{question}
...
G''+k^2G=0.
\]
出射条件要求 \(x>x'\) 时取 \(\ee^{\ii k(x-x')}\)，\(x<x'\) 时取 \(\ee^{\ii k(x'-x)}\)，所以设
\[
G=C \ee^{\ii k|x-x'|}.
\]
在 \(x=x'\) 附近积分方程，得到导数跳跃
...
代入上式得 \(2ikC=-1\)，因此
\[
G_1(x,x')=\frac{\ii}{2k}\ee^{\ii k|x-x'|}.
\]
...
\[
H_0^{(1)}(kR)\sim 1+\frac{2\ii}{\pi}\ln R
\]
与二维基本解的对数奇性匹配，得
\[
G_2(\bm r,\bm r')=\frac{\ii}{4}H_0^{(1)}(k|\bm r-\bm r'|).
\]
\end{solution}
\end{question}
...
作空间变换，得到每个波数的受迫振子
\[
\hat G_{tt}+a^2|\bm k|^2\hat G=\ee^{-\ii\bm k\cdot\bm r'}\delta(t-t').
\]
迟滞条件给出
\[
\hat G=\frac{\sin(a|\bm k|(t-t'))}{a|\bm k|}H(t-t')\ee^{-\ii\bm k\cdot\bm r'}.
\]
...
由三维热核得
\[
u(\bm r,t)=\int_{\mathbb R^3}G(\bm r,t;\bm r',0)\phi(\bm r')\,\dd{\bm r'}
+\int_0^t\int_{\mathbb R^3}G(\bm r,t;\bm r',\tau)f(\bm r',\tau)\,\dd{\bm r'}\dd{\tau}.
\]
其中 \(G\) 为上一题热核。
\end{solution}
\end{question}
...
乘以积分因子 \(\ee^{-x^2}\)，方程化为
\[
\left(\ee^{-x^2}y'\right)'+2n\ee^{-x^2}y=0.
\]
对应泛函可取
\[
J[y]=\int_0^1 \ee^{-x^2}\left[(y')^2-2ny^2\right]\dd{x},
\]
边界条件为 \(y(0)=0,\ y(1)=1\)。
\end{solution}
\end{question}
