...
边界函数为奇函数，只有正弦项，且
\[
B_n=\frac1\pi\int_0^{2\pi}u(a,\phi)\sin n\phi\,\dd{\phi}
...
Neumann 问题的相容条件为
\[
\iint_D A\,\dd{S}=\int_{\partial D}B\,\dd{s}.
\]
故
\[
...
由 \(u(a,\phi)=100\) 得
\[
A_n=\frac{2}{\alpha}\int_0^\alpha100\sin\frac{n\pi\phi}{\alpha}\,\dd{\phi}
...
取
\[
v=A r \ee^{-(p\pi)^2\kappa t}+w.
\]
则 \(w(0,t)=w(1,t)=0\)，且
\[
w_t-\kappa w_{rr}=A\kappa(p\pi)^2r \ee^{-(p\pi)^2\kappa t}.
\]
...
\left(\ee^{-(n\pi)^2\kappa t}-\ee^{-(p\pi)^2\kappa t}\right).
\]
因此
\[
u(r,t)=A \ee^{-(p\pi)^2\kappa t}
+\frac1r\sum_{n=1}^{\infty}b_n(t)\sin n\pi r.
\]
若 \(p=m\) 为正整数，\(n=m\) 项由共振极限给出
\[
b_m(t)=2A(-1)^m m\pi\kappa t\,\ee^{-(m\pi)^2\kappa t},
\]
其余项仍用上式。
\end{solution}
\end{question}
