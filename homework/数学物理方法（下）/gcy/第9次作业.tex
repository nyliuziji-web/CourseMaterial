...
相加、相减得到。于是
\[
\int_0^x t^{n+1}J_n(t)\,\dd{t}=x^{n+1}J_{n+1}(x)
\]
由第一条递推式直接得到。其余题目通过分部积分把多余的 \(t\) 次幂降下去，再反复用上面的递推式化为相邻阶柱函数。例如
\[
\int_0^x t^3J_0(t)\,\dd{t}
\]
令 \(dv=tJ_0(t)\,\dd{t}=\dd[tJ_1(t)]\)，则
\[
\int_0^x t^3J_0(t)\,\dd{t}
=x^3J_1(x)-2\int_0^x t^2J_1(t)\,\dd{t}.
\]
再由 \(\dd[t^2J_2(t)]=t^2J_1(t)\,\dd{t}\)，得
\[
\int_0^x t^3J_0(t)\,\dd{t}
=x^3J_1(x)-2x^2J_2(x).
\]
...
\[
\frac{\dd}{\dd{x}}\left[x(J_m'J_n-J_n'J_m)\right]
=\frac{m^2-n^2}{x}J_mJ_n.
\]
因此
\[
\int\frac1xJ_nJ_m\,\dd{x}
...
取 \(\nu=1,\lambda=2\)，得到
\[
\int_0^\infty\frac{J_1^2(x)}{x^2}\,\dd{x}=\frac{4}{3\pi}.
\]
\end{solution}
\end{question}
