...
\pyfig[0.55\linewidth]{figures/第2次作业-fig01.png}{两端固定的均匀铁索（悬链线）示意图}
\begin{solution}
...
\textbf{(iii)} 由 Euler--Lagrange 方程一次积分：
...
\pyfig[0.4\linewidth]{figures/第2次作业-fig02.png}{绕铅直轴匀速转动的光滑圆环}
\begin{itemize}
...
一维阻尼振子的 Lagrange 量为
\[
L = \ee^{\lambda t}\Bigl(\frac12 m\dot q^2 - \frac12 m\omega^2 q^2\Bigr),
\]
...
\begin{itemize}
  ...
  q(t) \to \tilde q(\tilde t) = \ee^{-\lambda\alpha/2}\, q(t)
  \]
  下，\(\tilde S[\tilde q] = S[q]\)；
...
\[
S[q] = \int L\,\dd{t}
= \int \ee^{\lambda t}\Bigl(\frac12 m\dot q^2 - \frac12 m\omega^2 q^2\Bigr)\dd{t} .
\]
变换后的作用量为
\[
\tilde S[\tilde q] = \int \ee^{\lambda\tilde t}
\Bigl(\frac12 m\frac{\dd{\tilde q}}{\dd{\tilde t}}^2
- \frac12 m\omega^2 \tilde q^2\Bigr)\dd{\tilde t} .
\]
由于 \(\tilde t = t + \alpha\)，有 \(\dd{\tilde t} = \dd{t}\)，且 \(\tilde q(\tilde t) = \ee^{-\lambda\alpha/2} q(t)\)。因此
\[
\frac{\dd{\tilde q}}{\dd{\tilde t}}
= \ee^{-\lambda\alpha/2}\frac{\dd{q}}{\dd{t}},
\qquad
\tilde q = \ee^{-\lambda\alpha/2} q(t) .
\]
代入：
\begin{align*}
\tilde S[\tilde q]
&= \int \ee^{\lambda(t+\alpha)}\Bigl(
\frac12 m\, \ee^{-\lambda\alpha}\dot q^2
- \frac12 m\omega^2 \ee^{-\lambda\alpha} q^2\Bigr)\dd{t} \\
&= \int \ee^{\lambda t}\Bigl(\frac12 m\dot q^2 - \frac12 m\omega^2 q^2\Bigr)\dd{t}
= S[q] .
\end{align*}
...
\delta q = \tilde q(\tilde t) - q(t) = \ee^{-\lambda\alpha/2} q(t) - q(t)
...
计算
\[
\frac{\partial L}{\partial\dot q} = \ee^{\lambda t}\, m\dot q .
\]
代入：
\begin{align*}
I &= \ee^{\lambda t} m\dot q\Bigl(-\frac{\lambda\alpha}{2} q + \dot q\alpha - \dot q\alpha\Bigr)
+ \alpha\, \ee^{\lambda t}\Bigl(\frac12 m\dot q^2 - \frac12 m\omega^2 q^2\Bigr) \\
&= \ee^{\lambda t} m\dot q\Bigl(-\frac{\lambda\alpha}{2} q\Bigr)
+ \alpha\, \ee^{\lambda t}\Bigl(\frac12 m\dot q^2 - \frac12 m\omega^2 q^2\Bigr) \\
&= \alpha\, \ee^{\lambda t}\Bigl(\frac12 m\dot q^2 - \frac12 m\omega^2 q^2
- \frac{\lambda m}{2} q\dot q\Bigr).
\end{align*}
去掉常数因子 \(\alpha\)，Noether 守恒量为
\[
\boxed{\;I = \ee^{\lambda t}\Bigl(\frac12 m\dot q^2 - \frac12 m\omega^2 q^2
- \frac{\lambda m}{2} q\dot q\Bigr)\;}.
\]
