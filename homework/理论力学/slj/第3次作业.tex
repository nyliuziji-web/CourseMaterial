...
单自由度系统的 Hamilton 量为
\[
H(q,p,t) = \frac{p^2}{2m} - b \ee^{-\lambda t} p q + \frac{mb}{2} \ee^{-\lambda t}(\lambda + b \ee^{-\lambda t}) q^2 + \frac12 k q^2,
\]
其中 \(m,b,\lambda,k\) 为常数。
...
\[
\dot q = \frac{\partial H}{\partial p} = \frac{p}{m} - b \ee^{-\lambda t} q .
\]
反解出 \(p\)：
\[
p = m\bigl(\dot q + b \ee^{-\lambda t} q\bigr).
\]

Legendre 变换 \(L = p\dot q - H\)，代入 \(p\) 和 \(H\)：
\[
\begin{aligned}
L &= m(\dot q + b \ee^{-\lambda t} q)\dot q
- \frac{1}{2m}\cdot m^2(\dot q + b \ee^{-\lambda t} q)^2
+ b \ee^{-\lambda t}\cdot m(\dot q + b \ee^{-\lambda t} q) q \\
&\qquad - \frac{mb}{2} \ee^{-\lambda t}(\lambda + b \ee^{-\lambda t})q^2 - \frac12 k q^2 \\[2mm]
&= m\dot q^2 + m b \ee^{-\lambda t} q\dot q
- \frac12 m(\dot q + b \ee^{-\lambda t} q)^2
+ m b \ee^{-\lambda t} q\dot q + m b^2 \ee^{-2\lambda t} q^2 \\
&\qquad - \frac{mb}{2} \ee^{-\lambda t}(\lambda + b \ee^{-\lambda t}) q^2 - \frac12 k q^2 .
\end{aligned}
\]

注意到 \((\dot q + b \ee^{-\lambda t} q)^2 = \dot q^2 + 2b \ee^{-\lambda t} q\dot q + b^2 \ee^{-2\lambda t} q^2\)，代入化简：
\[
\begin{aligned}
L &= m\dot q^2 + m b \ee^{-\lambda t} q\dot q
- \frac12 m\dot q^2 - m b \ee^{-\lambda t} q\dot q - \frac12 m b^2 \ee^{-2\lambda t} q^2 \\
&\qquad + m b \ee^{-\lambda t} q\dot q + m b^2 \ee^{-2\lambda t} q^2
- \frac{mb}{2} \ee^{-\lambda t}(\lambda + b \ee^{-\lambda t}) q^2 - \frac12 k q^2 \\[2mm]
&= \frac12 m\dot q^2 + m b \ee^{-\lambda t} q\dot q
+ \frac12 m b^2 \ee^{-2\lambda t} q^2
- \frac{mb}{2} \ee^{-\lambda t}(\lambda + b \ee^{-\lambda t}) q^2 - \frac12 k q^2 .
\end{aligned}
\]
...
\[
\begin{aligned}
L &= \frac12 m\dot q^2 + m b \ee^{-\lambda t} q\dot q
+ \frac12 m b^2 \ee^{-2\lambda t} q^2
- \frac{mb\lambda}{2} \ee^{-\lambda t} q^2
- \frac12 m b^2 \ee^{-2\lambda t} q^2 - \frac12 k q^2 \\[2mm]
&= \frac12 m\dot q^2 + m b \ee^{-\lambda t} q\dot q
- \frac{mb\lambda}{2} \ee^{-\lambda t} q^2 - \frac12 k q^2 .
\end{aligned}
\]

因此
\[
\boxed{\;L = \frac12 m\dot q^2 + m b \ee^{-\lambda t} q\dot q
- \frac{mb\lambda}{2} \ee^{-\lambda t} q^2 - \frac12 k q^2\;}.
\]

\textbf{(ii) 将 \(L\) 化为不显含时间的形式 \(L'\)。}

考察交叉项 \(m b \ee^{-\lambda t} q\dot q\)。注意到
\[
\frac{\mathrm d}{\mathrm d t}\Bigl(\frac12 m b \ee^{-\lambda t} q^2\Bigr)
= m b \ee^{-\lambda t} q\dot q - \frac12 m b \lambda \ee^{-\lambda t} q^2 .
\]

故
\[
m b \ee^{-\lambda t} q\dot q
= \frac{\mathrm d}{\mathrm d t}\Bigl(\frac12 m b \ee^{-\lambda t} q^2\Bigr)
+ \frac12 m b \lambda \ee^{-\lambda t} q^2 .
\]
...
中间两项恰好抵消，故
\[
L = \frac12 m\dot q^2 - \frac12 k q^2 + \frac{\mathrm d}{\mathrm d t}\Bigl(\frac12 m b \ee^{-\lambda t} q^2\Bigr) .
\]
...
\[
L' = L - \frac{\mathrm d}{\mathrm d t}\Bigl(\frac12 m b \ee^{-\lambda t} q^2\Bigr),
\]
即两个 Lagrange 量相差一个全导数，因此给出完全相同的 Euler--Lagrange 方程。相应的正则方程亦等价。事实上，从 \(H\) 出发的正则方程为
\[
\dot q = \frac{\partial H}{\partial p} = \frac{p}{m} - b \ee^{-\lambda t} q,\qquad
\dot p = -\frac{\partial H}{\partial q},
\]
而从 \(H'\) 出发的正则方程为
\[
...
它们通过变量代换 \(p' = m\dot q = p + m b \ee^{-\lambda t} q\) 相联系，在该代换下两组方程完全等价。因此 \(H\) 和 \(H'\) 描述同一物理系统，给出相同的运动轨迹。
\end{solution}
\end{question}
