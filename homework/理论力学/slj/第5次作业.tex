...
\[
\boxed{\;(q,p)\;\longrightarrow\;(\ee^{\varepsilon}q,\;\ee^{-\varepsilon}p)\;},
\]
即相平面上的伸缩变换。

\noindent\textbf{第二类生成函数：} 伸缩变换 \((q,p)\to(Q,P)=(q \ee^\varepsilon,\;p \ee^{-\varepsilon})\) 等价于
\[
Q=q\,\ee^{\varepsilon},\qquad P=p\,\ee^{-\varepsilon}.
\]
第二类生成函数 \(F_2(q,P,t)\) 满足
\[
p=\frac{\partial F_2}{\partial q},\qquad Q=\frac{\partial F_2}{\partial P}.
\]
由 \(p=P\,\ee^{\varepsilon}\) 积分得 \(F_2(q,P)=P\,\ee^{\varepsilon}q\)；但需验证 \(Q\)：
\[
Q=\frac{\partial F_2}{\partial P}=q\,\ee^{\varepsilon},
\]
与变换一致。因此
\[
\boxed{\;F_2(q,P)=qP \ee^{\varepsilon}\;}.
\]
...
\[
L=\ee^{2\lambda t}\Bigl(\frac12 m\dot q^2-\frac12 m\omega^2 q^2\Bigr),
\qquad\lambda,m,\omega\ \text{正常数}.
\]
\begin{itemize}
  \item[\textup{(i)}] 求系统的 Hamilton 量；
  \item[\textup{(ii)}] 考虑第二类生成函数 \(F_2(q,P,t)=\ee^{\lambda t}qP\)，求变换后的 Hamilton 量 \(K(Q,P,t)\)；
...
Hamilton 量
\[
H=p\dot q-L=\frac{p^2}{2m}\ee^{-2\lambda t}+\frac12 m\omega^2 \ee^{2\lambda t}q^2 .
\]
故
\[
\boxed{\;H=\frac{p^2}{2m}\ee^{-2\lambda t}+\frac12 m\omega^2 \ee^{2\lambda t}q^2\;}.
\]

\noindent\textbf{(ii)} 第二类生成函数 \(F_2(q,P,t)=\ee^{\lambda t}qP\)，变换关系为
\[
p=\frac{\partial F_2}{\partial q}=\ee^{\lambda t}P,\qquad
Q=\frac{\partial F_2}{\partial P}=\ee^{\lambda t}q .
\]
故
\[
\boxed{\;P=p\,\ee^{-\lambda t},\qquad Q=q\,\ee^{\lambda t}\;}.
\]

新 Hamilton 量
\[
K=H+\frac{\partial F_2}{\partial t}
=\Bigl(\frac{p^2}{2m}\ee^{-2\lambda t}+\frac12 m\omega^2 \ee^{2\lambda t}q^2\Bigr)
+\lambda \ee^{\lambda t}qP .
\]
以新变量表示：\(p=P \ee^{\lambda t},\; q=Q \ee^{-\lambda t}\)，代入得
\[
...
\boxed{\;
\begin{aligned}
q(t)&=\ee^{-\lambda t}Q(t)=A \ee^{-\lambda t}\cos(\Omega t+\delta),\\[4pt]
p(t)&=\ee^{\lambda t}P(t)=m \ee^{\lambda t}\bigl[\dot Q(t)-\lambda Q(t)\bigr]
=-m A \ee^{\lambda t}\bigl[\Omega\sin(\Omega t+\delta)+\lambda\cos(\Omega t+\delta)\bigr].
\end{aligned}
\;}
\]
...
\noindent\textbf{角频率：}
\[
\frac{1}{T}=\frac{\partial E}{\partial J}
...
=\frac{\pi\lambda}{2\sqrt{2mE}} .
\]

用作用量变量表示亦可：
\[
\omega=2\pi\frac{\partial E}{\partial J}
=\frac{4\pi}{3}\Bigl(\frac{3\lambda}{8\sqrt{2m}}\Bigr)^{2/3}J^{-1/3}.
\]
...
\omega=\frac{\pi\lambda}{2\sqrt{2mE}}
=\frac{4\pi}{3}\Bigl(\frac{3\lambda}{8\sqrt{2m}}\Bigr)^{2/3}J^{-1/3}.
\;}
\]
