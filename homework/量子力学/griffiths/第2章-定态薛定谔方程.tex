...
证明下列三个定理:
\begin{enumerate}
\item 对归一化的解,其分离变量常数 $E$ 必为实数。提示:把 $E$ 写成 $E_0+\ii\Gamma$ 的形式,其中 $E_0$ 和 $\Gamma$ 都是实数;然后证明若式 (1.20) 对任何时刻 $t$ 都成立,则 $\Gamma$ 必为零。
\item 定态波函数 $\psi(x)$ 总可以取作实数。提示:对给定能量 $E$,如果 $\psi(x)$ 满足定态薛定谔方程,那么 $\psi^*(x)$ 也满足;于是 $\psi+\psi^*$ 和 $\ii(\psi-\psi^*)$ 给出实函数解。
...
\[
\psi=A\ee^{\kappa x}+B\ee^{-\kappa x}.
\]
由 $\psi(0)=0$ 得 $A+B=0$,由 $\psi(a)=0$ 得 $A(\ee^{\kappa a}-\ee^{-\kappa a})=0$,所以 $A=B=0$。因此 $E\le0$ 时不存在非零物理解。
\end{solution}
\end{question}
...
\[
\Psi(x,t)=\frac1{\sqrt2}\left[\psi_1(x)\ee^{-\ii\omega t}+\psi_2(x)\ee^{-4\ii\omega t}\right].
\]
因此
\[
...
\[
\Psi(x,0)=A[\psi_1(x)+\ee^{\ii\phi}\psi_2(x)],
\]
...
仍有 $A=1/\sqrt2$。含时态为
\[
\Psi(x,t)=\frac1{\sqrt2}\left[\psi_1\ee^{-\ii\omega t}+\ee^{\ii\phi}\psi_2\ee^{-4\ii\omega t}\right].
\]
于是
\[
...
(2) 展开为无限深方势阱定态:
\[
\Psi(x,t)=\sum_{n=1}^{\infty}c_n\psi_n(x)\ee^{-\ii E_nt/\hbar}.
\]
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.21):若 $\Psi=\sum_n c_n\psi_n \ee^{-\ii E_nt/\hbar}$,则 $\langle H\rangle=\sum_n |c_n|^2E_n$。}
...
\[
\psi_n(x)=\left(\frac{\alpha}{\pi}\right)^{1/4}\frac{1}{\sqrt{2^n n!}}H_n(\sqrt\alpha x)\ee^{-\alpha x^2/2}.
\]
因 $H_2(\xi)=4\xi^2-2$,
\[
\psi_2(x)=\left(\frac{\alpha}{\pi}\right)^{1/4}\frac1{\sqrt2}(2\alpha x^2-1)\ee^{-\alpha x^2/2}.
\]
...
(2) 
\[
\Psi(x,t)=\frac15\left[3\psi_0\ee^{-\ii\omega t/2}+4\psi_1\ee^{-3\ii\omega t/2}\right].
\]
因此
\[
...
基态概率密度为
\[
|\psi_0|^2=\sqrt{\frac{\alpha}{\pi}}\ee^{-\alpha x^2},
\qquad \alpha=\frac{m\omega}{\hbar}.
\]
故经典禁区概率为
\[
P=2\int_{A_c}^{\infty}\sqrt{\frac{\alpha}{\pi}}\ee^{-\alpha x^2}\dd x
=\operatorname{erfc}(1)
\approx 0.157.
\]
\end{solution}
...
\item 罗德里格斯公式为
\[
H_n(\xi)=(-1)^n \ee^{\xi^2}\left(\frac{d}{d\xi}\right)^n \ee^{-\xi^2} .
\]
利用该式推出 $H_3$ 和 $H_4$。
...
\[
\ee^{-z^2+2z\xi}=\sum_{n=0}^{\infty}\frac{z^n}{n!}H_n(\xi).
\]
...
(4) 展开母函数
\[
\ee^{-z^2+2z\xi}=1+2\xi z+(2\xi^2-1)z^2+\left(\frac43\xi^3-2\xi\right)z^3+\cdots.
\]
与 $\sum z^nH_n/n!$ 比较,得
\[
H_1=2\xi,
...
\begin{question}
证明 $(A\ee^{\ii kx}+B\ee^{-\ii kx})$ 与 $(C\cos kx+D\sin kx)$ 是同一函数的等价表示形式;用 $A$ 和 $B$ 表示 $C$ 和 $D$,并用 $C$ 和 $D$ 表示 $A$ 和 $B$。注释:在量子力学中,当 $V=0$ 时,用指数形式讨论自由粒子的行波最方便;正弦和余弦则对应驻波,常出现在无限深方势阱问题中。
\begin{solution}
利用欧拉公式,
\[
A\ee^{\ii kx}+B\ee^{-\ii kx}
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.95):自由粒子平面波:$\Psi_k(x,t)=A\ee^{\ii(kx-\omega t)}$,$\omega=\hbar k^2/(2m)$。}

\begin{solution}
自由粒子行波可写为
\[
\Psi(x,t)=A\ee^{\ii(kx-\omega t)}.
\]
几率流为
\[
...
\[
f(x)=\sum_{n=-\infty}^{\infty}c_n \ee^{\ii n\pi x/a},
\]
并把 $c_n$ 用普通傅里叶系数表示出来。
\item 证明
\[
c_n=\frac{1}{2a}\int_{-a}^{a}f(x)\ee^{-\ii n\pi x/a}\,dx .
\]
\item 引入 $k=n\pi/a$、$\Delta k=\pi/a$ 和 $F(k)=\sqrt{2a/\pi}\,c_n$,证明
\[
f(x)=\frac{1}{\sqrt{2\pi}}\sum F(k)\ee^{\ii kx}\Delta k,
\qquad
F(k)=\frac{1}{\sqrt{2\pi}}\int_{-a}^{a}f(x)\ee^{-\ii kx}\,dx .
\]
...
\]
用 $\cos u=(\ee^{\ii u}+\ee^{-\ii u})/2$、$\sin u=(\ee^{\ii u}-\ee^{-\ii u})/(2\ii)$ 可写成
\[
f(x)=\sum_{n=-\infty}^{\infty}c_n\ee^{\ii n\pi x/a},
\]
...
(2) 由正交性
\[
\int_{-a}^{a}\ee^{\ii(n-m)\pi x/a}\dd x=2a\delta_{mn},
\]
故
\[
c_n=\frac1{2a}\int_{-a}^{a}f(x)\ee^{-\ii n\pi x/a}\dd x.
\]

(3) 令 $k=n\pi/a$、$\Delta k=\pi/a$、$F(k)=\sqrt{2a/\pi}c_n$,则
\[
f(x)=\frac1{\sqrt{2\pi}}\sum_k F(k)\ee^{\ii kx}\Delta k,
\]
且
\[
F(k)=\frac1{\sqrt{2\pi}}\int_{-a}^{a}f(x)\ee^{-\ii kx}\dd x.
\]

(4) 当 $a\to\infty$ 时,$\Delta k\to0$,求和变成积分,得到
\[
f(x)=\frac1{\sqrt{2\pi}}\int_{-\infty}^{\infty}F(k)\ee^{\ii kx}\dd k,
\qquad
F(k)=\frac1{\sqrt{2\pi}}\int_{-\infty}^{\infty}f(x)\ee^{-\ii kx}\dd x.
\]
\end{solution}
\end{question}
...
(1) 归一化:
\[
1=2A^2\int_0^\infty \ee^{-2ax}\dd x=\frac{A^2}{a},
\qquad A=\sqrt a.
\]

(2) 傅里叶振幅为
\[
\phi(k)=\frac1{\sqrt{2\pi}}\int_{-\infty}^{\infty}\sqrt a\,\ee^{-a|x|}\ee^{-\ii kx}\dd x
...
(1) 归一化给出
\[
1=A^2\int_{-\infty}^{\infty}\ee^{-2ax^2}\dd x,
\qquad
...
\item $\displaystyle \int_{-\infty}^{\infty}(\ee^{|x|}+3)\delta(x-2)\,dx$。
\end{enumerate}
\begin{solution}
利用 $\delta$ 函数的抽样性质:
\[
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.132):$\delta$ 势阱 $V(x)=-\alpha\delta(x)$ 的束缚态:}\[\begin{aligned}\psi(x)&=\sqrt{m\alpha}/\hbar\,\ee^{-m\alpha|x|/\hbar^2},\\E&=-m\alpha^2/(2\hbar^2).\end{aligned}\]
...
\qquad
\psi(x)=\sqrt\beta\,\ee^{-\beta |x|}.
\]
显然 $\langle x\rangle=0$,并且
\[
\langle x^2\rangle=2\beta\int_0^\infty x^2\ee^{-2\beta x}\dd x=\frac1{2\beta^2}.
\]
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (2.132):$\delta$ 势阱 $V(x)=-\alpha\delta(x)$ 的束缚态:$\psi(x)=\sqrt{m\alpha}/\hbar\,\ee^{-m\alpha|x|/\hbar^2}$,$E=-m\alpha^2/(2\hbar^2)$。
...
\[
\delta(x)=\frac{1}{2\pi}\int_{-\infty}^{\infty}\ee^{\ii kx}\,dk .
\]
评注:这个公式需要以广义函数的意义理解。
\begin{solution}
按本章的傅里叶约定,
\[
F(k)=\frac1{\sqrt{2\pi}}\int_{-\infty}^{\infty}\delta(x)\ee^{-\ii kx}\dd x
=\frac1{\sqrt{2\pi}}.
\]
代回反变换,
\[
\delta(x)=\frac1{\sqrt{2\pi}}\int_{-\infty}^{\infty}F(k)\ee^{\ii kx}\dd k
=\frac1{2\pi}\int_{-\infty}^{\infty}\ee^{\ii kx}\dd k.
\]
...
\[
\beta=\eta\left(1+\ee^{-2\beta a}\right),
\qquad\text{即}\qquad z=g(1+\ee^{-z}),
\]
奇态满足
\[
\beta=\eta\left(1-\ee^{-2\beta a}\right),
...
阱外为指数衰减,
\[
\psi(x)=B\operatorname{sgn}(x)\ee^{-\kappa |x|},
\qquad
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.154):有限深方势阱偶束缚态可取}\[\psi(x)=\begin{cases}F\ee^{\kappa x},&x<-a,\\D\cos(\ell x),&|x|<a,\\F\ee^{-\kappa x},&x>a.\end{cases}\]
...
\[
F \ee^{-\kappa a}=D\cos(\ell a),
\qquad
F=D\cos(\ell a)\ee^{\kappa a}.
\]
归一化条件为
\[
1=D^2\int_{-a}^{a}\cos^2(\ell x)\dd x
+2F^2\int_a^\infty \ee^{-2\kappa x}\dd x.
\]
代入 $F$ 得
\[
...
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (2.170--2.172):由上述边界条件得有限深方势阱散射的透射率 $T=\{1+[V_0^2/(4E(E+V_0))]\sin^2(2\ell a)\}^{-1}$,其中 $\ell=\sqrt{2m(E+V_0)}/\hbar$。
\item 式 (2.166--2.169):有限方势阱散射中取 $\psi_I=A\ee^{\ii kx}+B\ee^{-\ii kx}$、$\psi_{II}=C\ee^{\ii\ell x}+D\ee^{-\ii\ell x}$、$\psi_{III}=F\ee^{\ii kx}$,并在 $x=\pm a$ 令 $\psi$ 和 $\psi'$ 连续。
\end{enumerate}
...
\quad
\psi_{II}=C\ee^{\ii\ell x}+D\ee^{-\ii\ell x},
\quad
\psi_{III}=F\ee^{\ii kx}.
\]
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.99):一维概率流:$J=(\hbar/m)\operatorname{Im}(\Psi^*\partial_x\Psi)$;平面波 $A\ee^{\ii kx}$ 的流为 $\hbar k|A|^2/m$。}
...
(3) 几率流对 $C\ee^{\ii kx}$ 为 $j=(\hbar k/m)|C|^2$。因此
\[
T=\frac{j_{\rm tr}}{j_{\rm \ii n}}
...
含时波函数为
\[
\Psi(x,t)=\frac3{\sqrt{10}}\psi_1\ee^{-\ii E_1t/\hbar}
-\frac1{\sqrt{10}}\psi_3\ee^{-\ii E_3t/\hbar}.
\]
...
时,
\[
\ee^{-\ii E_nT/\hbar}=\exp(-\ii 2\pi n^2)=1,
\]
...
\[
\Psi(x,0)=A\left(1-2\sqrt{\frac{m\omega}{\hbar}}x\right)^2 \ee^{-m\omega x^2/(2\hbar)},
\]
其中 $A$ 为常数。
\begin{enumerate}
...
\item 经过时间 $T$ 后波函数为
\[
\Psi(x,T)=B\left(1+2\sqrt{\frac{m\omega}{\hbar}}x\right)^2 \ee^{-m\omega x^2/(2\hbar)},
\]
...
\[
-\frac{\hbar^2}{2m}\psi_i''+V\psi_i=E\psi_i\quad(\ii=1,2).
\]
...
归一化后
\[
\psi_n(x)=\frac1{\sqrt L}\ee^{\ii2\pi nx/L},
\]
能量为
\[
...
\]
阱外 $x>a$ 为 $B\ee^{-\kappa(x-a)}$。在 $x=a$ 处连续并导数连续,得到
\[
k\cot(ka)=-\kappa.
\]
...
\exp\!\left\{-\frac{\ii}{\hbar}\left[\frac{\hbar\omega t}{2}+m\omega x x_0\sin\omega t-\frac{m\omega x_0^2}{4}\sin(2\omega t)\right]\right\}
\]
...
\exp\!\left[-\frac{m\alpha}{\hbar^2}|x-vt|+\frac{\ii}{\hbar}\left((E+\frac12mv^2)t-mvx\right)\right],
\]
...
\[
\psi_0(x)=\sqrt\beta \ee^{-\beta|x|},
\qquad
E=-\frac{m\alpha^2}{2\hbar^2}.
\]
运动势阱的解是静止束缚态的伽利略变换。对向 $+x$ 方向速度为 $v$ 的势阱,可写作
\[
\Psi(x,t)=\sqrt\beta \ee^{-\beta|x-vt|}
\exp\left[\frac{\ii}{\hbar}\left(mvx-\left(E+\frac12mv^2\right)t\right)\right],
\]
与题中公式只差等价的相位符号约定。利用
\[
\frac{\dd^2}{\dd x^2}\ee^{-\beta|x-vt|}
=\beta^2\ee^{-\beta|x-vt|}-2\beta\delta(x-vt)\ee^{-\beta|x-vt|}
\]
即可直接代入验证含时薛定谔方程。
...
\exp\left[-\frac{\ii}{\hbar}mgt\left(x+\frac{g t^2}{6}\right)\right]
\]
满足粒子在引力势 $V(x)=mgx$ 中的含时薛定谔方程,其中 $\Psi_0(x,t)$ 为自由高斯波包(式 (2.111))。求作为时间函数的 $\langle x\rangle$,并讨论结果。

\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.111):自由高斯波包常用形式:若 $\Psi(x,0)=(2a/\pi)^{1/4}\ee^{-ax^2}$,则 $\Psi_0(x,t)=(2a/\pi)^{1/4}(1+2\ii\hbar at/m)^{-1/2}\exp[-ax^2/(1+2\ii\hbar at/m)]$。}

\begin{solution}
将给定形式写成
\[
\Psi(x,t)=\Psi_0(X,t)\ee^{-\ii\Phi(x,t)/\hbar},
\qquad
X=x+\frac12gt^2,
\qquad
...
\item 证明对任意正能量,
\[
\psi_k(x)=A\frac{\ii k-a\tanh(ax)}{\ii k+a}\ee^{\ii kx},\qquad k=\sqrt{2mE}/\hbar,
\]
...
(3) 把
\[
\psi_k=A\frac{\ii k-a\tanh(ax)}{\ii k+a}\ee^{\ii kx}
\]
代入定态方程可直接验证,对应 $E=\hbar^2k^2/(2m)$。当 $x\to-\infty$,$\tanh(ax)\to-1$,故
\[
\psi_k\sim A\ee^{\ii kx},
\]
没有反射波;当 $x\to+\infty$,$\tanh(ax)\to1$,
\[
\psi_k\sim A\frac{\ii k-a}{\ii k+a}\ee^{\ii kx}.
\]
透射振幅的模为 $1$,所以
\[
R=0,
...
\[
\psi_I(x)=A\ee^{\ii kx}+B\ee^{-\ii kx},\qquad
\psi_{III}(x)=F\ee^{\ii kx}+G\ee^{-\ii kx}.
\]
...
可取
\[
t=\frac{\ee^{-2\ii ka}}{\cos(2\ell a)-\ii\dfrac{k^2+\ell^2}{2k\ell}\sin(2\ell a)},
\]
\[
r=\frac{\ii\dfrac{k^2-\ell^2}{2k\ell}\sin(2\ell a)\ee^{-2\ii ka}}
...
\[
M_a=\begin{pmatrix}
1+\ii\gamma&\ii\gamma \ee^{-2\ii ka}\\
-\ii\gamma \ee^{2\ii ka}&1-\ii\gamma
\end{pmatrix}.
\]
...
玻尔兹曼分布
\[
P(n)=\frac{1}{Z}\ee^{-\beta E_n},\qquad Z=\sum_n \ee^{-\beta E_n},
\]
...
\[
C=3k_B\left(\frac{\Theta_E}{T}\right)^2\frac{\ee^{\Theta_E/T}}{(\ee^{\Theta_E/T}-1)^2},
\]
...
(1) 
\[
\bar E=\sum_nE_n\frac{\ee^{-\beta E_n}}{Z}
=\frac1Z\sum_nE_n\ee^{-\beta E_n}.
\]
而
\[
\frac{\partial Z}{\partial\beta}=\sum_n(-E_n)\ee^{-\beta E_n},
\]
故
\[
...
(2) 对谐振子,
\[
Z=\sum_{n=0}^{\infty}\ee^{-\beta(n+1/2)\hbar\omega}
...
=\frac{\hbar\omega}{2}+\frac{\hbar\omega}{\ee^{\beta\hbar\omega}-1}.
\]
...
\frac{\ee^{\Theta_E/T}}{(\ee^{\Theta_E/T}-1)^2}.
\]
每个原子有三个振动自由度,所以
\[
