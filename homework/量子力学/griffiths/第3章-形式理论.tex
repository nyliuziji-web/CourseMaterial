...
周期归一化本征函数为
\[
f_n(\phi)=\frac1{\sqrt{2\pi}}\ee^{\ii n\phi},
...
\[
u(x)=\ee^x+\ee^{-x}=2\cosh x,
\qquad
v(x)=\ee^x-\ee^{-x}=2\sinh x.
\]
一个为偶函数,一个为奇函数,因此
\[
...
=\frac1{2\pi}\int_0^{2\pi}\ee^{\ii(n-m)\phi}\dd\phi=0,
\]
所以不同本征值的本征函数正交。
...
\exp\left(-\frac{p^2}{2m\hbar\omega}\right)\ee^{-\ii\omega t/2}.
\]
因此
\[
...
\[
P_{\rm out}=\frac1{\sqrt\pi}\int_{|u|>1}\ee^{-u^2}\dd u
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.101):自由粒子波包的动量展开:}\[\Psi(x,t)=(2\pi\hbar)^{-1/2}\int \Phi(p,0)\ee^{\ii(px-p^2t/2m)/\hbar}\dd p.\]
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.147):动量空间变换:$\Phi(p,t)=(2\pi\hbar)^{-1/2}\int \ee^{-\ii px/\hbar}\Psi(x,t)\dd x$。}

\begin{solution}
采用傅里叶表示
\[
\Psi(x,t)=\frac1{\sqrt{2\pi\hbar}}\int \Phi(p,t)\ee^{\ii px/\hbar}\dd p.
\]
则
\[
...
利用
\[
x \ee^{\ii px/\hbar}=-\ii\hbar\frac{\partial}{\partial p}\ee^{\ii px/\hbar},
\]
得
\[
\langle x\rangle
=\frac1{2\pi\hbar}\int\dd x\int\dd p'\int\dd p\,
\Phi^*(p')\ee^{-\ii p'x/\hbar}\left[-\ii\hbar\frac{\partial}{\partial p}\ee^{\ii px/\hbar}\right]\Phi(p).
\]
对 $p$ 分部积分,并用
\[
\int \ee^{\ii(p-p')x/\hbar}\dd x=2\pi\hbar\delta(p-p'),
\]
即得
\[
...
归一化常数可取
\[
A=(2\pi\sigma_x^2)^{-1/4}\ee^{\ii\alpha},
\]
...
\item 式 (2.21):若 $\Psi=\sum_n c_n\psi_n \ee^{-\ii E_nt/\hbar}$,则 $\langle H\rangle=\sum_n |c_n|^2E_n$。
\end{enumerate}
...
对习题 2.42 的运动高斯包,初态
\[
\Psi(x,0)=\left(\frac{2a}{\pi}\right)^{1/4}\ee^{-ax^2}\ee^{\ii\ell x}
\]
具有
\[
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (3.100):算符指数的级数定义:$\ee^{\hat Q}=\sum_{n=0}^{\infty}\hat Q^n/n!$。}
...
(2) 若用幂级数定义
\[
\ee^{\hat Q}=\sum_{k=0}^{\infty}\frac{\hat Q^k}{k!},
\]
则由谱分解得
\[
...
因此
\[
\ee^{\hat Q}=\sum_n\left(\sum_{k=0}^{\infty}\frac{q_n^k}{k!}\right)
\ket{e_n}\bra{e_n}
=\sum_n \ee^{q_n}\ket{e_n}\bra{e_n},
\]
与式 (3.104) 中 $f(q)=\ee^q$ 的定义一致。
\end{solution}
\end{question}
...
\item 证明
\[
[\mathrm \ee^A,B]=\mathrm \ee^A C,
\]
其中 $\mathrm \ee^A$ 由幂级数定义。
\item 推导贝克-坎贝尔-豪斯多夫公式(Baker-Campbell-Hausdorff formula):
\[
\mathrm \ee^{A+B}=\mathrm \ee^A\mathrm \ee^B\mathrm \ee^{-C/2}.
\]
提示:定义函数
\[
F(\lambda)=\mathrm \ee^{\lambda(A+B)},\qquad
G(\lambda)=\mathrm \ee^{\lambda A}\mathrm \ee^{\lambda B}\mathrm \ee^{-\lambda^2C/2}.
\]
...
(2) 由幂级数,
\[
[\ee^A,B]=\sum_{n=0}^{\infty}\frac1{n!}[A^n,B]
...
(3) 令
\[
F(\lambda)=\ee^{\lambda(A+B)},
\qquad
G(\lambda)=\ee^{\lambda A}\ee^{\lambda B}\ee^{-\lambda^2C/2}.
\]
显然 $F(0)=G(0)=1$。又因为 $C$ 与 $A,B$ 都对易,且
\[
[\ee^{\lambda A},B]=\lambda \ee^{\lambda A}C,
\]
直接求导可得
\[
...
所以
\[
\boxed{c_n(t)=c_n(0)\ee^{-\ii E_nt/\hbar}.}
\]
...
动量空间波函数为
\[
\Phi_n(p,t)=\frac{\ee^{-\ii E_nt/\hbar}}{\sqrt{\pi a\hbar}}
\frac{(n\pi/a)\left[1-(-1)^n\ee^{-\ii ap/\hbar}\right]}
{(n\pi/a)^2-(p/\hbar)^2}.
\]
该式在表面奇点处取极限即可。
...
\frac{(n\pi/a)^2\left|1-(-1)^n\ee^{-\ii ap/\hbar}\right|^2}
...
\psi(x,0)=
\begin{cases}
\displaystyle \frac{1}{\sqrt{2n\lambda}}\,\mathrm \ee^{2\pi \ii x/\lambda},& -n\lambda<x<n\lambda,\\[0.4em]
0,& \text{其他地方},
\end{cases}
\]
...
(3) 利用
\[
\int_{-\infty}^{\infty}\frac{\ee^{-\ii px/\hbar}}{x^2+a^2}\dd x
=\frac\pi a \ee^{-a|p|/\hbar},
\]
得
\[
\boxed{\Phi(p,0)=\sqrt{\frac a\hbar}\,\ee^{-a|p|/\hbar}.}
\]
归一化验证:
\[
\int_{-\infty}^{\infty}|\Phi|^2\dd p
=\frac a\hbar\int_{-\infty}^{\infty}\ee^{-2a|p|/\hbar}\dd p=1.
\]
...
并且
\[
\langle p^2\rangle=\frac a\hbar 2\int_0^\infty p^2\ee^{-2ap/\hbar}\dd p
=\frac{\hbar^2}{2a^2}.
\]
故
\[
...
随时间演化为
\[
\psi(t)=\frac1{\sqrt2}\left(\psi_1\ee^{-\ii E_1t/\hbar}
+\psi_2\ee^{-\ii E_2t/\hbar}\right).
\]
与初态的重叠为
\[
\left\langle \psi(0)|\psi(t) \right\rangle
=\frac12\left(\ee^{-\ii E_1t/\hbar}+\ee^{-\ii E_2t/\hbar}\right)
=\ee^{-\ii(E_1+E_2)t/(2\hbar)}
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.67--2.68):谐振子定态:}\[\begin{aligned}E_n&=(n+1/2)\hbar\omega,\\\Psi(x,t)&=\sum_n c_n\psi_n(x)\ee^{-\ii E_nt/\hbar}.\end{aligned}\]
...
谐振子势中粒子的波函数一般可写为
\[
\Psi(x,t)=\sum_n c_n\psi_n(x)\mathrm \ee^{-\ii E_nt/\hbar}.
\]
证明位置期望值为
\[
...
其中实常数 $C$ 和 $\phi$ 由下式给出:
\[
C\ee^{-\ii\phi}=\left(\frac{2\hbar}{m\omega}\right)^{1/2}
...
因此
\[
C\ee^{-\ii\phi}=-\frac{28}{25}\sqrt{\frac{\hbar}{m\omega}},
\]
可取
\[
...
\[
\ket\psi=\frac1{\sqrt2}\ket0+\frac{\ee^{\ii\theta}}{\sqrt2}\ket1.
\]
动量算符为
\[
p=\ii\sqrt{\frac{m\hbar\omega}{2}}(a_+-a_-).
...
\[
\boxed{\Psi(x,t)=\frac1{\sqrt2}\psi_0(x)\ee^{-\ii\omega t/2}
+\frac{\ii}{\sqrt2}\psi_1(x)\ee^{-3\ii\omega t/2}.}
\]
...
\item 现在,引入时间因子 $\ket n\to \mathrm \ee^{-\ii E_nt/\hbar}\ket n$,证明 $\ket{\alpha(t)}$ 仍然是 $a_-$ 的本征态,但本征值是随时间变化的:
\[
\alpha(t)=\mathrm \ee^{-\ii\omega t}\alpha.
\]
因此一个相干态将维持相干,并继续取不确定度乘积的最小值。
\item 基于 (a)、(b) 和 (e) 中得到的结果,求出作为时间函数的 $\langle x\rangle$ 和 $\sigma_x$。如果把复数 $\alpha$ 写成如下形式将会大有帮助:
\[
\alpha=C\sqrt{\frac{m\omega}{2\hbar}}\,\mathrm \ee^{\ii\phi},
\]
...
可取
\[
\boxed{c_0=\ee^{-|\alpha|^2/2}.}
\]

含时演化为
\[
\ket{\alpha(t)}=\ee^{-|\alpha|^2/2}
\sum_{n=0}^{\infty}\frac{\alpha^n}{\sqrt{n!}}\ee^{-\ii(n+1/2)\omega t}\ket n.
\]
略去整体相位 $\ee^{-\ii\omega t/2}$ 后,系数中 $\alpha$ 被替换为
\[
\boxed{\alpha(t)=\alpha \ee^{-\ii\omega t}.}
\]

若
\[
\alpha=C\sqrt{\frac{m\omega}{2\hbar}}\ee^{\ii\phi},
\]
则
\[
\boxed{\langle x\rangle=C\cos(\omega t-\phi)},
...
则
\[
\boxed{\ket{s(t)}=\ee^{-\ii ct/\hbar}\begin{pmatrix}0\\1\\0\end{pmatrix}.}
\]
...
则
\[
\ket{s(t)}=\frac1{\sqrt2}\left(\ee^{-\ii(a+b)t/\hbar}\ket+
+\ee^{-\ii(a-b)t/\hbar}\ket-\right).
\]
化回原基,
\[
\boxed{\ket{s(t)}=\ee^{-\ii at/\hbar}
\begin{pmatrix}
...
测量 $A$ 的几率为
\[
P_A(\lambda)=\frac12\left|c_1\ee^{-\ii\omega t}+c_2\ee^{-2\ii\omega t}\right|^2,
\]
\[
P_A(-\lambda)=\frac12\left|c_1\ee^{-\ii\omega t}-c_2\ee^{-2\ii\omega t}\right|^2,
\qquad
