...
\item 假设波函数
\[
\psi(r,\theta,\phi)=A \ee^{-r/a},
\]
其中 $A$ 和 $a$ 都为常数。求能量 $E$ 和势场 $V(r)$,当 $r\to\infty$ 时,$V(r)\to0$。
\item 设 $V(0)=0$,对波函数
\[
\psi(r,\theta,\phi)=A \ee^{-r^2/a^2}
\]
重复上述计算。
\end{enumerate}
\begin{solution}
...
\item 式 (4.32):球谐函数:$Y_\ell^m(\theta,\phi)=\epsilon\sqrt{(2\ell+1)(\ell-|m|)!/[4\pi(\ell+|m|)!]}\,\ee^{\ii m\phi}P_\ell^m(\cos\theta)$。
\end{enumerate}
...
由
\[
Y_\ell^m=N_{\ell m}\ee^{\ii m\phi}P_\ell^m(\cos\theta),
\]
可得
\[
...
\]
\[
Y_2^1=-\sqrt{\frac{15}{8\pi}}\sin\theta\cos\theta\,\ee^{\ii\phi}.
\]
归一化直接由
\[
\int_0^{2\pi}\!\dd\phi\int_0^\pi |Y|^2\sin\theta\dd\theta=1
\]
验证。二者正交,因为 $\phi$ 积分给出 $\int \ee^{\ii\phi}\dd\phi=0$;同 $m$ 不同 $\ell$ 的正交性则来自缔合勒让德函数的正交性。
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (4.32):球谐函数:}\[Y_\ell^m(\theta,\phi)=\epsilon\sqrt{(2\ell+1)(\ell-|m|)!/[4\pi(\ell+|m|)!]}\,\ee^{\ii m\phi}P_\ell^m(\cos\theta).\]

\begin{solution}

按定义
\[
Y_\ell^m=N_{\ell m}P_\ell^m(\cos\theta)\ee^{\ii m\phi},
\quad
Y_\ell^{-m}=N_{\ell,-m}P_\ell^{-m}(\cos\theta)\ee^{-\ii m\phi}.
\]
...
因此
\[
Y_3^2=\sqrt{\frac{105}{32\pi}}\sin^2\theta\cos\theta\,\ee^{2\ii\phi},
\]
\[
Y_3^3=-\sqrt{\frac{35}{64\pi}}\sin^3\theta\,\ee^{3\ii\phi}.
\]
...
\qquad u(r)=B\ee^{-\kappa r}\quad (r>a).
\]
在 $r=a$ 连续且导数连续给
\[
k\cot ka=-\kappa,
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (4.76):氢径向级数递推:设 $u(\rho)=\rho^{\ell+1}\ee^{-\rho/2}\sum_j c_j\rho^j$,则 $c_{j+1}$ 由 $c_j$ 递推,终止条件给出主量子数。}

\begin{solution}

氢原子径向函数标准结果为
\[
R_{30}=\frac{2}{81\sqrt3a^{3/2}}\left(27-18\frac{r}{a}+2\frac{r^2}{a^2}\right)\ee^{-r/(3a)},
\]
\[
R_{31}=\frac{8}{27\sqrt6a^{3/2}}\left(1-\frac{r}{6a}\right)\frac{r}{a}\ee^{-r/(3a)},
\]
\[
R_{32}=\frac{4}{81\sqrt{30}a^{3/2}}\left(\frac{r}{a}\right)^2\ee^{-r/(3a)}.
\]
它们由递推多项式得到,归一化条件为
\[
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (4.82--4.83):}\[\begin{aligned}R_{20}&=(2\sqrt{2a^3})^{-1}(2-r/a)\ee^{-r/(2a)},\\R_{21}&=(2\sqrt{6a^3})^{-1}(r/a)\ee^{-r/(2a)}.\end{aligned}\]

\begin{solution}

归一化后
\[
R_{20}=\frac{1}{2\sqrt2a^{3/2}}\left(2-\frac{r}{a}\right)\ee^{-r/(2a)},
\]
故
\[
\psi_{200}=\frac{1}{4\sqrt{2\pi}a^{3/2}}\left(2-\frac{r}{a}\right)\ee^{-r/(2a)}.
\]
又
\[
R_{21}=\frac{1}{2\sqrt6a^{3/2}}\frac{r}{a}\ee^{-r/(2a)}.
\]
配合球谐函数得
\[
\psi_{210}=\frac{1}{4\sqrt{2\pi}a^{3/2}}\frac{r}{a}\ee^{-r/(2a)}\cos\theta,
\]
\[
\psi_{211}=-\frac{1}{8\sqrt\pi a^{3/2}}\frac{r}{a}\ee^{-r/(2a)}\sin\theta \ee^{\ii\phi},
\quad
\psi_{21,-1}=\frac{1}{8\sqrt\pi a^{3/2}}\frac{r}{a}\ee^{-r/(2a)}\sin\theta \ee^{-\ii\phi}.
\]
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (4.86--4.88):氢径向函数:$R_{n\ell}=N_{n\ell}\rho^\ell \ee^{-\rho/2}L_{n-\ell-1}^{2\ell+1}(\rho)$,$\rho=2r/(na)$;$L$ 为缔合 Laguerre 多项式。
\item 式 (4.76):氢径向级数递推:设 $u(\rho)=\rho^{\ell+1}\ee^{-\rho/2}\sum_j c_j\rho^j$,则 $c_{j+1}$ 由 $c_j$ 递推,终止条件给出主量子数。
\end{enumerate}
...
(1) 按公式
\[
L_q(x)=\ee^x\frac{\dd^q}{\dd x^q}(\ee^{-x}x^q)
\]
得
\[
L_0=1,
...
基态
\[
\psi_{100}=\frac{1}{\sqrt{\pi a^3}}\ee^{-r/a}.
\]
径向概率密度为 $4r^2a^{-3}\ee^{-2r/a}$。因此
\[
...
\[
P(r)\dd r=4\pi r^2|\psi_{100}|^2\dd r=\frac{4r^2}{a^3}\ee^{-2r/a}\dd r.
\]
最大值由
\[
\frac{\dd}{\dd r}\left(r^2\ee^{-2r/a}\right)=0
\]
给出,故
\[
r=a.
...
\[
\Psi(t)=\ee^{-\ii E_2t/\hbar}\frac{\psi_{211}+\psi_{21,-1}}{\sqrt2}.
\]
利用球谐函数组合可写为实的 $p_x$ 型轨道
\[
\Psi(t)=-\frac{1}{4\sqrt{\pi}a^{3/2}}\frac{r}{a}\ee^{-r/(2a)}\sin\theta\cos\phi\ \ee^{-\ii E_2t/\hbar}
\]
(整体相位或符号无物理意义)。
...
\]
\item 利用这些结果直接从式 (4.96) 证明 $[L_x,L_y]=\ii\hbar L_z$。
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (4.10):正则对易关系:$[x_i,p_j]=\ii\hbar\delta_{\ii j}$,$[x_i,x_j]=[p_i,p_j]=0$。
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (4.129--4.132):微分形式:$L_z=-\ii\hbar\partial_\phi$,$L_\pm=\hbar \ee^{\pm\ii\phi}(\pm\partial_\theta+\ii\cot\theta\partial_\phi)$。
...
\[
L_\pm=\hbar \ee^{\pm\ii\phi}\left(\pm\frac{\partial}{\partial\theta}+\ii\cot\theta\frac{\partial}{\partial\phi}\right).
\]
这就是式 (4.131)。

再把 $L_\pm$ 作用到 $Y_\ell^m=\Theta(\theta)\ee^{\ii m\phi}$ 上,利用角方程把结果化成与 $Y_\ell^{m\pm1}$ 成正比的函数,并由习题 4.21 的归一化系数得到
\[
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (4.129--4.132):微分形式:$L_z=-\ii\hbar\partial_\phi$,$L_\pm=\hbar \ee^{\pm\ii\phi}(\pm\partial_\theta+\ii\cot\theta\partial_\phi)$。}
...
(2) 设 $Y_1^1=f(\theta)\ee^{\ii\phi}$。代入
\[
L_-Y_1^1=\hbar \ee^{-\ii\phi}\left(-\partial_\theta+\ii\cot\theta\partial_\phi\right)f \ee^{\ii\phi}
=-\hbar(f'+f\cot\theta).
\]
令其等于 $\hbar\sqrt2Y_1^0=\hbar\sqrt{3/(2\pi)}\cos\theta$,解得 $f=-C\sin\theta$。归一化给
\[
Y_1^1=-\sqrt{\frac{3}{8\pi}}\sin\theta \ee^{\ii\phi}.
\]
...
在习题 4.4 中,你证明了
\[
Y_2^1(\theta,\phi)=-\sqrt{\frac{15}{8\pi}}\sin\theta\cos\theta\,\ee^{\ii\phi}.
\]
应用升算符求出 $Y_2^2(\theta,\phi)$,利用式 (4.121) 对其归一化。

\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (4.120--4.121):最高权球谐函数形如 $Y_\ell^\ell=K_\ell^\ell \ee^{\ii\ell\phi}\sin^\ell\theta$;归一化由 $\int|Y|^2\dd\Omega=1$ 决定。}
...
代入 $L_+$ 微分表达式,得
\[
Y_2^2=\sqrt{\frac{15}{32\pi}}\sin^2\theta \ee^{2\ii\phi}.
\]
...
\[
\sigma_i\sigma_j=\delta_{\ii j}I+\ii\sum_k\epsilon_{\ii jk}\sigma_k.
\]
\end{enumerate}
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (4.145--4.148):$S_i=(\hbar/2)\sigma_i$,其中 $\sigma_x=\begin{pmatrix}0&1\\1&0\end{pmatrix}$、$\sigma_y=\begin{pmatrix}0&-\ii\\\ii&0\end{pmatrix}$、$\sigma_z=\begin{pmatrix}1&0\\0&-1\end{pmatrix}$,且 $\sigma_i\sigma_j=\delta_{\ii j}I+\ii\epsilon_{\ii jk}\sigma_k$。
...
\[
\hat{\mathbf n}=\sin\theta\cos\phi\,\hat{\mathbf \ii}+\sin\theta\sin\phi\,\hat{\mathbf j}+\cos\theta\,\hat{\mathbf k}.
\]
...
\cos\theta&\sin\theta \ee^{-\ii\phi}\\
\sin\theta \ee^{\ii\phi}&-\cos\theta
\end{pmatrix}.
\]
本征值为 $\pm\hbar/2$。可取归一化本征旋量
\[
\chi_+^{(n)}=\binom{\cos(\theta/2)}{\ee^{\ii\phi}\sin(\theta/2)},
\]
\[
\chi_-^{(n)}=\binom{-\ee^{-\ii\phi}\sin(\theta/2)}{\cos(\theta/2)}.
\]
整体相位任意。
...
\[
\chi(t)=\binom{a \ee^{-\ii\omega t/2}}{b \ee^{\ii\omega t/2}}
\]
...
(2) 方程解为
\[
\chi(t)=\frac1{\sqrt2}\binom{\ee^{\ii\alpha\sin\omega t}}{\ee^{-\ii\alpha\sin\omega t}},
\qquad
...
假设
\[
\mathbf A=B y\,\hat{\mathbf \ii},\qquad \varphi=Kz^2,
\]
其中 $B$ 和 $K$ 为常数。
\begin{enumerate}
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (4.190--4.199):电磁场中最小耦合 Hamiltonian,在库仑规范下为}\[\begin{aligned}H={}&(\mathbf p-q\mathbf A)^2/(2m)+q\varphi,\\&p^2/(2m)-(q/2m)(\mathbf p\cdot\mathbf A+\mathbf A\cdot\mathbf p)+q^2A^2/(2m)+q\varphi.\end{aligned}\]
...
\[
(-\ii\hbar\nabla-q\mathbf A')\psi'
=\ee^{\ii q\Lambda/\hbar}(-\ii\hbar\nabla-q\mathbf A)\psi.
\]
时间导数给
\[
\ii\hbar\partial_t\psi'=\ee^{\ii q\Lambda/\hbar}(\ii\hbar\partial_t\psi-q\partial_t\Lambda\psi).
\]
...
(1) 势能可写成三个一维谐振子的和:
\[
H=\sum_{\ii=x,y,z}\left(\frac{p_i^2}{2m}+\frac12m\omega^2x_i^2\right).
\]
所以
\[
...
(2) 对定态 $\psi_{211}=R_{21}Y_1^1$,相位为 $\ee^{\ii\phi}$,只有方位角流:
\[
...
\[
\phi(\mathbf p)=\frac{1}{(2\pi\hbar)^{3/2}}\int \ee^{-\ii\mathbf p\cdot\mathbf r/\hbar}\psi(\mathbf r)\,\mathrm d^3r.
\]
\begin{enumerate}
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (3.54):三维动量空间波函数:$\Phi(\mathbf p)=(2\pi\hbar)^{-3/2}\int \ee^{-\ii\mathbf p\cdot\mathbf r/\hbar}\Psi(\mathbf r)\dd^3r$。
\item 式 (4.80):氢基态:$\psi_{100}(r)=\pi^{-1/2}a^{-3/2}\ee^{-r/a}$。
\end{enumerate}
...
\[
\int_0^\infty r \ee^{-r/a}\sin(pr/\hbar)\dd r=\frac{2(p/\hbar)(1/a)}{[(1/a)^2+(p/\hbar)^2]^2}
\]
得
\[
...
二维圆阱的 $m=0$ 束缚态可写为
\[
R_{\text{in}}=AJ_0(kr),\quad r<a;
\qquad
R_{\text{out}}=BK_0(\kappa r),\quad r>a,
\]
其中 $K_0$ 在无穷远衰减。连续性给
\[
...
得
\[
\psi_{321}=-\frac{1}{81\sqrt{\pi}a^{3/2}}\left(\frac{r}{a}\right)^2\ee^{-r/(3a)}\sin\theta\cos\theta \ee^{\ii\phi}.
\]
径向和角向分别归一化,故总体归一化成立。
...
归一化给
\[
R_{43}=\frac{1}{96\sqrt{35}a^{3/2}}\left(\frac{r}{a}\right)^3\ee^{-r/(4a)}.
\]
因此
\[
\psi_{433}=-\frac{1}{768\sqrt\pi a^{3/2}}\left(\frac{r}{a}\right)^3\ee^{-r/(4a)}\sin^3\theta \ee^{3\ii\phi}.
\]
氢原子平均半径
\[
...
精确概率为
\[
P=\int_0^b4\pi r^2\frac{1}{\pi a^3}\ee^{-2r/a}\dd r
=1-\ee^{-\epsilon}\left(1+\epsilon+\frac{\epsilon^2}{2}\right),
...
\[
R_{n,n-1}=N_n r^{n-1}\ee^{-r/(na)},
\]
并通过直接积分求出归一化常数 $N_n$。
\item 对于形如 $r^s \ee^{-r/(na)}$ 的径向概率分布,求 $\langle r\rangle$ 和 $\langle r^2\rangle$。
...
\[
R_{n,n-1}=N_nr^{n-1}\ee^{-r/(na)}.
\]
归一化
\[
1=N_n^2\int_0^\infty r^{2n}\ee^{-2r/(na)}\dd r
...
电子处在如下自旋态上:
\[
\chi=A\begin{pmatrix}1+\ii\\2\end{pmatrix}.
\]
\begin{enumerate}
\item 归一化 $\chi$。
...
\[
P_x(+)=\frac12|a+b|^2=\frac12\cdot\frac{10}{6}=\frac56,
\quad P_x(-)=\frac12|a-b|^2=\frac12\cdot\frac{2}{6}=\frac16,
\]
\[
\langle S_x\rangle=\frac\hbar2(P_x(+)-P_x(-))=\frac\hbar3.
\]

$S_y$ 的本征态为 $(1,\pm\ii)^T/\sqrt2$,
\[
P_y(+)=\frac12|a-\ii b|^2=\frac12\cdot\frac{2}{6}=\frac16,
\quad P_y(-)=\frac12|a+\ii b|^2=\frac12\cdot\frac{10}{6}=\frac56,
\]
\[
\langle S_y\rangle=-\frac\hbar3.
\]
...
单态具有旋转不变性,且
\[
\langle S_i^{(1)}S_j^{(2)}\rangle=-\frac{\hbar^2}{4}\delta_{\ii j}.
\]
因此
\[
\langle S_a^{(1)}S_b^{(2)}\rangle
=\sum_{\ii j}a_i b_j\langle S_i^{(1)}S_j^{(2)}\rangle
...
求球谐函数归一化因子。设
\[
Y_\ell^m(\theta,\phi)=K_\ell^m \ee^{\ii m\phi}P_\ell^m(\cos\theta).
\]
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (4.120--4.121):最高权球谐函数形如 $Y_\ell^\ell=K_\ell^\ell \ee^{\ii\ell\phi}\sin^\ell\theta$;归一化由 $\int|Y|^2\dd\Omega=1$ 决定。
\item 式 (4.129--4.132):微分形式:$L_z=-\ii\hbar\partial_\phi$,$L_\pm=\hbar \ee^{\pm\ii\phi}(\pm\partial_\theta+\ii\cot\theta\partial_\phi)$。
\end{enumerate}
...
设
\[
Y_\ell^m=K_\ell^m \ee^{\ii m\phi}P_\ell^m(\cos\theta).
\]
由
\[
...
(1) 对三角形,
\[
S^2=\sum_iS_i^2+2\sum_{\ii<j}S_i\cdot S_j.
\]
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (4.155):沿 $\hat{\mathbf n}=(\sin\theta\cos\phi,\sin\theta\sin\phi,\cos\theta)$ 自旋向上旋量:$\chi_+=\binom{\cos(\theta/2)}{\ee^{\ii\phi}\sin(\theta/2)}$。}
...
\]
它们分别正比于 $z \ee^{-r/2a}$、$x \ee^{-r/2a}$、$y \ee^{-r/2a}$。
...
\item 从例题 4.3 知道,自旋在均匀磁场 $B_0\hat{\mathbf k}$ 中进动。把这部分从解中提取为相位因子,写成 $\psi_\pm(\mathbf r,t)=\ee^{\mp \ii\gamma B_0t/2}\varphi_\pm(\mathbf r,t)$,并求 $\varphi_\pm$ 的方程。
...
(2) 提取 $\ee^{\mp\ii\gamma B_0t/2}$ 后,耦合项带有快速相位 $\ee^{\pm\ii\gamma B_0t}$。
...
\[
\Psi_n(\phi,t)=\frac{1}{\sqrt{2\pi}}\ee^{\ii n\phi}\ee^{-\ii f_n(t)}
\]
...
把
\[
\Psi_n=\frac{1}{\sqrt{2\pi}}\ee^{\ii n\phi}\ee^{-\ii f_n(t)}
\]
