...
\]
其中,$\Phi(\ii,j)$ 是 $n=1$ 态和自旋单态组合下两个粒子的波函数,
\[
\Phi(\ii,j)=\psi_1(x_i)\psi_1(x_j)\frac{|\uparrow_i\downarrow_j\rangle-|\downarrow_i\uparrow_j\rangle}{\sqrt2},
\]
其中,$\phi(\ii)$ 是第 $i$ 个粒子处于 $n=2$ 且自旋向上的态:$\phi(\ii)=\psi_2(x_i)|\uparrow_i\rangle$。注意:$\Phi(\ii,j)$ 在 $\ii\leftrightarrow j$ 交换下是反对称的,检查 $\Phi(1,2,3)$ 在交换作用下($1\leftrightarrow2$,$2\leftrightarrow3$,$3\leftrightarrow1$)是反对称的。
\end{enumerate}
\begin{solution}
...
其中 $q$ 是正奇数,且
\[
z_j=\sqrt{\frac{eB}{2\hbar c}}(x_j+\ii y_j).
\]
...
对试探基态
\[
\psi_0=\frac{Z^3}{\pi a^3}\ee^{-Z(r_1+r_2)/a}
\]
(氦 $Z=2$),标准积分给
\[
...
基态项符号为 ${}^{2S+1}L_J={}^{5}I_{8}$。由 $2S+1=5$ 得 $S=2$；字母 $I$ 对应 $L=6$；下标 $J=8$。因此
\[
S=2,\qquad L=6,\qquad J=8.
\]
镝（$Z=66$）的基态电子组态为 $[\mathrm{Xe}]\,4f^{10}6s^2$。$4f$ 壳层可容纳 $14$ 个电子，$10$ 个电子超过半满（$7$），按洪德第三定则取 $J=L+S=8$，与项符号一致。
...
\[
\psi(x)=C\left\{\sin(kx)+\ee^{-\ii qa}\sin\bigl[k(a-x)\bigr]\right\},\qquad 0\le x\le a.
\]
(归一化常数 $C$ 的具体值不需要费心求出。)
...
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:}
\begin{enumerate}[leftmargin=2.2em,itemsep=0.12em,topsep=0.12em,parsep=0pt]
\item 式 (5.66):Bloch 条件:$\psi(x+a)=\ee^{\ii qa}\psi(x)$。
...
\[
\psi(x)=C\{\sin kx+\ee^{-\ii qa}\sin[k(a-x)]\}.
\]
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (5.71):Kronig--Penney 色散关系:}\[\begin{aligned}\cos(qa)&=\cos(ka)+(m\alpha/\hbar^2k)\sin(ka),\\k&=\sqrt{2mE}/\hbar.\end{aligned}\]
...
\[
\hat a_{k\pm}=\frac{1}{\sqrt N}\sum_{j=1}^{N}\ee^{\pm i2\pi jk/N}\left[\sqrt{\frac{m\omega_k}{2\hbar}}x_j\mp\sqrt{\frac{\hbar}{2m\omega_k}}\frac{\partial}{\partial x_j}\right],
\]
...
\[
\frac{1}{N}\sum_{j=1}^{N}\ee^{i2\pi j(k-k')/N}=\delta_{k',k},
\]
\[
\frac{1}{N}\sum_{j=1}^{N}\ee^{i2\pi j(k+k')/N}=\delta_{k',N-k}.
\]
提示:对几何级数求和。
\item 推导阶梯算符的对易关系:
\[
...
\item 利用上式,证明
\[
x_j=R+\frac{1}{\sqrt N}\sum_{k=1}^{N-1}\sqrt{\frac{\hbar}{2m\omega_k}}(\hat a_{k-}+\hat a_{N-k,+})\ee^{i2\pi jk/N},
\]
\[
\frac{\partial}{\partial x_j}=\frac{1}{N}\frac{\partial}{\partial R}+\frac{1}{\sqrt N}\sum_{k=1}^{N-1}\sqrt{\frac{m\omega_k}{2\hbar}}(\hat a_{k-}-\hat a_{N-k,+})\ee^{i2\pi jk/N},
\]
...
(3) 这是离散傅里叶变换的反变换;把定义式乘 $\ee^{\mp\ii2\pi jk/N}$ 求和并用正交关系即可得到题中 $x_j$ 和 $\partial_{x_j}$。
...
这样可以将波函数表示为行波,
\[
\psi=\frac{1}{\sqrt{l_xl_yl_z}}\ee^{\ii\mathbf k\cdot\mathbf r}=\frac{1}{\sqrt{l_xl_yl_z}}\ee^{\ii(k_xx+k_yy+k_zz)},
\]
...
周期边界条件要求
\[
\ee^{\ii k_xl_x}=\ee^{\ii k_yl_y}=\ee^{\ii k_zl_z}=1,
\]
所以
\[
k_xl_x=2\pi n_x,
