...
\[
\begin{aligned}E^{(1)}={}&\iint \Psi^*(x_1,x_2)\,aV_0\delta(x_1-x_2)\,\Psi(x_1,x_2)\,\dd x_1\dd x_2\\={}&aV_0\int_0^a |\Psi(x,x)|^2\,\dd x.\end{aligned}
\]
...
\[
\begin{aligned}E_1^{(1)}={}&aV_0\int_0^a \bigl|\sqrt2\,\psi_1(x)\psi_2(x)\bigr|^2\dd x\\={}&2aV_0\int_0^a |\psi_1(x)|^2|\psi_2(x)|^2\,\dd x\\={}&2aV_0\left({2\over a}\right)^2\int_0^a \sin^2{\pi x\over a}\sin^2{2\pi x\over a}\,\dd x.\end{aligned}
\]
...
\[
\boxed{\begin{aligned}E_1^{(1)}&=2aV_0\cdot{16\over a^2}\cdot{a\over16}\\&=2V_0.\end{aligned}}
\]
\end{solution}
\end{question}

\begin{question}
将微扰理论用于一般二能级系统。设未微扰哈密顿量有两个非简并能级,微扰矩阵元为 $V_{\ii j}$。
\begin{enumerate}
\item 求精确本征值。
...
\[
\begin{aligned}\sqrt{\left({A-D\over2}\right)^2+\lambda^2|V_{12}|^2}\\=\sqrt{\left({(E_1^0-E_2^0)+\lambda(V_{11}-V_{22})\over2}\right)^2+\lambda^2|V_{12}|^2}\\={E_2^0-E_1^0\over2}\sqrt{1+{2\lambda(V_{11}-V_{22})\over E_2^0-E_1^0}+{\lambda^2\bigl[(V_{11}-V_{22})^2+4|V_{12}|^2\bigr]\over (E_2^0-E_1^0)^2}}.\end{aligned}
\]
用 $\sqrt{1+x}=1+x/2-x^2/8+\cdots$ 展开到 $\lambda^3$ 项:
\[
\begin{aligned}\sqrt{\cdots}={}&{\Delta\over2}\Bigl[1+{\lambda(V_{11}-V_{22})\over\Delta}\\&+{\lambda^2\over2\Delta^2}\bigl(4|V_{12}|^2-(V_{11}-V_{22})^2\bigr)+\cdots\Bigr].\end{aligned}
\]
...
\[
\boxed{\begin{aligned}E_+={}&E_2^0+\lambda V_{22}+\lambda^2{|V_{12}|^2\over E_2^0-E_1^0}\\&+\lambda^3{|V_{12}|^2(V_{11}-V_{22})\over (E_2^0-E_1^0)^2}+O(\lambda^4).\end{aligned}}
\]
这与非简并微扰公式逐项一致。
...
={8m\alpha^2\over\pi^2\hbar^2}\sin^2{n\pi\over2}
...
代入得
\[
\boxed{E_n^{(2)}=-{8m\alpha^2\over\pi^2\hbar^2}\cdot{\pi^2\over8n^2}
=-{m\alpha^2\over\hbar^2 n^2}\quad(\text{对奇数 }n).}
\]
...
\approx -{\hbar^2\over2m}{\psi_{\ii+1}-2\psi_i+\psi_{\ii-1}\over h^2}.
\]
总哈密顿量化为 $N\times N$ 三对角矩阵:
\[
H_{\ii j}=
\begin{cases}
{\hbar^2\over m h^2}+V(x_i), & \ii=j,\\[2pt]
-{\hbar^2\over2m h^2}, & |\ii-j|=1,\\[2pt]
0, & \text{else}.
\end{cases}
\]
...
\]
其中 $W_{\ii j}=\langle\psi_i|H'|\psi_j\rangle$。
...
\begin{enumerate}
\item 证明定态为 $L^{-1/2}\ee^{2\pi\ii nx/L}$,除 $n=0$ 外能级二重简并。
...
\[
\psi_n(x)={1\over\sqrt L}\ee^{2\pi\ii nx/L},\qquad
...
\[
\begin{aligned}E_\pm={}&{E_1^0+E_2^0\over2}+{\lambda(V_{11}+V_{22})\over2}\\&\pm{1\over2}\sqrt{\bigl[E_2^0-E_1^0+\lambda(V_{22}-V_{11})\bigr]^2+4\lambda^2|V_{12}|^2}.\end{aligned}
\]
设 $E_2^0>E_1^0$,引入小参数 $\epsilon$ 展开。以 $E_-$ 为例,令 $\Delta=E_2^0-E_1^0$,根号内提 $\Delta^2$:
\[
\begin{aligned}\sqrt{\Delta^2+2\lambda\Delta(V_{22}-V_{11})+\lambda^2[(V_{22}-V_{11})^2+4|V_{12}|^2]}\\=\Delta\sqrt{1+{2\lambda(V_{22}-V_{11})\over\Delta}+{\lambda^2[(V_{22}-V_{11})^2+4|V_{12}|^2]\over\Delta^2}}.\end{aligned}
\]
...
\[
\begin{aligned}E_\pm^{(1)}={}&{V_{11}+V_{22}\over2}\\&\pm\sqrt{\left({V_{11}-V_{22}\over2}\right)^2+|V_{12}|^2}\end{aligned}
\]
...
外积矩阵的秩为1,只有一个非零本征值:
\[
E^{(1)}=\alpha V_0\sum_{\ii=1}^3|\psi_i(\mathbf r_0)|^2.
\]
...
\end{pmatrix},
\]
对角化得本征值 $E_{1,2}^{(1)}=\pm\epsilon|a|$,好态为 $(|1\rangle\pm \ee^{-\ii\arg a}|2\rangle)/\sqrt2$。若 $E_1^{(1)}=E_2^{(1)}$,则需二阶有效矩阵。这样得到的微扰展开与严格解的幂级数逐项一致。
\end{solution}
\end{question}
...
\]
利用正交归一性 $\langle\psi_i^0|\psi_j^0\rangle=\delta_{\ii j}$:
\[
\sum_{j=1}^n W_{\ii j}a_j = E^{(1)}a_i,\quad
W_{\ii j}\equiv\langle\psi_i^0|H'|\psi_j^0\rangle.
\]
...
\begin{solution}
利用角动量对易关系 $[L_i,L_j]=\ii\hbar\epsilon_{\ii jk}L_k$、$[S_i,S_j]=\ii\hbar\epsilon_{\ii jk}S_k$,以及 $[L_i,S_j]=0$。
...
&=-\ii\hbar\sum_{j,k}\epsilon_{\ii jk}L_kS_j
=\ii\hbar\sum_{j,k}\epsilon_{\ii kj}L_kS_j
...
&=-\ii\hbar\sum_{j,k}\epsilon_{\ii jk}L_jS_k
=\ii\hbar\sum_{j,k}\epsilon_{\ii kj}L_jS_k
...
\]
由于 $(\mathbf S\times\mathbf L)_i=-(\mathbf L\times\mathbf S)_i$(因 $\epsilon_{\ii jk}$ 反对称),两项相消,得
\[
...
\begin{solution}
(a) 判断塞曼效应的强弱需比较外磁场 $B$ 与内磁场 $B_{\rm \ii nt}$ 的大小。氢原子精细结构能量量级为
\[
\Delta E_{\rm fs}\sim \alpha^2 |E_n|,
\]
其中 $\alpha\approx1/137$ 为精细结构常数。设内磁场 $B_{\rm \ii nt}$ 满足 $\mu_B B_{\rm \ii nt}\sim\Delta E_{\rm fs}$,则
\[
B_{\rm \ii nt}\sim {\alpha^2|E_n|\over\mu_B}.
\]
对基态 $n=1$,$|E_1|=13.6\,\mathrm{eV}$,$\mu_B=5.79\times10^{-5}\,\mathrm{eV/T}$,得
\[
B_{\rm \ii nt}\sim {(1/137)^2\times13.6\over5.79\times10^{-5}}\,\mathrm{T}
\approx 12.5\,\mathrm{T}.
\]
因此基态内磁场量级约为十几特斯拉。

(b) 外磁场 $B$ 远小于 $B_{\rm \ii nt}$ 时为弱场(反常)塞曼效应,好态为 $|n\ell j m_j\rangle$,能量修正由朗德 $g_j$ 因子给出:
\[
\Delta E_Z = \mu_B g_j B m_j,\qquad
g_j=1+{j(j+1)+\ell(\ell+1)-s(s+1)\over2j(j+1)}.
\]

(c) 若外磁场远大于 $B_{\rm \ii nt}$,自旋--轨道耦合可视为微扰,好态变为 $|n\ell m_\ell m_s\rangle$,此为强场(帕邢--巴克)极限:
\[
\Delta E_Z = \mu_B B(m_\ell+2m_s).
\]
当 $B$ 与 $B_{\rm \ii nt}$ 量级相当时为中间场区域,需数值对角化。
\end{solution}
\end{question}
...
\begin{solution}
弱场($B\ll B_{\rm \ii nt}$)中好态为 $|n\ell j m_j\rangle$,总能量为
\[
...
\item $2P_{3/2}$:$\ell=1$,$j=3/2$。
  \[
  g=1+{(15/4)+(3/4)-2\over2\cdot(15/4)}=1+{10/4\over15/2}=1+{1\over3}={4\over3}.
  \]
  故
  \[
...
\[
\int \hat r_i\hat r_j\,\dd\Omega = C\delta_{\ii j},
\]
其中 $C$ 由缩并确定:取 $\ii=j$ 并对 $i$ 求和:
\[
\sum_i\int \hat r_i^2\,\dd\Omega = \int \sum_i \hat r_i^2\,\dd\Omega = \int (1)\,\dd\Omega = 4\pi.
\]
左边 $=\sum_i C\delta_{\ii i} = 3C$,故 $C=4\pi/3$。因此
\[
\boxed{\int \hat r_i\hat r_j\,\dd\Omega={4\pi\over3}\delta_{\ii j}.}
\]

对于 $\mathbf a\cdot\hat r)(\mathbf b\cdot\hat r) = \sum_{\ii,j}a_i b_j \hat r_i\hat r_j$,积分得
\[
\begin{aligned}
\int (\mathbf a\cdot\hat r)(\mathbf b\cdot\hat r)\,\dd\Omega
&= \sum_{\ii,j} a_i b_j \int \hat r_i\hat r_j\,\dd\Omega \\
&= \sum_{\ii,j} a_i b_j \cdot {4\pi\over3}\delta_{\ii j}
...
基态氢原子波函数 $\psi_{100}(r)=(\pi a^3)^{-1/2}\ee^{-r/a}$。由于 $b\ll a$($b/a\sim10^{-5}$),在 $r<b$ 的小球内指数因子 $\ee^{-r/a}\approx 1$,因此
\[
...
\]
代入 $a=0.529\,\text{\AA}$,$b=0.84\,\mathrm{fm}=8.4\times10^{-6}\,\text{\AA}$,得 $\Delta E\sim 4\times10^{-9}\,\mathrm{eV}$。该修正为正(能量升高),表示有限核尺寸使库仑吸引在原点附近变弱。这解释了为什么氢原子 $s$ 态的精细结构实验值略偏离点核模型的计算值(Lamb 位移的一部分来源于此)。
\end{solution}
\end{question}
...
\]
其中 $Q_0=1-P_0$ 投影到子空间外。在 $D$ 的基 $\{|\ii\rangle\}$ 下,二阶有效矩阵元为
\[
W^{(2)}_{\ii j} = \sum_{m\notin D} {\langle \ii|H'|m\rangle\langle m|H'|j\rangle\over E_D^0 - E_m^0}.
\]
...
\[
W_{\ii j}=A\langle \ii|xyz|j\rangle = 0,\quad \ii,j\in\{|100\rangle,|010\rangle,|001\rangle\}.
\]
故第一激发三重态的一阶能量修正全部为零。
...
\[
H_{\rm \ii nt} \approx -{2e^2\over4\pi\epsilon_0 R^3}x_1x_2.
\]
...
\end{enumerate}
\begin{solution}
(a) 设简并子空间 $D$ 的维数为 $d$,基 $\{|\ii\rangle\}_{\ii=1}^d$ 满足 $H^0|\ii\rangle=E_D^0|\ii\rangle$。一阶波函数修正只对子空间外的态求和:
\[
|\psi^{(1)}\rangle = \sum_{m\notin D} {\langle m|H'|\psi^{(0)}\rangle\over E_D^0-E_m^0}|m\rangle.
\]

(b) 若一阶简并微扰矩阵 $W_{\ii j}=\langle \ii|H'|j\rangle$ 的本征值仍有简并,则 $W$ 不能唯一确定好态。将 $\psi^{(0)}=\sum_{\ii=1}^d a_i|\ii\rangle$ 代入二阶方程。通过投影算符技术,在子空间 $D$ 中得到有效本征方程:
\[
\sum_{j=1}^d W^{(2)}_{\ii j} a_j = E^{(2)} a_i,
\]
其中二阶有效矩阵为
\[
\boxed{W^{(2)}_{\ii j} = \sum_{m\notin D} {\langle \ii|H'|m\rangle\langle m|H'|j\rangle\over E_D^0 - E_m^0}.}
\]
其本征值给出二阶能量修正,本征矢给出好态 $\psi^{(0)}$ 的系数。

推导要点:将 $\psi=\psi^{(0)}+\lambda\psi^{(1)}+\lambda^2\psi^{(2)}+\cdots$ 和 $E=E_D^0+\lambda E^{(1)}+\lambda^2 E^{(2)}+\cdots$ 代入薛定谔方程,收集 $\lambda^2$ 项。左乘 $\langle \ii|$,利用 $\langle \ii|H^0=E_D^0\langle \ii|$ 和一阶结果消去 $E^{(1)}$ 项,即得上述方程。

(c) 用于习题 7.39:取 $D=\{|1\rangle,|2\rangle\}$,$W^{(2)}_{\ii j}={\langle \ii|H'|3\rangle\langle 3|H'|j\rangle\over E_0-E_3}$,得 $2\times2$ 矩阵。对角化后本征值 $\lambda^2(|a|^2+|b|^2)/(E_0-E_3)$ 和 $0$,与严格展开一致。
\end{solution}
\end{question}
...
\[
\psi_n(x)={1\over\sqrt L}\ee^{2\pi \ii n x/L}.
\]
加入微扰
\[
...
\begin{solution}
(a) 圆环上的未微扰态为 $\psi_n(x)=L^{-1/2}\ee^{2\pi \ii n x/L}$,$n=0,\pm1,\pm2,\dots$,能量 $E_n^{(0)}={\hbar^2\over2m}(2\pi n/L)^2$。$n\ne0$ 时 $\psi_n$ 与 $\psi_{-n}$ 简并。

(b) 微扰 $H'=V_0\cos(2\pi x/L)=V_0(\ee^{2\pi \ii x/L}+\ee^{-2\pi \ii x/L})/2$。矩阵元:
\[
\begin{aligned}
H'_{mn} &= \langle\psi_m|H'|\psi_n\rangle
= {V_0\over L}\int_0^L \ee^{-2\pi \ii mx/L}\cos{2\pi x\over L}\,\ee^{2\pi \ii nx/L}\,\dd x \\[4pt]
&= {V_0\over2L}\int_0^L \left[\ee^{2\pi \ii (n-m+1)x/L}+\ee^{2\pi \ii (n-m-1)x/L}\right]\dd x \\[4pt]
...
\[
W^{(2)}_{\ii j} = \sum_{m\notin D} {\langle \ii|H'|m\rangle\langle m|H'|j\rangle\over E_D^{(0)}-E_m^{(0)}},\quad \ii,j\in\{1,-1\}.
\]
...
\[
{1\over|\mathbf R-\mathbf r|} = {1\over R} + {\mathbf r\cdot\mathbf R\over R^3} + {1\over2}\sum_{\ii,j}{3R_iR_j-R^2\delta_{\ii j}\over R^5}r_ir_j + \cdots.
\]
...
在弱磁场中,塞曼哈密顿量为
\[
H_Z = {\mu_B B\over\hbar}(L_z+2S_z) + {e^2B^2\over8m}\sum_{\ii=1}^2(x_i^2+y_i^2).
\]
第一项(线性 Zeeman 项)在 $|L=0,S=0\rangle$ 态上的期望值为零,故一阶能量修正为零。

二阶主要来自抗磁项($B^2$ 项):
\[
\Delta E = {e^2B^2\over8m}\sum_{\ii=1}^2\langle x_i^2+y_i^2\rangle.
\]
...
\[
\psi_{100}(\mathbf r) = {Z^{3/2}\over\sqrt{\pi a_0^3}}\ee^{-Zr/a_0},
\]
...
(b) 第一激发态由 $\psi_{10}(x,y)=\sqrt{2m\omega\over\pi\hbar}\,x\,\ee^{-m\omega(x^2+y^2)/(2\hbar)}$ 和 $\psi_{01}$($x\leftrightarrow y$)张成,能量 $2\hbar\omega$,二重简并。
...
= {qB\hbar\over2m}\begin{pmatrix}0 & -\ii\\ \ii & 0\end{pmatrix}.
\]
...
\quad \psi_{100}(\mathbf r) = {Z^{3/2}\over\sqrt{\pi a^3}}\ee^{-Zr/a}.
\]
一阶排斥能量为
\[
...
\[
E^{(0)} = 2\times\left(-Z^2\cdot{\mathrm{Ry}}\right) = -8\,\mathrm{Ry} = -108.8\,\mathrm{eV}.
\]
故一阶总能量
\[
...
(b) 变分法改进:使用有效核电荷 $Z_{\rm eff}$ 的试探波函数 $\psi(\mathbf r_1,\mathbf r_2) = {Z_{\rm eff}^3\over\pi a^3}\ee^{-Z_{\rm eff}(r_1+r_2)/a}$。能量期望值写为
\[
...
\]
动能项期望值 $\langle T\rangle = 2\times Z_{\rm eff}^2\mathrm{Ry} = 2Z_{\rm eff}^2\mathrm{Ry}$(每个电子 $Z_{\rm eff}^2\mathrm{Ry}$);
势能项(核吸引)$\langle V_{\rm nucl}\rangle = 2\times(-2Z\cdot Z_{\rm eff})\mathrm{Ry} = -8Z_{\rm eff}\mathrm{Ry}$($Z=2$);
电子排斥 $\langle V_{ee}\rangle = {5\over4}Z_{\rm eff}\mathrm{Ry}$。
故
\[
\langle H\rangle = \bigl[2Z_{\rm eff}^2 - 8Z_{\rm eff} + {5\over4}Z_{\rm eff}\bigr]\mathrm{Ry}
= \bigl[2Z_{\rm eff}^2 - {27\over4}Z_{\rm eff}\bigr]\mathrm{Ry}.
\]
对 $Z_{\rm eff}$ 求极值:
\[
{\dd\langle H\rangle\over\dd Z_{\rm eff}} = 4Z_{\rm eff} - {27\over4} = 0
\quad\Longrightarrow\quad Z_{\rm eff} = {27\over16} \approx 1.69.
\]
代入得 $\langle H\rangle_{\min} = -{729\over128}\,\mathrm{Ry} \approx -77.5\,\mathrm{eV}$,与实验值 $-79.0\,\mathrm{eV}$ 更接近。
...
\[
\left[-{\hbar^2\over2m}{\dd^2\over\dd x^2} + V(x)\right] \ee^{\ii qx}u_{nq} = E_{nq} \ee^{\ii qx}u_{nq}.
\]
对 $\ee^{\ii qx}u_{nq}$ 求导:
\[
{\dd\over\dd x}(\ee^{\ii qx}u) = \ee^{\ii qx}(\ii q + \partial_x)u,
\qquad
{\dd^2\over\dd x^2}(\ee^{\ii qx}u) = \ee^{\ii qx}(\ii q+\partial_x)^2u.
\]
代入得关于 $u_{nq}$ 的本征值方程:
\[
\left[{1\over2m}(-\ii\hbar\partial_x + \hbar q)^2 + V(x)\right]u_{nq}(x) = E_{nq} u_{nq}(x).
\]

(b) 对于小 $q$(布里渊区中心附近),将 $q$ 视为小参数展开。令
\[
H^0 = {p^2\over2m} + V(x),\qquad p = -\ii\hbar\partial_x,
\]
\[
...
一阶系数:$B_n = {\hbar\over m}\langle u_n^{(0)}|p|u_n^{(0)}\rangle$。在周期边界下可选取 $u_n^{(0)}$ 为实函数,而 $p=-\ii\hbar\partial_x$ 为纯虚算符,故期望值为零:
\[
B_n = 0.
\]
