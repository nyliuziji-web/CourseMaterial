\heading{第12章-跋}
\begin{question}
纠缠态(Entangled states)。纠缠态的一个经典例子是自旋单态(式 (12.1)):两个粒子体系的状态不能写成两个单粒子状态波函数的乘积。考虑两能级体系 $|\phi_a\rangle$、$|\phi_b\rangle$,且 $\langle\phi_i|\phi_j\rangle=\delta_{\ii j}$。证明:对于任意单粒子状态 $|\psi_r\rangle$ 和 $|\psi_s\rangle$,两粒子体系状态
\[
...
\[
\ii\hbar\frac{\dd\hat\rho}{\dd t}=[\hat H,\hat\rho].
\]
...
\[
\ii\hbar\frac{\dd}{\dd t}|\psi(t)\rangle = H|\psi(t)\rangle,
\]
其厄米共轭为
\[
-\ii\hbar\frac{\dd}{\dd t}\langle\psi(t)| = \langle\psi(t)|H.
\]
...
&= \Bigl(\frac{1}{\ii\hbar}H|\psi\rangle\Bigr)\langle\psi| + |\psi\rangle\Bigl(-\frac{1}{\ii\hbar}\langle\psi|H\Bigr)\\[2mm]
&= \frac{1}{\ii\hbar} H|\psi\rangle\langle\psi| - \frac{1}{\ii\hbar} |\psi\rangle\langle\psi|H \\[2mm]
&= \frac{1}{\ii\hbar}(H\rho - \rho H) = \frac{1}{\ii\hbar}[H,\rho].
\end{aligned}
\]
两边乘以 $\ii\hbar$,得
\[
\boxed{\ii\hbar\frac{\dd\rho}{\dd t} = [H,\rho].}
\]
...
\[
\frac{\dd}{\dd t}|\psi_i\rangle\langle\psi_i| = \frac{1}{\ii\hbar}[H,|\psi_i\rangle\langle\psi_i|].
\]
取概率加权和:
\[
\frac{\dd\rho}{\dd t} = \sum_i p_i\frac{\dd}{\dd t}|\psi_i\rangle\langle\psi_i|
= \frac{1}{\ii\hbar}\sum_i p_i [H,|\psi_i\rangle\langle\psi_i|]
= \frac{1}{\ii\hbar}[H,\rho].
\]
...
\sigma_y = \begin{pmatrix}0&-\ii\\i&0\end{pmatrix},\quad
...
\hat y\cdot\boldsymbol\sigma = \sigma_y = \begin{pmatrix}0&-\ii\\i&0\end{pmatrix}.
\]
解本征方程:
\[
\begin{pmatrix}0&-\ii\\i&0\end{pmatrix}\begin{pmatrix}a\\b\end{pmatrix} = -\begin{pmatrix}a\\b\end{pmatrix}
\;\Longrightarrow\;
\begin{cases}
-\ii b = -a,\\
\ii a = -b,
\end{cases}
\]
两式等价,给出 $a = \ii b$。归一化得 $|a|^2+|b|^2 = 2|b|^2 = 1$,故取
\[
\chi_{-y} = \frac{1}{\sqrt{2}}\begin{pmatrix}1\\-\ii\end{pmatrix}.
\]

密度矩阵为
\[
\rho = \chi_{-y}\chi_{-y}^\dagger
= \frac{1}{2}\begin{pmatrix}1\\-\ii\end{pmatrix}\begin{pmatrix}1&\ii\end{pmatrix}
= \frac{1}{2}\begin{pmatrix}1 & \ii\\ -\ii & 1\end{pmatrix}.
\]
...
对于 $S_x$:
\[
\rho\sigma_x = \frac{1}{2}\begin{pmatrix}1&\ii\\-\ii&1\end{pmatrix}\begin{pmatrix}0&1\\1&0\end{pmatrix}
= \frac{1}{2}\begin{pmatrix}\ii&1\\1&-\ii\end{pmatrix},
\]
\[
\operatorname{Tr}(\rho\sigma_x) = \frac{1}{2}(\ii - \ii) = 0
\;\Longrightarrow\; \langle S_x\rangle = 0.
\]

对于 $S_y$:
\[
\rho\sigma_y = \frac{1}{2}\begin{pmatrix}1&\ii\\-\ii&1\end{pmatrix}\begin{pmatrix}0&-\ii\\i&0\end{pmatrix}
...
对于 $S_z$:
\[
\rho\sigma_z = \frac{1}{2}\begin{pmatrix}1&\ii\\-\ii&1\end{pmatrix}\begin{pmatrix}1&0\\0&-1\end{pmatrix}
= \frac{1}{2}\begin{pmatrix}1&-\ii\\-\ii&-1\end{pmatrix},
\]
\[
...
\[
\rho P_{+\hat n} = \frac{1}{4}\bigl(I-\ii\sigma_y\bigr)\bigl(I+\hat n\cdot\boldsymbol\sigma\bigr),
\]
其中利用了 $\rho = \frac{1}{2}(I-\ii\sigma_y)$(因 $\chi_{-y}$ 对应的 Bloch 矢量为 $-\hat y$)。展开并取迹,利用 $\operatorname{Tr}\sigma_i=0$,$\operatorname{Tr}(\sigma_i\sigma_j)=2\delta_{\ii j}$:
\[
\begin{aligned}
\operatorname{Tr}(\rho P_{+\hat n})
&= \frac{1}{4}\operatorname{Tr}\bigl[I - \ii\sigma_y + \hat n\cdot\boldsymbol\sigma - \ii(\hat n\cdot\boldsymbol\sigma)\sigma_y\bigr]\\[2mm]
&= \frac{1}{4}\bigl(2 + 0 + 0 - \ii\cdot 2 n_y\bigr)
= \frac{1}{2}(1 - n_y).
\end{aligned}
\]
...
\sigma_x = \begin{pmatrix}0&1\\1&0\end{pmatrix},\quad
\sigma_y = \begin{pmatrix}0&-\ii\\i&0\end{pmatrix},
\]
得
\[
\rho = \frac{1}{2}\left[
...
- \frac{2}{3}\begin{pmatrix}0&-\ii\\i&0\end{pmatrix}
\right].
\]
计算各矩阵元:
\begin{itemize}
\item $\rho_{11} = \frac{1}{2}(1 + 0 + 0) = \frac{1}{2}$
\item $\rho_{12} = \frac{1}{2}\left(0 + \frac{1}{3}\cdot 1 - \frac{2}{3}\cdot(-\ii)\right) = \frac{1}{2}\left(\frac{1}{3} + \frac{2i}{3}\right) = \frac{1}{6} + \frac{\ii}{3}$
\item $\rho_{21} = \frac{1}{2}\left(0 + \frac{1}{3}\cdot 1 - \frac{2}{3}\cdot \ii\right) = \frac{1}{2}\left(\frac{1}{3} - \frac{2i}{3}\right) = \frac{1}{6} - \frac{\ii}{3}$
...
\]
利用 $\operatorname{Tr}\sigma_y = 0$ 和 $\operatorname{Tr}(\sigma_i\sigma_j)=2\delta_{\ii j}$:
\[
...
\frac{\ii(1+2i)}{3} & -\ii \\[4pt]
\ii & -\frac{\ii(1-2i)}{3}
\end{pmatrix},
\]
取迹:
\[
\operatorname{Tr}(\rho\sigma_y) = \frac{1}{2}\left(\frac{\ii(1+2i)}{3} - \frac{\ii(1-2i)}{3}\right)
= \frac{1}{2}\cdot\frac{\ii(1+2i-1+2i)}{3}
...
\[
\rho=\frac12\begin{pmatrix}1+a_3&a_1-\ii a_2\\a_1+\ii a_2&1-a_3\end{pmatrix}=\frac12(1+\mathbf a\cdot\boldsymbol\sigma),
\]
...
\sigma_2 = \begin{pmatrix}0&-\ii\\i&0\end{pmatrix},\quad
...
+ a_2\begin{pmatrix}0&-\ii\\i&0\end{pmatrix}
...
\item $\rho_{12} = \frac12(a_1 - \ii a_2)$
\item $\rho_{21} = \frac12(a_1 + \ii a_2)$
...
1 & \cos\phi - \ii\sin\phi\\
\cos\phi + \ii\sin\phi & 1
\end{pmatrix}
...
= \frac12\begin{pmatrix}1 & \ee^{-\ii\phi}\\ \ee^{\ii\phi} & 1\end{pmatrix}.
\]
比较得 $|a|^2 = |b|^2 = \frac12$,且 $ab^* = \frac12 \ee^{-\ii\phi}$。可取 $a = \frac{1}{\sqrt{2}}$,$b = \frac{1}{\sqrt{2}}\ee^{\ii\phi}$(整体相位自由),因此
\[
\boxed{\chi = \frac{1}{\sqrt{2}}\begin{pmatrix}1\\ \ee^{\ii\phi}\end{pmatrix},}
\]
