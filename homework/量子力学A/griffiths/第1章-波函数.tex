...
\item 归一化条件给出
\[
1=A\int_{-\infty}^{\infty}\ee^{-\lambda(x-a)^2}\dd x
...
考虑波函数
\[
\Psi(x,t)=A \ee^{-\lambda |x|} \ee^{-\ii\omega t},
\]
...
\item 因为
\[
1=|A|^2\int_{-\infty}^{\infty}\ee^{-2\lambda |x|}\dd x
=|A|^2\frac{1}{\lambda},
\]
可取
\[
A=\sqrt{\lambda}.
\]
\item 概率密度 $|\Psi|^2=\lambda \ee^{-2\lambda |x|}$ 是偶函数,所以
\[
\langle x\rangle=0.
\]
又
\[
\langle x^2\rangle=2\lambda\int_0^\infty x^2\ee^{-2\lambda x}\dd x
...
\[
P(|x|>\sigma_x)=2\lambda\int_{\sigma_x}^{\infty}\ee^{-2\lambda x}\dd x
=\ee^{-2\lambda\sigma_x}=\ee^{-\sqrt2}.
\]
\end{enumerate}
\end{solution}
...
\end{question}
\begin{solution}
动量算符为 $\hat p=-\ii\hbar\partial_x$。从
\[
\langle p\rangle=\int \Psi^*(-\ii\hbar\partial_x)\Psi\,\dd x
\]
出发,对时间求导,并用
\[
\ii\hbar\frac{\partial\Psi}{\partial t}= -\frac{\hbar^2}{2m}\frac{\partial^2\Psi}{\partial x^2}+V\Psi,
\qquad
-\ii\hbar\frac{\partial\Psi^*}{\partial t}= -\frac{\hbar^2}{2m}\frac{\partial^2\Psi^*}{\partial x^2}+V\Psi^*
\]
...
\begin{question}
假定在势能项中增加了一个常数 $V_0$(这里常数的意思是它不依赖 $x$ 和 $t$)。在经典力学中这不改变任何结论,但在量子力学中是什么样的情况?证明波函数将多一个含时的相因子 $\exp(-\ii V_0t/\hbar)$。它对动力学变量的期望值有何影响?
\end{question}
\begin{solution}
设 $\Psi$ 是势能 $V(x,t)$ 下的解。若把势能改成 $V+V_0$,其中 $V_0$ 为常数,令
\[
\Psi'(x,t)=\Psi(x,t)\ee^{-\ii V_0t/\hbar}.
\]
则
\[
\ii\hbar\frac{\partial\Psi'}{\partial t}
=\ee^{-\ii V_0t/\hbar}\left(\ii\hbar\frac{\partial\Psi}{\partial t}+V_0\Psi\right)
...
\item 直接代入薛定谔方程。记
\[
\Psi=A\ee^{-amx^2/\hbar}\ee^{-\ii at}.
\]
左边为
\[
\ii\hbar\partial_t\Psi=a\hbar\Psi.
\]
又
\[
...
其中,
\[
J(x,t)=\frac{\ii\hbar}{2m}\left(\Psi\frac{\partial\Psi^*}{\partial x}-\Psi^*\frac{\partial\Psi}{\partial x}\right).
\]
...
其中
\[
J(x,t)=\frac{\ii\hbar}{2m}\left(\Psi\frac{\partial\Psi^*}{\partial x}
...
\[
\ii\hbar\partial_t\Psi_j=\left(-\frac{\hbar^2}{2m}\partial_x^2+V\right)\Psi_j,
\qquad j=1,2.
\]
则
\[
...
\[
\frac{\ii\hbar}{2m}\int \left[(\partial_x^2\Psi_1^*)\Psi_2-
...
\item
\[
\langle p\rangle=\int_{-a}^a\Psi(-\ii\hbar\partial_x)\Psi\dd x
=-\frac{\ii\hbar}{2}\int_{-a}^a\partial_x(\Psi^2)\dd x=0,
\]
...
\[
P(t)=\int_{-\infty}^{\infty}|\Psi(x,t)|^2\dd x=\ee^{-t/\tau}.
\]
...
\item 解这个微分方程:
\[
P(t)=P(0)\ee^{-2\Gamma t/\hbar}.
\]
若初始归一化 $P(0)=1$,则
\[
P(t)=\ee^{-2\Gamma t/\hbar}.
\]
与 $P(t)=\ee^{-t/\tau}$ 比较,得到
\[
