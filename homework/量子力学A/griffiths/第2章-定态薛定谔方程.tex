...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.132):$\delta$ 势阱 $V(x)=-\alpha\delta(x)$ 的束缚态:$\psi(x)=\sqrt{m\alpha}/\hbar\,\ee^{-m\alpha|x|/\hbar^2}$,$E=-m\alpha^2/(2\hbar^2)$。}
...
\par\smallskip
\noindent{\kaishu 为避免查阅正文,本题中引用的公式为:式 (2.154):有限深方势阱偶束缚态可取 $\psi(x)=F \ee^{\kappa x}$($x<-a$),$D\cos(\ell x)$($|x|<a$),$F \ee^{-\kappa x}$($x>a$)。}
