...
\[
\psi_{100}(\mathbf r) = {Z^{3/2}\over\sqrt{\pi a_0^{3/2}}}\ee^{-Zr/a_0},
\]
...
\[
\langle Q\rangl\ee^{(1)} = \sum_{m\ne n} {H'_{nm} Q_{mn} + Q_{nm} H'_{mn}\over E_n^{(0)}-E_m^{(0)}}.
\]
当 $Q=H'$ 时,$H'_{nm} H'_{mn}=|H'_{mn}|^2$,且两项相同,故
\[
\langle H'\rangl\ee^{(1)} = 2\sum_{m\ne n}{|H'_{mn}|^2\over E_n^{(0)}-E_m^{(0)}} = 2E_n^{(2)}.
\]
这不矛盾:总能量 $E_n = E_n^{(0)} + \langle H'\rangl\ee^{(0)} + \langle H'\rangl\ee^{(1)} + \cdots$,其中 $\langle H'\rangl\ee^{(0)}=E_n^{(1)}$ 是零阶波函数下的一阶能修,而 $\langle H'\rangl\ee^{(1)}=2E_n^{(2)}$ 来自波函数一阶修正的贡献,两者不同。

(b) 对 $H'=-q\mathcal Ex$(外电场中的带电谐振子),诱导电偶极矩为
\[
p = q\langle x\rangle = q\bigl[\langle x\rangl\ee^{(0)} + \langle x\rangl\ee^{(1)} + \cdots\bigr].
\]
基态 $\langle x\rangl\ee^{(0)}=0$,一阶修正为
\[
\langle x\rangl\ee^{(1)} = 2\sum_{m\ne 0} {\langle0|x|m\rangle\langle m|H'|0\rangle\over E_0^{(0)}-E_m^{(0)}}
...
(c) 对非谐微扰 $H' = kx^3/6$,计算 $\langle x\rangl\ee^{(1)}$。用升降算符表示 $x=\sqrt{\hbar/(2m\omega)}(a+a^\dagger)$,$x^3$ 展开为 $a^3$、$a^{\dagger3}$、$a^\dagger a^2$ 等项的线性组合,联结 $m=n\pm1,n\pm3$。求和后得
\[
\langle x\rangl\ee^{(1)} = \sum_{m\ne n} {2\,\mathrm{Re}[\langle n|x|m\rangle\langle m|H'|n\rangle]\over E_n^{(0)}-E_m^{(0)}}.
\]
计算得 $\langle x\rangl\ee^{(1)} \propto n$(与量子数 $n$ 成正比),表示非谐性导致谐振子各能级的平均位移不同,对应经典非谐振子的热膨胀效应。
\end{solution}
...
\quad \psi_{100}(\mathbf r) = {Z^{3/2}\over\sqrt{\pi a^{3/2}}}\ee^{-Zr/a}.
\]
一阶排斥能量为
\[
